...
function TimeToString   (Time: Cardinal; TimeFormat: String = {$IFDEF VIPLAY}'hh:mm:ss'{$ELSE}'hh:mm:ss,zzz'{$ENDIF}): String;
...
var I: longword;
    tmpStr: string;
    tmpSource, tmpfind: string;
begin
  Result := 0;

  if length(aFindString) > length(aSourceString) then exit;

  TmpSource := aSourceString;
  TmpFind := aFindString;

  if not CaseSensitive then
  begin
    TmpSource := uppercase(TmpSource);
    TmpFind := uppercase(TmpFind);
  end;

  for I := 1 to length(TmpSource) - length(TmpFind)  + 1 do
  begin
    tmpStr := copy(TmpSource, I, length(TmpFind));
    if tmpStr = TmpFind then inc(Result);
  end;
end;
...
Begin
  ...
  Result := S;
  while System.length(Result) < Length do Result := PadChar + Result;

  if Cut then
  while System.length(Result) > Length do delete(Result,1,1);
end;
...
begin
  ...
  Result := S;
  while System.length(Result) < Length do Result := Result + PadChar;

  if Cut then
  while System.length(Result) > Length do delete(Result,1,System.Length(Result));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      TimeFormat := Copy(ReplaceString(TimeFormat,
        StringOfChar('z', tmpApariciones),
        Copy(PadLeft(IntToStr(MSec),
        '0', 3), 0, tmpApariciones)), 0, tmp + tmpApariciones-1);
    end;
    Result := TimeFormat;
  end;
end;
...
begin
{$If CompilerVersion >= 22}
  Result := ReplaceStr(S, #13#10, NewPattern);
{$else}
