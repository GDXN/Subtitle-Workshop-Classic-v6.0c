...
type
  ...
  public
    ...
    procedure Set_UTF8_File(value: Longbool); // <--------- added by Kameleon
    function  Get_UTF8_File: longbool; // <------ added by Kameleon
  end;
...
begin
  ...
  Begin
    ...
    if AllSupportedText <> '' then
      Result := AllSupportedText + '|' {' (' + AllExts + ')|'} + AllExts + '|'
      + 'All Files (*.*)|*.*|'
      + Result;
  End;
end;
...
// <------ added by Kameleon
procedure TSubTitleAPI.Set_UTF8_File(value: LongBool);
var proc_: procedure (value:longbool); stdCall;
begin
  if FInstance <> 0 then
  begin
    proc_ := GetProcAddress(FInstance, 'Set_UTF8_File');
    if @proc_ <> nil then proc_(value);
  end;
end;

function TSubTitleAPI.Get_UTF8_File : LongBool;
var proc_: function :longbool; stdCall;
begin
  Result := false;
  if FInstance <> 0 then
  begin
    proc_ := GetProcAddress(FInstance, 'Get_UTF8_File');
    if @proc_ <> nil then Result := proc_;
  end;
end;
// end Kameleon
