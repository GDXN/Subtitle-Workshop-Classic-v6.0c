...
uses
  Forms, Windows, Classes, SysUtils, StdCtrls, Mask, IniFiles, ComCtrls, ExtCtrls,
  ShellApi, Controls, Math, Graphics, {JvRichEditToHtml,} Messages, RichEdit,
  ClipBrd, //Graphics, JvRichEditToHtml, Messages, RichEdit, ClipBrd added by adenry
  VirtualTrees,
  StrMan, FastStrings,
...
//Clipboard
function GetUnicodeTextFromClipboard: String; //added by adenry
// ---------------------//
//   Search & Replace   //
// ---------------------//
...
begin
  ...
  begin
    ...
    begin
      if Pos(Text[i],'0123456789+-E' + System.SysUtils.FormatSettings.DecimalSeparator) = 0 then
      begin
        Result := False;
        Break;
      end;
...
//    W.Caption      := ID_PROGRAM;
    ...
    But1.Parent      := W;
    But1.Caption     := BCap1;
    But1.Font.Size   := W.Font.Size; // Kameleon
    But1.ModalResult := 1;
    ...
    begin
      ...
      But2.Parent      := W;
      But2.Caption     := BCap2;
      But2.Font.Size   := W.Font.Size; // Kameleon
      But2.ModalResult := 2;
      If BCap3 <> '' then
      begin
        ...
        But3.Parent      := W;
        But3.Caption     := BCap3;
        But3.Font.Size   := W.Font.Size; // Kameleon
        But3.ModalResult := 3;
        But3.Cancel      := True;
      end else
      But2.Cancel := True;
...
var
  f : File;
  // Temporary values
  TempFormatTest : array[0..2] of AnsiChar; // should be "AVI"    // Kameleon
...
begin
  if SWTextEdit.ClassName = TRichEdit.ClassName  //if TSWTextEdit.InheritsFrom(TRichEdit) then
  then //Result := GetRichEditText(TRichEdit(SWTextEdit)) else
    Result := SWTextEdit.Text
  else Result := SWTextEdit.Text;
end;
//added by adenry: end
...
begin
  if SWTextEdit.ClassName = TRichEdit.ClassName then //if TSWTextEdit.InheritsFrom(TRichEdit) then
    {SetRichEditSelText(TRichEdit(SWTextEdit), Text)}
...
//added by adenry: begin
function GetUnicodeTextFromClipboard:String;

  function WideStrAlloc(Size: Cardinal): PChar;
  begin
  ...
  function StrLCopyW(Dest: PChar; const Source: PChar; MaxLen: Cardinal): PChar;
  var
    Src: PChar;
  begin
    Result := Dest;
