...
{-$DEFINE LOGFILES}// Kameleon
{-$DEFINE EXPORTGRAPH}// Kameleon

uses
  Forms, Windows, Controls, ComCtrls, ExtCtrls, ActiveX, DirectShow9, SysUtils,
  Menus, Classes, {Graphics,} // Graphics added by adenry for Screengrab;
  VirtualTrees, // VirtualTrees added by adenry
  _CommonTypes, formVariousInfo;
...
const
  ...
  CLSID_GabestMatroskaSplitter
    : TGUID = '{0A68C3B5-9164-4A54-AFAF-995B2FF0E0D4}';
  CLSID_GabestRealMediaSplitter
    : TGUID = '{E21BE468-5C18-43EB-B0CC-DB93A847D769}';
  ...
  // by BDZL
  CLSID_HaaliMediaSplitter2: TGUID = '{564FD788-86C9-4444-971E-CC4A243DA150}';
  // by Kameleon
...
const // Kameleon
  NrFilterNames = 23;
  FilterNames: array[0..NrFilterNames - 1] of FilterName =
  (
    (GUID: '{1B544C20-FD0B-11CE-8C63-00AA0044B51E}'; Name: 'AVISplitter'),
    (GUID: '{F07E245F-5A1F-4D1E-8BFF-DC31D84A55AB}'; Name: 'OGGSplitter'),
    (GUID: '{336475D0-942A-11CE-A870-00AA002FEAB5}'; Name: 'MPEG1Splitter'),
    (GUID: '{3AE86B20-7BE8-11D1-ABE6-00A0C905F375}'; Name: 'MPEG2Splitter'),
    (GUID: '{9736D831-9D6C-4E72-B6E7-560EF9181001}'; Name: 'GabestAviSplitter'),
    (GUID: '{0A68C3B5-9164-4A54-AFAF-995B2FF0E0D4}'; Name: 'GabestMatroskaSplitter'),
    (GUID: '{04FE9017-F873-410E-871E-AB91661A4EF7}'; Name: 'ffdshowVideDecoder'),
    (GUID: '{55DA30FC-F16B-49FC-BAA5-AE59FC65F82D}'; Name: 'HaaliMediaSplitter'),
    (GUID: '{564FD788-86C9-4444-971E-CC4A243DA150}'; Name: 'HaaliMediaSplitter (AR)'),
    (GUID: '{70E102B0-5556-11CE-97C0-00AA0055595A}'; Name: 'VideoRenderer'),
    (GUID: '{B87BEB7B-8D29-423F-AE4D-6582C10175AC}'; Name: 'Video Mixing Renderer (VMR 7)'),
    (GUID: '{51b4abf3-748f-4e3b-a276-c828330e926a}'; Name: 'Video Mixing Renderer (VMR 9)'),
    (GUID: '{171252A0-8820-4AFE-9DF8-5C92B2D66B04}'; Name: 'LAVSplitter'),
    (GUID: '{E21BE468-5C18-43EB-B0CC-DB93A847D769}'; Name: 'GabestRealMediaSplitter'),
    (GUID: '{E1F1A0B8-BEEE-490D-BA7C-066C40B5E2B9}'; Name: 'Microsoft DTV-DVD Audio Decoder'),
    (GUID: '{0F40E1E5-4F79-4988-B1A9-CC98794E6B55}'; Name: 'Ffdshow audio decoder'),
    (GUID: '{212690FB-83E5-4526-8FD7-74478B7939CD}'; Name: 'Microsoft MPEG-2 Video Decoder'),
    (GUID: '{79376820-07D0-11CF-A24D-0020AFD79767}'; Name: 'DirectSound Audio Renderer'),
    (GUID: '{48025243-2D39-11CE-875D-00608CB78066}'; Name: 'Internal Script Command Renderer Filter'),
    (GUID: '{EE30215D-164F-4A92-A4EB-9D4C13390F9F}'; Name: 'LAV Video decoder'),
    (GUID: '{E8E73B6B-4CB3-44A4-BE99-4F7BCB96E491}'; Name: 'LAV Audio Decoder'),
    (GUID: '{e436ebb5-524f-11ce-9f53-0020af0ba770}'; Name: 'AsyncReader'),
    (GUID: '{94297043-BD82-4DFD-B0DE-8177739C6D20}'; Name: 'DMO Wrapper Filter')
    );
{$ENDIF}
...
function LoadMovie({ const } FileName: string; AudioStream: SmallInt = 0;
  OverridePlayOnLoad: Boolean = False; VidPos: Int64 = 0): Boolean;
// const removed by adenry, AudioStream, OverridePlayOnLoad, VidPos added by adenry
procedure FreeFile;
procedure SetPlaybackRate(Rate: Double;
  AlterAltPlayRateButton: Boolean = False);
procedure SetDefaultShortCut;
function GetPlaybackRate: Double;
procedure SetVideoPos(

  const Pos: Int64; MoveSeekBar: Boolean = True);
procedure Play;
procedure Pause;
procedure Stop;
function GetCurrentPos: Integer;
procedure SetTimeCounter(
...
// added by adenry
procedure UpdateSubSubtitleVisibilityAfterNodeChange(Node: PVirtualNode;
  OldShowTime, OldHideTime, NewShowTime, NewHideTime: Integer);
// added by adenry
...
var
  ...
  VideoFileSize: Double; // size in Kb Kameleon

  ROT: Integer; // Kameleon

  // -----------------------------------------------------------------------------

implementation

uses General, Functions, USubtitlesFunctions, TreeViewHandle,
  // TreeViewHandle added by adenry
  formMain;
...
begin
  ...
  begin
    ...
    begin
      ...
      pnlVideo.Height := Round(pnlParent1.Height * VideoPanelRatio);
      // modified by adenry //pnlVideo.Height := (pnlParent1.Height div 2) - (spSplitter.Height div 2);
      ...
      if spSplitter.Visible = False then
        spSplitter.Show;
      lstSubtitles.Height := pnlParent1.Height - spSplitter.Top -
        spSplitter.Height;
      ...
      UpdateSubSubtitleTextAndVisibility;
      // added by adenry
    end
    else
    begin
...
begin
  ...
  if fVideoRenderer = nil then // deleted by Kameleon
  ...
  Result := RGB(GetRValue(Ck.HighColorValue), GetGValue(Ck.HighColorValue),
    GetBValue(Ck.HighColorValue));
...
{$IFDEF LOGFILES} // added by Kameleon

function getFilterName(ID: TGUID): string;
var
  I: byte;
begin
  Result := 'Unknown Filter';
  for I := 0 to NrFilterNames - 1 do
  begin
    if IsEqualGUID(ID, FilterNames[I].GUID) then
    begin
      Result := FilterNames[I].Name;
      exit;
    end;
  end;
end;
...
function FindSplitter: IBaseFilter;
  ...
  begin
{$IFDEF LOGFILES}
    AssignFile(Fl, 'LogfileFilters.txt');
    Rewrite(Fl);
    CloseFile(Fl);
{$ENDIF}
    Result := nil;
    ...
    if EnumF <> nil then
      ...
      begin
        ...
        if IsEqualGUID(g, CLSID_AviSplitter) or
          ...
          IsEqualGUID(g, CLSID_ffdshowVideDecoder) or // by BDZL
          IsEqualGUID(g, CLSID_HaaliMediaSplitter) or // by BDZL
          IsEqualGUID(g, CLSID_HaaliMediaSplitter2) or // by Kameleon
          IsEqualGUID(g, CLSID_VideoRenderer) or // by BDZL
          IsEqualGUID(g, CLSID_LAVSplitter) or // by BDZL
          IsEqualGUID(g, CLSID_GabestRealMediaSplitter) then
        begin
...
{$IFDEF LOGFILES}
        ...
        writeln(Fl, '; Total No of audio pins: ', inttostr(Cnt), '  ');
...
{$IFDEF LOGFILES}
    ...
    begin
      ...
      S1 := GetFilterName(g);
      writeln(Fl);
      writeln(Fl, 'Selected audio Filter: ' + S + ' ' + S1);
    end;
    CloseFile(Fl);
{$ENDIF}
...
var
  ...
  Pin2: IPin;
  PinType: TAMMediaType;
  PinType2: PAMMediaType;
  Splitter: IBaseFilter;
  a: byte;
  I: Integer; // added by adenry
  //tmi, tmi2: TMenuItem; // added by adenry // removed by Kameleon
  ...
  StreamSelect: IAMStreamSelect; // Kameleon
  Crd, Crd2, Lcid, PdwGroup: cardinal; // Kameleon
  PdwFlags: DWord; // Kameleon
  Nme: PWideChar; // Kameleon
  Unkn: IUnknown; // Kameleon
  P1, P2: integer; // Kameleon
  TmpS: string; // Kameleon
  NoStreams: boolean;
const
  null: word = 0;
begin
  ...
  begin
    ...
    // begin added by Kameleon
    StreamSelect := nil;
    Splitter.QueryInterface(IID_IAMStreamSelect, StreamSelect);
    // test for the interface presence

    if StreamSelect <> nil then
    begin
      StreamSelect.Count(Crd); // number of streams

      if Crd > 0 then
        for Crd2 := 0 to Crd - 1 do
        begin
          StreamSelect.Info(Crd2, PinType2, PdwFlags, Lcid, PdwGroup, Nme,
            Unkn, Unkn);
          if IsEqualGUID(PinType2.MajorType, MEDIAType_Audio) then
          begin
            SetLength(StreamTitles, a + 1); // added by adenry
            StreamTitles[a] := Nme; // added by adenry - get stream title

            if a = 0 then
              StreamSelect.Enable(Crd2, 0); // disable all audio streams
            if a = StreamNum then
              StreamSelect.Enable(Crd2, AMSTREAMSELECTENABLE_ENABLE); // an enable the correct one

            inc(a); // one more audiostream available
          end;
        end;
      StreamSelect := nil;
      Unkn := nil;
    ...
    NoStreams := a = 0; // Kameleon, no streams detected yet; use the original method

    if NoStreams or (StreamNum < 0) then // Kameleon, still no audio streams detected or audio muted
    begin
      ...
      begin
        ...
        *)
        ...
        begin
          if IsEqualGUID(PinType.MajorType, MEDIAType_Audio) then
          begin
            if NoStreams then // no previously detected audio streams yet, add a detected one
            begin
              SetLength(StreamTitles, a + 1); // added by adenry
            ...
            if a <> StreamNum then
            // changed by adenry - disconnect all audio streams, except the chosen one
            begin
    ...
    if a > 0 then
    begin
      if a > 20 then a := 20; // Kameleon, limited to 20 audio streams, the audio menus have only 21 items

      frmMain.mnuAudioStream.Enabled := True;
      //frmMain.mnuAudioStream.Clear; // Kameleon
      frmMain.mnuAudioStreamPopup.Enabled := True;
      //frmMain.mnuAudioStreamPopup.Clear; // Kameleon

      for I := 0 to frmMain.mnuAudioStream.Count - 1 do
      begin
        if I > a  // a is number of audio streams
        then
        begin
          frmMain.mnuAudioStream.Items[I].Visible := false;
          frmMain.mnuAudioStreamPopup.Items[I].Visible := false;
        end
        else
        begin
          frmMain.mnuAudioStream.Items[I].Visible := true;
          frmMain.mnuAudioStreamPopup.Items[I].Visible := true;
        end;
      end;

      for I := 0 to a do //for all audio streams
      begin
        //tmi := TMenuItem.Create(frmMain); // Kameleon

        // begin Kameleon replacement
        if I = 0
        then TmpS := NoneAudioStream
        else TmpS := StreamTitles[I - 1];
        TmpS := trim(TmpS);
        ...
        if pos(':', TmpS) = 2 then Delete(TmpS, 1, 2);

        TmpS := trim(TmpS);

        frmMain.mnuAudioStream.Items[I].Caption := TmpS;
        frmMain.mnuAudioStream.Items[I].Tag := I - 1;
        frmMain.mnuAudioStream.Items[I].OnClick := frmMain.mnuAudioStreamNClick;
        frmMain.mnuAudioStream.Items[I].AutoCheck := true;
        if (StreamNum = I - 1)
        then frmMain.mnuAudioStream.Items[I].Checked := True
        else frmMain.mnuAudioStream.Items[I].Checked := false;

        frmMain.mnuAudioStreamPopup.Items[I].Caption := TmpS;
        frmMain.mnuAudioStreamPopup.Items[I].Tag := I - 1;
        frmMain.mnuAudioStreamPopup.Items[I].OnClick := frmMain.mnuAudioStreamNClick;
        frmMain.mnuAudioStreamPopup.Items[I].AutoCheck := true;
        if (StreamNum = I - 1)
        then frmMain.mnuAudioStreamPopup.Items[I].Checked := True
        else frmMain.mnuAudioStreamPopup.Items[I].Checked := false;

        // end Kameleon Replacement

        {
        tmi.Tag := I - 1;
        tmi.OnClick := frmMain.mnuAudioStreamNClick;
        tmi.AutoCheck := True;
        if StreamNum = I - 1 then tmi.Checked := True;
...
function AddGraphToRot(UnkGraph: IUnknown; var pdwRegister: Integer): HRESULT;
var
  Moniker: IMoniker;
  ROT: IRunningObjectTable;
  Wsz: array[0..127] of widechar;
  Hr: HRESULT;
begin
  if (FAILED(GetRunningObjectTable(0, ROT))) then
  begin
    Result := E_FAIL;
    exit;
  end;

  wsprintfW(Wsz, 'FilterGraph %08x pid %08x', UnkGraph, GetCurrentProcessId());

  Hr := CreateItemMoniker('!', Wsz, Moniker);

  if SUCCEEDED(Hr) then
  begin
    Hr := ROT.Register(0, UnkGraph, Moniker, pdwRegister);
    Moniker := nil;
  end;
...
procedure RemoveGraphFromRot(pdwRegister: Integer);
var
  ROT: IRunningObjectTable;
begin
...
function LoadMovie({ const } FileName: string; AudioStream: SmallInt = 0;
  ...
  var
    ...
    // Builder2: ICaptureGraphBuilder2;
    // MT: TGUID;
    // PinInfo: TPinInfo;
  begin
    if GraphBuilder = nil then
    ...
    VideoRenderer := nil;
    VideoRenderer := FindFilter(CLSID_VideoRenderer);
    if VideoRenderer = nil then
      VideoRenderer := FindFilter(CLSID_VideoMixingRenderer);
    if VideoRenderer = nil then
      VideoRenderer := FindFilter(CLSID_VideoMixingRenderer9);
    // added by Kameleon
    ...
    begin
      ...
      if frmMain.mnuVMR9VidRend.Checked then // added by adenry
        CoCreateInstance(TGUID(CLSID_VideoMixingRenderer9), nil,
          CLSCTX_INPROC_SERVER, TGUID(IID_IBaseFilter), VideoRenderer)
      else // added by adenry
        CoCreateInstance(TGUID(CLSID_VideoMixingRenderer), nil,
          CLSCTX_INPROC_SERVER, TGUID(IID_IBaseFilter), VideoRenderer);

      if Assigned(VideoRenderer) then
      begin
        GraphBuilder.AddFilter(VideoRenderer,
          PWideChar(WideString('Video Renderer')));
        VideoRenderer.EnumPins(EnumPins);
        EnumPins.Next(1, Pin, @Fetched);
        EnumPins := nil;

        // if assigned(Outpin) then Graphbuilder.Render(OutPin); // Connect renderer
        // should give the same effect as the connct below
      ...
      { // Kameleon tryout
        ...
        GraphBuilder.AddSourceFilter(PChar(String(FileName)), 'Source1', Source);

        CoCreateInstance(CLSID_CaptureGraphBuilder2, nil,
        CLSCTX_INPROC_SERVER, TGUID(IID_ICaptureGraphBuilder2), Builder2 );

        if assigned(Builder2) then
        begin
        Builder2.SetFilterGraph(GraphBuilder);
        Builder2.RenderStream(nil, nil, Source, nil, VideoRenderer);
        MT := MEDIATYPE_Audio;
        Builder2.RenderStream(nil, @MT, Source, nil, nil);
        end;
        end;
      }
...
begin
  ...
  VideoFileSize := frmVariousInfo.GetFileSize(FileName); // Kameleon
  ...
  begin
    ...
    if FAILED(CoCreateInstance(TGUID(CLSID_FilterGraph), nil,
      CLSCTX_INPROC_SERVER, TGUID(IID_IGraphBuilder), GraphBuilder)) then
      exit;
    ...
    // File to play    //
    // ------------------ //
    // Fl := TFileStream.Create('GraphLogfile.txt', fmCreate);
    // Graphbuilder.SetLogFile(Fl.Handle);

{$IFDEF LOGFILES}
    if FileExists('GraphLogfile.txt') then
      DeleteFile('GraphLogfile.txt');
    Fl := TFileStream.Create('GraphLogfile.txt', fmCreate or fmShareCompat);
    GraphBuilder.SetLogFile(Fl.Handle);
{$ENDIF}
    RenderResult := GraphBuilder.RenderFile(PChar(string(FileName)), nil);

{$IFDEF EXPORTGRAPH}
    Rr := AddGraphToRot(GraphBuilder, ROT);
{$ENDIF}
{$IFDEF LOGFILES}
    GraphBuilder.SetLogFile(0);
    Fl.Free;
{$ENDIF}
    ...
    begin
      if (RenderResult <> VFW_S_AUDIO_NOT_RENDERED) and
        ...
        exit
      else if (RenderResult <> VFW_S_PARTIAL_RENDER) and
        (RenderResult <> VFW_S_DUPLICATE_NAME) then // added by adenry
        MsgBox(Format(InfoMsg[10], ['$' + IntToHex(RenderResult, 8)]), BTN_OK,
          '', '', MB_ICONWARNING, frmMain);
    end;
    ...
    // -------------------------- //
    if FAILED(GraphBuilder.QueryInterface(IID_IMediaControl, MediaControl)) then
      exit;
    if FAILED(GraphBuilder.QueryInterface(IID_IMediaSeeking, MediaSeeking)) then
      exit;
    if FAILED(GraphBuilder.QueryInterface(IID_IMediaEventEx, MediaEventEx)) then
      exit;
    if FAILED(GraphBuilder.QueryInterface(IID_IBasicAudio, BasicAudio)) then
      exit;
    if FAILED(GraphBuilder.QueryInterface(IID_IBasicVideo, BasicVideo)) then
      exit;
    if FAILED(GraphBuilder.QueryInterface(IID_IVideoWindow, VideoWindow)) then
      exit;
    ...
    VideoWindow.put_MessageDrain(frmMain.pnlVideoDisplay.Handle);
    // Set message drain to parent window
    VideoWindow.put_WindowStyle(WS_CHILD or WS_CLIPCHILDREN or WS_CLIPSIBLINGS);
    // Set window style
    ...
    frmMain.VolumeSet(frmMain.sbVolume.Position);
    // added by adenry

    // --------------- //
    // Playback rate  //
    // --------------- //
    if frmMain.mnu10P.Checked then
      SetPlaybackRate(0.10, True)
    else if frmMain.mnu20P.Checked then
      SetPlaybackRate(0.20, True)
    else if frmMain.mnu30P.Checked then
      SetPlaybackRate(0.30, True)
    else if frmMain.mnu40P.Checked then
      SetPlaybackRate(0.40, True)
    else if frmMain.mnu50P.Checked then
      SetPlaybackRate(0.50, True)
    else if frmMain.mnu60P.Checked then
      SetPlaybackRate(0.60, True)
    else if frmMain.mnu70P.Checked then
      SetPlaybackRate(0.70, True)
    else if frmMain.mnu80P.Checked then
      SetPlaybackRate(0.80, True)
    else if frmMain.mnu90P.Checked then
      SetPlaybackRate(0.90, True)
    else if frmMain.mnu200P.Checked then
      SetPlaybackRate(2, True)
    else if frmMain.mnu300P.Checked then
      SetPlaybackRate(3, True)
    else if frmMain.mnu400P.Checked then
...
begin
  ...
  begin
    ...
    begin
{$IFDEF EXPORTGRAPH}
      RemoveGraphFromRot(ROT); // Kameleon
{$ENDIF}
      GraphBuilder := nil;
    end;
    ...
    begin
      ...
      MovieFile := '';
      EnableVPCtrls(False);
      //frmMain.mnuAudioStream.Clear; // added by adenry, removed by Kameleon
      //frmMain.mnuAudioStreamPopup.Clear; // added by adenry, removed by Kameleon
      Playing := False;
      btnPause.Hide;
      btnPlay.Show;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if Rate = 1 then
          btnAlterPlaybackRate.Down := False
        else
...
begin
  ...
  begin
    ...
    case DefAltPlayRate of
      1:
        mnu10P.ShortCut := DefAltPlayRateShortcut;
      2:
        mnu20P.ShortCut := DefAltPlayRateShortcut;
      3:
        mnu30P.ShortCut := DefAltPlayRateShortcut;
      4:
        mnu40P.ShortCut := DefAltPlayRateShortcut;
      5:
        mnu50P.ShortCut := DefAltPlayRateShortcut;
      6:
        mnu60P.ShortCut := DefAltPlayRateShortcut;
      7:
        mnu70P.ShortCut := DefAltPlayRateShortcut;
      8:
        mnu80P.ShortCut := DefAltPlayRateShortcut;
      9:
        mnu90P.ShortCut := DefAltPlayRateShortcut;
      10:
        mnu150P.ShortCut := DefAltPlayRateShortcut;
      11:
        mnu200P.ShortCut := DefAltPlayRateShortcut;
      12:
        mnu300P.ShortCut := DefAltPlayRateShortcut;
      13:
        mnu400P.ShortCut := DefAltPlayRateShortcut;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Player.MediaSeeking.SetPositions(vPos, AM_SEEKING_ABSOLUTEPOSITIONING,
        vPos, AM_SEEKING_NOPOSITIONING);
      // SET VIDEO POSITION
      ...
      begin
        ...
        begin
          ...
          if MarkingInVideoPreview then
            if Data.Marked then
              subSubtitle.Font.Color := MarkingInVideoPreviewColor
            else
              subSubtitle.Font.Color := subSubtitle.TextColor;
          // subSubtitle.Font.Color stores the color for marking
          // added by adenry: end
          ...
          begin
            ...
            if AlignLeftLineWithDash and
              ((Pos(#13#10 + '-', RemoveSWTags(SubtitleText, True, True, True,
              True, True)) > 0) or ((OrgCharset in DashCharsets1) and
              (Pos(#13#10 + #150, RemoveSWTags(SubtitleText, True, True, True,
              True, True)) > 0)) or ((OrgCharset in DashCharsets1) and
              (Pos(#13#10 + #151, RemoveSWTags(SubtitleText, True, True, True,
              True, True)) > 0)) or ((OrgCharset in DashCharsets2) and
              (Pos(#13#10 + #173, RemoveSWTags(SubtitleText, True, True, True,
              True, True)) > 0))) then
              subSubtitle.Alignment := taLeftJustify
            else
    ...
    if (Player.Initialized) and (mnuVideoPreviewMode.Checked = True) then
      ...
      begin
        ...
        begin
          ...
          else if (CurrTime >= NewShowTime) and (CurrTime <= NewHideTime) then
          begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Data := lstSubtitles.GetNodeData(Node);
          if (CurrTime >= Data.InitialTime) and (CurrTime <= Data.FinalTime)
            then
          begin
            // we found the subtitle!
            subSubtitle.SubtitleNum := Node.Index + 1; // added by adenry
            if mnuTranslatorMode.Checked and mnuDisplayTranslation.Checked then
              SubtitleText := Data.Translation
            else
          ...
          else if (CurrTime < Data.FinalTime) // we are between subtitles
            or (CurrTime > LastNodeData.FinalTime) then
          // we are after the last subtitle
          begin
            subSubtitle.Visible := False;
            subSubtitle.Text := '';
            subSubtitle.SubtitleNum := 0;
  ...
  with frmMain do
    ...
    begin
      ...
      begin
        ...
        // if (Assigned(Player.BasicVideo)) and KeepSubOnVideo then
        begin
          ...
          begin
            ...
            if FullScreen and pnlVideoControls.Visible then
              bottom := H +
                ((pnlVideoDisplay.Height + pnlVideoControls.Height - H) div 2);
          end;
        end;
        // Set font size
        ...
        begin
          ...
          if (Player.Initialized) and (Player.VideoAvail) then
          // if Assigned(Player.BasicVideo) then
          begin
            Player.VideoWindow.get_Width(W);
            newFontSize := Round(W / (FONT_SIZE_TO_WIDTH / SubFontSize));
            if newFontSize > MAX_FONT_SIZE then
              newFontSize := MAX_FONT_SIZE
            else if newFontSize < MIN_FONT_SIZE then
              newFontSize := MIN_FONT_SIZE;
            // subSubtitle.Font.Size := newFontSize;
          end;
          subSubtitle.Font.Size := newFontSize;
          newBorderWidth :=
            Round(SubBorderWidth * (newFontSize / FONT_SIZE_TO_BORDER));
          if newBorderWidth < 1 then
            newBorderWidth := 1;
          subSubtitle.BorderWidth := newBorderWidth;
          newShadowWidth :=
            Round(SubShadowWidth * (newFontSize / FONT_SIZE_TO_SHADOW));
          if newShadowWidth < 1 then
            newShadowWidth := 1;
        ...
        if (Player.Initialized) and (Player.VideoAvail) then
          ...
          if subSubtitleTopShift <> 0.0 then
            if KeepSubOnVideo then
              newTop := bottom - H + Round(subSubtitleTopShift * H) -
                subSubtitle.Height
            else
              newTop := bottom - Round(subSubtitleTopShift * H) -
                subSubtitle.Height;
        // Set Top and Left
        subSubtitle.Top := newTop;
        subSubtitle.Left := (pnlVideoDisplay.Width div 2) -
          (subSubtitle.Width div 2);
        // added by adenry: end
...
begin
  if not frmMain.TogglingFullScreen then // added by adenry
    if (Player.Initialized) and (Player.VideoAvail) and
      (frmMain.mnuVideoPreviewMode.Checked) then
    begin
      if (Player.VideoWidth > 0) and (Player.VideoHeight > 0) then
      begin
        if VideoAspectRatio <> -1.0 then
          // added by adenry
          Proportion := VideoAspectRatio
        else // added by adenry
          Proportion := Player.VideoWidth / Player.VideoHeight;
        if (frmMain.pnlVideoDisplay.Width / frmMain.pnlVideoDisplay.Height) > Proportion
          then
        begin
        ...
        begin
          ...
          if frmMain.VertVideoAlign or frmMain.FullScreen then
            newTop := (frmMain.pnlVideoDisplay.Height - NewHeight) div 2
          else
            newTop := 0;
          // added by adenry: end
        end;
        ...
        Player.VideoWindow.SetWindowPosition(NewLeft, newTop,
          (NewWidth - NewLeft), (NewHeight));
        // 0 replaced with NewTop by adenry
...
begin
  ...
  begin
    if frmMain.InPausedVideoPreviewMode then // added later by adenry
      SetVideoPos(frmMain.sbSeekBar.Position);
    // added by adenry - for when the video is played through a key shortcut while the mouse is still over the seekbar
...
begin
  ...
  begin
    ...
    begin
      ...
      frmMain.sbSeekBar.Position := GetCurrentPos;
      // added by adenry - bug fix - set exact (still rounded) position when the video is paused
      SetTimeCounter(frmMain.sbSeekBar.Position);
      // added by adenry - bug fix - show exact (still rounded) current time when the video is paused
    end
    else
      Play;
  end;
...
begin
  ...
  begin
    ...
    if Playing = True then
      Player.MediaControl.Pause;
    // added by adenry - Stop replaced with Pause so that MouseOver video preview could continue to function
    Playing := False;
    SetVideoPos(0);
...
begin
  with frmMain do
  begin
    if pnlControl.Visible and (Width < Constraints.MinWidth + pnlControl.Width)
      then
    begin
