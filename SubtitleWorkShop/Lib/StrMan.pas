...
uses Windows, System.SysUtils, {System.}Character , FastStrings,
     Variants,
     Classes;
...
type
  ...
  T2GBCharArray  = array[1..MaxInt div 2] of Char;
...
TStringManager = class //Global instance "sm" in initialization section
  ...
  //Memory related--------------------------------------------------------------
  function  StrHeader(const s : String) : PAnsiStrHeader;
  function  RefCount(const s : String) : Integer;
  function  NTLen(const s : String) : Integer; overload;
  function  NTLen(const pc : PChar) : Integer; overload;
  function  EnsureNTLen(var s : String) : Integer;
...
type
  ...
  public
    procedure Init(var AnsiStr : String); //Set's AnsiStr to ''
    procedure Truncate;
...
begin
  Result := TCharacter.ToUpper(C); // Kameleon
(*
...
//var i : Integer;
begin
  S := Uppercase(S); // Kameleon
(*
  for i := 1 to Length(s) do s[i] := char(gaANSIUpperArray[Char(s[i])]);
*)
end;
...
begin
  C := TCharacter.ToUpper(c); // Kameleon
(*
...
begin
  S := uppercase(S); // Kameleon
(*
  for i := 1 to Length(s) do s[i] := Char(gaANSILowerArray[Char(s[i])]);
*)
end;
...
begin
  C :=  TCharacter.ToLower(C); // Kameleon
...
begin
  Result := True;
  for i := 1 to Length(s) do if Flags * gaANSICharType[Char(s[i])] <> [] then Exit;
  Result := False;
end;
...
begin
  ...
  Result := system.Pos(SubStr, S, StartPos); // Kameleon
(*
  Result := 0;
...
begin
  TmpSubStr := uppercase(SubStr);  // Kameleon
  TmpStr := uppercase(S);
  Result := pos(TmpSubStr, TmpStr, StartPos);
...
begin
 if Substr = '' then begin
  ...
  For I := StartPos downto 1 do  // Kameleon
  begin
    if pos(SubStr, S, Startpos) > 0
    then
    begin
      Result := I;
      break;
...
begin
  ...
  //PUpCase(c);
  C := TCharacter.ToUpper(C); // Kameleon
  for i := StartPos to Length(TmpS) do
  begin
    if c = TmpS[i] then
    begin
      Result := i;
...
begin
  ...
  for I := StartPos to Length(S) - length(SubStr) + 1 do  // Kameleon
  begin
    p := pos(SubStr, S, I);
    if P > 0 then
    begin
      if ((P = 1) or (S[P-1] in Delims)) and
         ((P + length(SubStr) > length(S)) or (S[P + length(Substr)] in delims)) then
      begin
        Result := p;
        exit;
      end;
...
(*
  ...
  for i := StartPos to SLen do
  begin
    if Char(s[i]) = c1 then if (i=1) or (s[i-1] in Delims) then
    begin
      if SubLen+i>SLen+1 then Exit;
      si := i;
      j := 2;
...
begin
  ...
  for I := StartPos to Length(S) - length(TmpSubStr) + 1 do
  begin
    p := pos(TmpSubStr, TmpStr, I);
    if P > 0 then
    begin
      if ((P = 1) or (TmpStr[P-1] in Delims)) and
         ((P + length(TmpSubStr) > length(TmpStr)) or (TmpStr[P + length(TmpSubstr)] in delims)) then
      begin
        Result := p;
        exit;
      end;
...
(*
  ...
  c1 := gaANSIUpperArray[Char(SubStr[1])];
  for i := StartPos to SLen do
  begin
    if gaANSIUpperArray[Char(s[i])] = c1 then if (i=1) or (s[i-1] in Delims) then
    begin
      if SubLen+i>SLen+1 then Exit;
      si := i;
      j := 2;
      ...
      begin
        Inc(si);
        if gaANSIUpperArray[Char(s[si])] <> gaANSIUpperArray[Char(SubStr[j])] then Break;
        Inc(j);
      end;
      if j=SubLen+1 then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      for i := 1 to More do Result[i] := Char(c);
    end else
    begin
      Result := s;
      SetLength(Result, Abs(PadLen));
      for i := SLen+1 to Slen+More do Result[i] := Char(c);
    end;
  end else Result := s;
end;
...
begin
  ...
  begin
    Result := s;
    SetLength(Result, Abs(PadLen));
    if More>0 then for i := SLen+1 to Slen+More do Result[i] := Char(c);
  end;
end;
...
begin
  ...
  try
    ...
    until Value=0;
    if Digits<>0 then Result := PadCut(Result, -Digits, Char(Key[1]));
  except
    {$IFDEF StrManEnableERaise}
...
begin
  Result := #0;
  if s<>'' then Result := Char(s[Length(s)]);
end;
...
begin
  ...
  while (aStop>0) and (Char(s[aStop]) = c) do System.Dec(aStop);
  ...
  aStart := 1;
  while (aStart<=aStop) and (Char(s[aStart]) = c) do Inc(aStart);
  ...
  begin
    if (Char(s[i]) = c) then
    begin
      while (i+1<=aStop) and (Char(s[i+1]) = c) do Inc(i);
    end;
    System.Inc(RLen);
    Result[RLen] := s[i];
    Inc(i);
  ...
  if DoUpCase then for i := 1 to RLen do Result[i] := Char(gaANSIUpperArray[Char(Result[i])]);
end;
...
begin
  if aFormat=''
    then {SysUtils.}DateTimeToString(Result, System.SysUtils.FormatSettings.ShortDateFormat, Value)
...
begin
  if aFormat=''
    then Result := System.SysUtils.DateTimeToStr(Value)
    else System.SysUtils.DateTimeToString(Result, aFormat, Value);
end;
...
begin
  Result := System.Variants.VarToDateTime(s);
end;
...
function TStringManager.PReplace(
           ...
           const Delims     : TCharSet = gcWordDelims)
                            : Integer;
var TmpFindStr, TmpInStr: string;
    //RFlags: TReplaceFlags;
    P, Tmp: integer;

    procedure DoReplace;
    var I: integer;
        TmpReplaceStr, FoundStr: string;
    begin
      TmpReplaceStr := ReplaceStr;
      if roPreserveCase in Flags then
      begin
        FoundStr := copy(Instr, P, length(FindStr));
        for I := 1 to length(FoundStr) do
        begin
          if I <= length(ReplaceStr) then
          begin
            if  TCharacter.IsUpper(FoundStr[I]) then TmpReplaceStr[I] := TCharacter.ToUpper(TmpReplaceStr[I]);
            if  TCharacter.IsLower(FoundStr[I]) then TmpReplaceStr[I] := TCharacter.ToLower(TmpReplaceStr[I]);
          end;
        end;
      end;

      if (roWholeWords in Flags) and (roRemoveDelim in Flags)
      then
      begin // delete delimiters
        Tmp := P + length(FindStr);
        if (Tmp <= length(Instr)) and (InStr[Tmp] in Delims) then delete(instr, Tmp, 1);
        Delete(InStr, p, length(FindStr));
        if (P > 1) and (Instr[P-1] in Delims) then delete(Instr, P-1, 1);
      end
      else Delete(InStr, p, length(FindStr)); // do not delete delimiters

      Insert(TmpReplaceStr, InStr, P);

      if ReplaceStr >=  ''
      then Startpos := P + length(ReplaceStr)
      else Startpos := P + length(FindStr);

      if Startpos > length(InStr) then StartPos := 0;
...
begin
  ...
  begin
    TmpFindStr := uppercase(TmpFindStr);
    TmpInStr := Uppercase(TmpInStr);
  end;

  P := pos(TmpFindStr, TmpInStr, Startpos);
  while (StartPos > 0) and (P > 0) do
  begin
    if roWholeWords in Flags then
    begin
      if ((P = 1) or (TmpInStr[P-1] in Delims)) and
       ((P + length(TmpFindStr) > length(TmpInStr)) or (TmpInStr[P + length(TmpFindstr)] in delims))
       then DoReplace
       else
       begin
         Startpos := P + length(FindStr); // skip the word found
         if Startpos > length(InStr) then StartPos := 0;
       end;
    end else // not wholewords
      DoReplace;

    // get the new value of InStr
    TmpInstr := Instr;
    if roIgnoreCase in Flags then TmpInStr := Uppercase(TmpInStr);

    // check if done
    if roReplaceOnce in Flags
    then StartPos := 0 // do not repeat
    else P := pos(TmpFindStr, TmpInStr, Startpos); // search next
  end;
...
begin
  System.SysUtils.StringReplace(S, OldPattern, NewPattern,Flags);
{
  StartPos := 1;
  Result := s;
  lFlags := [];
...
begin
  ...
  begin
    Insert(System.SysUtils.FormatSettings.ThousandSeparator, Result, Length(Result) - (i + p)+ 1);
    Inc(p);
    Inc(i, 3);
  end;
...
begin
  Result := System.SysUtils.StrNew(PChar(s));
end;
...
begin
  try
    Result := System.SysUtils.Format(aFormat, Args);
  except
...
{-------------------------------------------------------------------------------
*StrHeader - returns the negative offset String control header.
Explanation:
  r = RefCount int
...
-------------------------------------------------------------------------------}
function TStringManager.StrHeader(const s : String) : PAnsiStrHeader;
begin
  Result := Pointer(s);
...
{-------------------------------------------------------------------------------
*RefCount - returns the reference count of an String.
...
Note that a Delphi String as a special length integer at a negative offset.
If a #0 character is manually inserted into an String the Length function
...
-------------------------------------------------------------------------------}
function TStringManager.NTLen(const s : String) : Integer;
begin
...
-------------------------------------------------------------------------------}
function TStringManager.EnsureNTLen(var s : String) : Integer;
begin
  Result := NTLen(s);
...
const
  C1 = 52845;
  C2 = 22719;
type aTextArray = array[1..MaxInt div 2] of Char;
...
-------------------------------------------------------------------------------}
procedure TStrBufPtr.Init(var AnsiStr : String);
begin
  AnsiStr := '';
  Fp := @AnsiStr;
  FCharLen := 0;
...
var   c : {Ansi}Char;
      ...
      f : TCharTypeFlags;
begin
  for c := Low({Ansi}Char) to High({Ansi}Char) do gaANSIUpperArray[c] :=  TCharacter.toUpper(char(c));
  for c := Low({Ansi}Char) to High({Ansi}Char) do gaANSILowerArray[c] :=  TCharacter.ToLower(char(c));
  ...
  for c := Low({Ansi}Char) to High({Ansi}Char) do
  begin
    f := [];
    {
    ...
    if TCharacter.IsUpper(C) then include(f, cfUpper);
    if TCharacter.IsLower(C) then include(f, cfLower);

    if c in ['A'..'Z']         then include(f, cfAZUpper);
    if c in ['a'..'z']         then include(f, cfAZLower);

    //if w and C1_PUNCT <> 0     then include(f, cfPunct);
    if IsPunctuation(C) then include(f, cfPunct);
...
initialization
  gcDefaultCurFormat := System.SysUtils.FormatSettings.CurrencyString+'###,##0.00';
  FillANSIArrays;
