...
//function FastCharPos(const aSource : string; const Ch: Char; StartPos : Integer): Integer;
//function FastCharPosNoCase(const aSource : string; Ch: Char; StartPos : Integer): Integer;
...
begin
  ...
  asm
      ...
      sub  ECX, StartPos
      mov  AL,  Ch              //and which char we want
    @Loop:
...
begin
  ...
  Result := System.Pos(aFindString, aSourceString, StartPos);
...
begin
  ...
  aSrc := Uppercase(ASourceString);
  aFnd := UpperCase(AFindString);
  Result := System.Pos(aFnd, aSrc, StartPos);
...
begin
  ...
  begin
    if pos(aFindString, aSourceString, Startpos) > 0
    then
    begin
      Result := I;
      break;
...
begin
  ...
  aSrc := Uppercase(ASourceString);
  aFnd := UpperCase(AFindString);
  Result := FastPosBack(aFnd, aSrc, aSourceLen, aFindLen, StartPos);
...
begin
  if CaseSensitive then Flags := [rfIgnoreCase];
  Flags := Flags + [rfReplaceAll];
  Result := StringReplace(aSourceString, aFindString, aReplaceString, Flags);
{
