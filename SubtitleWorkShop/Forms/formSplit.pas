...
uses
  ...
  FastStrings,
  _CommonTypes, System.UITypes, Rcs_Resize;
...
type
  ...
  TfrmSplit = class(TForm)
    ...
    edtNumberOfParts: TEdit;
    lblNumberOfParts: TLabel;
    ...
    procedure udNumberOfPartsChangingEx(Sender: TObject; var AllowChange: Boolean; NewValue: Integer;
      Direction: TUpDownDirection);
...
{$ENDIF}
    : integer): boolean;
var Ttme: TTimeMaskEdit;
begin
  Result := false;

  if Control is TTimeMaskEdit then
  begin
  ...
  if Control.Parent is TTimeMaskEdit then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      lblNumberOfParts.Caption             := ReadString('Split','17','Number of parts:');
...
procedure TfrmSplit.FormClose(Sender: TObject; var Action: TCloseAction);
var Ini: TIniFile;
begin
  Ini := TIniFile.Create(IniRoot); // Kameleon
  try
    if WindowState = wsMaximized then
    begin
      Ini.WriteBool('Split', 'Maximized', True);
    end
    else
    begin
    Ini.WriteInteger('Split', 'Left', Left);
    Ini.WriteInteger('Split', 'Top', Top);
    Ini.WriteInteger('Split', 'Height', Height);
    Ini.WriteInteger('Split', 'Width', Width);
    Ini.WriteBool('Split', 'Maximized', False);
    end;
...
begin
  ...
  MyResize_ := TMyResize.Create(Self, ResizeCallbackFunction); // Kameleon

  Ini := TInifile.Create(IniRoot);
    lstSplitParts.Font.Size := Ini.ReadInteger('List look', 'List Font', 11); // Kameleon
    lstSplitParts.DefaultNodeHeight := Ini.ReadInteger('List look', 'Row height', 18); // Kameleon
    lstSplitParts.Header.Font.Size  := lstSplitParts.Font.Size; // Kameleon
    lstSplitParts.Header.Height := lstSplitParts.DefaultNodeHeight; // Kameleon
  ini.Free;
...
procedure TfrmSplit.FormResize(Sender: TObject);
begin
  MyReSize_.Resize(Self);
  MyReSize_.Resize(Self);
end;

procedure TfrmSplit.FormShow(Sender: TObject);
var Ini: TIniFile;
    L, T,W, H: integer;
begin
  Ini := TIniFile.Create(IniRoot); // Kameleon
  try
    L := Ini.ReadInteger('Split', 'Left', 206);
    T := Ini.ReadInteger('Split', 'Top', 113);
    H := Ini.ReadInteger('Split', 'Height', 762);
    W := Ini.ReadInteger('Split', 'Width', 642);

    Font.Size := frmMain.Font.Size;
    SetBounds(L, T, W, H);

  if Ini.ReadBool('Split', 'Maximized', False) then
...
procedure TfrmSplit.udNumberOfPartsChangingEx(Sender: TObject; var AllowChange: Boolean; NewValue: Integer;
  Direction: TUpDownDirection);
(*
