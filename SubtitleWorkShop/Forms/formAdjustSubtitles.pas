...
begin
  ...
  Ini := TInifile.Create(IniRoot);
    lstPoints.Font.Size := Ini.ReadInteger('List Look', 'List Font', 11); // Kameleon
    lstPoints.DefaultNodeHeight := Ini.ReadInteger('List Look', 'Row height', 18); // Kameleon
    lstPoints.Header.Font.Size  := lstPoints.Font.Size; // Kameleon
    LstPoints.Header.Height := lstPoints.DefaultNodeHeight; // Kameleon
  ini.Free;
...
begin
  ...
  try
    ...
    begin
      Ini.WriteBool('Adjust Subtitles', 'Maximized', True);
    end
    else
    begin
    Ini.WriteInteger('Adjust Subtitles', 'Left', Left);
    Ini.WriteInteger('Adjust Subtitles', 'Top', Top);
    Ini.WriteInteger('Adjust Subtitles', 'Height', Height);
    Ini.WriteInteger('Adjust Subtitles', 'Width', Width);
    Ini.WriteBool('Adjust Subtitles', 'Maximized', False);
    end;
...
begin
  ...
  try
   ...
   MyResize_.Free; // Kameleon
end;

procedure TfrmAdjustSubtitles.FormResize(Sender: TObject);
begin
  MyReSize_.Resize(Self);
  MyReSize_.Resize(Self);
end;

procedure TfrmAdjustSubtitles.FormShow(Sender: TObject);
var Ini: TIniFile;
    L, T,W, H: integer;
begin
  Ini := TIniFile.Create(IniRoot); // Kameleon
  try
    L := Ini.ReadInteger('Adjust Subtitles', 'Left', 543);
    T := Ini.ReadInteger('Adjust Subtitles', 'Top', 138);
    H := Ini.ReadInteger('Adjust Subtitles', 'Height', 519);
    W := Ini.ReadInteger('Adjust Subtitles', 'Width', 489);

    Font.Size := frmMain.Font.Size;
    SetBounds(L, T, W, H);

  if Ini.ReadBool('Adjust Subtitles', 'Maximized', False) then
