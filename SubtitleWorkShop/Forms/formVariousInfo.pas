...
type
  ...
  TfrmVariousInfo = class(TForm)
    ...
    procedure SetFont;
    procedure pgeExtrasContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean); //added by adenry
  private
    procedure SetLanguage;
  public
    { Public declarations }
    function GetFileSize(const FileName: String): Double; // in Kb Kameleon
  end;
...
begin
  ...
  try
    Width   := Ini.ReadInteger('Various info', 'Width', 904); // Kameleon
    Height  := Ini.ReadInteger('Various info', 'Height', 504); // Kameleon
    ...
    lstInfo.Font.Size := Ini.ReadInteger('List look', 'List Font', 11); // <------- Kameleon
    lstInfo.DefaultNodeHeight := Ini.ReadInteger('List look', 'Row height', 18); // Kameleon
  finally
    Ini.Free;
  end;
...
procedure TfrmVariousInfo.pgeExtrasContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
begin
...
var
  ...
  Node: PVirtualNode;
begin
  if lstInfo.RootNodeCount = 0 then exit; // Kameleon
...
(*
function TfrmVariousInfo.GetFileSize(const FileName: String): Double; // Kameleon
    { // deleted by Kameleon
    function Redondear(Valor: Real; Redondeo: Integer): Real;
    begin
      Redondear := Round(Valor * Power(10, Redondeo)) / Power(10, Redondeo);
  ...
  begin
    ...
    begin
      ...
      if IOResult = 0 then
      begin
        Result := FileSize(f);  // result in Kb
        CloseFile(f);
      end;
    end;
  end;
...
function GetFileSizeEx(hFile: THandle; var FileSize: int64): BOOL; // Kameleon
...
begin
  ...
  try
    ...
    Result := Size / 1024.0; // Result is in kB
  end;
end;
...
begin
  ...
  try
    ...
    // Kameleon: read again these values
    lstInfo.Font.Size := Ini.ReadInteger('List look', 'List Font', 11); // <------- Kameleon
    lstInfo.DefaultNodeHeight := Ini.ReadInteger('List look', 'Row height', 18); // Kameleon
    lstInfo.Header.Columns[0].Width := lstInfo.Width div 3; // added by Kameleon
  finally
    Ini.Free;
  end;
...
begin
  ...
  pgeCtrl.Width       := ClientWidth - 20; // Kameleon
  pgeCtrl.Height      := btnRefreshInfo.Top - 16;

  pgeInformation.Top := 5;  // Kameleon
  pgeInformation.Height := PgeCtrl.ClientHeight - 10;  // Kameleon
  pgeInformation.Width  := PgeCtrl.ClientWidth - 10;   // Kameleon

  pgeExtras.Top := 5;
  pgeExtras.Height := pgeCtrl.ClientHeight - 10; // Kameleon
  pgeExtras.Width  := pgeCtrl.ClientWidth - 10;  // Kameleon

  lstInfo.Header.Columns[0].Width := lstInfo.Width div 3; // added by Kameleon

  LstInfo.Left        := 10;  // Kameleon
  LstInfo.Top         := 5;  // Kameleon
  LstInfo.Height      := pgeInformation.ClientHeight - 10; // Kameleon
  LstInfo.Width       := pgeInformation.ClientWidth - 20;  // Kameleon
end;
//added by adenry: end
