...
uses
  ...
  FastStrings, System.ImageList, USubTitleAPI;
...
begin
  ...
  Width:=Ini.ReadInteger('Save', 'Width', 510); // Kameleon
  Height:=Ini.ReadInteger('Save', 'Height', 371); // Kameleon
  //added by adenry: end
  try
...
begin
  if ChkUTF8.Checked
  then SubTitleAPI.Set_UTF8_File(longbool(true))
  else SubTitleAPI.Set_UTF8_File(longbool(false));
...
begin
  ...
  begin
    if LstFormats.Canvas.TextWidth(LstFormats.Items[I].Caption) > MaxW
    then MaxW := LstFormats.Canvas.TextWidth(LstFormats.Items[I].Caption);
  end;

  if MaxW = 0 then MaxW := Screen.Width div 8;
  LstFormats.LargeImages.Width := MaxW + 30; // added by Kameleon
