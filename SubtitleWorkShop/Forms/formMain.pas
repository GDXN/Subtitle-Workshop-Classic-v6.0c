...
uses
  ...
  StdCtrls, ExtCtrls, Menus, ComCtrls, IniFiles, ImgList,
  ShellAPI, ClipBrd, URLMon, FileCtrl, Grids, ToolWin, Mask, Math, {XPMan,}
  RichEdit, SymbolDlg, // Math, RichEdit, XPMan added by adenry
  VirtualTrees, MiMenu, MiSubtitulo, {MiHint,} {NFormSizing,} SWSeekBar,
  SWButton, USSpeller, SWTimeCounter, TimeMaskEdit,
  // MiHint, NFormSizing removed by adenry
  uPSComponent,
  uPSCompiler, upSRuntime,
  uPSC_controls, uPSC_std, uPSC_classes, uPSC_graphics, uPSC_forms,
  uPSC_stdctrls,
  uPSC_extctrls, uPSC_menus, uPSC_dateutils,
  uPSR_controls, uPSR_std, uPSR_classes, uPSR_graphics, uPSR_forms,
  uPSR_stdctrls,
  uPSR_extctrls, uPSR_menus, uPSR_dateutils,

  StrMan, FastStrings, WinShell,
  // DirectShow9, //WinShell added by adenry, DirectShow9 removed by adenry
  _CommonTypes, System.ImageList, System.Types, System.UITypes,
  uPSComponent_StdCtrls, uPSComponent_Controls,
  uPSComponent_Forms, uPSComponent_DB, uPSComponent_COM, uPSComponent_Default;
...
type
  TfrmMain = class(TForm)
    ...
    N211: TMenuItem;
    N212: TMenuItem;
    procedure lstSubtitlesInitNode(Sender: TBaseVirtualTree;
    ...
    procedure lstSubtitlesFocusChanged(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Column: TColumnIndex);
    procedure lstSubtitlesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure lstSubtitlesPaintText(Sender: TBaseVirtualTree;
    ...
    procedure lstSubtitlesKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure lstSubtitlesMouseUp(Sender: TObject; Button: TMouseButton;
    ...
    procedure sbSeekBarMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure sbSeekBarMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure MarkNode(Node: PVirtualNode; BindUndoToNext: Boolean = False);
    // added by adenry
    procedure mnuMarkSelectedSubsClick(Sender: TObject);
    procedure UnmarkNode(Node: PVirtualNode; BindUndoToNext: Boolean = False);
    // added by adenry
    ...
    procedure ChangeShowTime(Node: PVirtualNode; NewShowTime: Cardinal;
      BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
      Shift: Boolean = False; UndoActionName: TUndoActionName = uanShowEdit);
    // added by adenry
    procedure tmeHideTimeChangeFromEditOnly(Sender: TObject; NewTime: Cardinal);
    procedure ChangeHideTime(Node: PVirtualNode; NewHideTime: Cardinal;
      BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
      Shift: Boolean = False; UndoActionName: TUndoActionName = uanHideEdit);
    // added by adenry
    procedure tmeDurationTimeChangeFromEditOnly(Sender: TObject;
      NewTime: Cardinal);
    procedure ChangeDuration(Node: PVirtualNode; NewDuration: Cardinal;
      BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
      Shift: Boolean = False); // added by adenry
    procedure lstSubtitlesCreateEditor(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Column: TColumnIndex; out EditLink: IVTEditLink);
    procedure lstSubtitlesEditing(Sender: TBaseVirtualTree; Node: PVirtualNode;
    ...
    procedure psCompExecCompImport(Sender: TObject; X: TPSPascalCompiler);
    procedure PSCompExecExecImport(Sender: TObject; se: TPSExec;
    ...
    procedure lstSubtitlesColumnResize(Sender: TVTHeader; Column: TColumnIndex);
    ...
    procedure PlayerShortcutsKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure pnlVideoDisplayMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure pnlVideoDisplayMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    procedure tmrMouseHideTimer(Sender: TObject);
    procedure sbVolumeMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure PlayerShortcutsMouseWheelDown(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    procedure btnMuteClick(Sender: TObject);
    procedure subSubtitleMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure subSubtitleMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure tmeHideMouseWheelUp(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    procedure tmeDurationMouseWheelDown(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    ...
    procedure tbLoadSubtitleMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    ...
    procedure tbVariousInfoMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tbInfoErrorsSettingsMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tbVideoPreviewModeMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure mnuTextStyleBarsClick(Sender: TObject);
    procedure tbBoldItalicUnderlineMouseUp(Sender: TObject;
    ...
    procedure tbSetColorMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tbBoldItalicUnderlineTransMouseUp(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure mmoTransKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure tbSetColorTransMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tbRemoveAllTagsTransMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    ...
    procedure tmePauseTimeChangeFromEditOnly(Sender: TObject;
      NewTime: Cardinal);
    procedure ChangePause(Node: PVirtualNode; NewPause: Cardinal;
      BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False);
    // added by adenry
    ...
    procedure tmePauseMouseWheelUp(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    procedure tbTimingsTextsSubtitlesMouseDown(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    ...
    procedure tcTimeCounterMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tcTimeCounterMouseWheelDown(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    procedure tcTimeCounterMouseWheelUp(Sender: TObject; Shift: TShiftState;
      MousePos: TPoint; var Handled: Boolean);
    ...
    procedure tbtnTimeControlButtonMouseDown(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    ...
    procedure tbtnDurationSetMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure tbtnShowSnapMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure lstSubtitlesChange(Sender: TBaseVirtualTree; Node: PVirtualNode);
    ...
    procedure lstSubtitlesHeaderDragged(Sender: TVTHeader; Column: TColumnIndex;
      OldPosition: Integer);
    procedure lstSubtitlesHeaderClick(Sender: TVTHeader;
      HitInfo: TVTHeaderHitInfo);
    procedure lstSubtitlesEdited(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex);
    procedure lstSubtitlesEditCancelled(Sender: TBaseVirtualTree;
      Column: TColumnIndex);
    procedure lstSubtitlesGetHint(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex; var LineBreakStyle: TVTTooltipLineBreakStyle;
      var HintText: string);
    procedure lstSubtitlesGetText(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
    procedure tbTimingsClick(Sender: TObject);
    procedure CompExecCompile(Sender: TPSScript);
    procedure OnScriptExecute(Sender: TPSScript);
    procedure mnuSettingsClick(Sender: TObject);
    procedure mnuAudioStreamPopupClick(Sender: TObject);
  protected
    UTF8_Orig, UTF8_Trans: Boolean;
  ...
  public
    ...
    SaveEmptyLines: Boolean;
    // Save empty lines with EMPTY_SUBTITLE text //added by adenry
    MarkUntransSubs: Boolean; // Mark untranslated subtitles
    MarkUntransSubsBckgr: Boolean;
    // Mark untranslated subtitles with background //added by adenry
    ...
    OnDoubleClick: Byte;
    // Action when double click in a subtitle and we are in video preview mode
    OnShiftDblClick: Byte;
    // Action when shift-double click in a subtitle and we are in video preview mode
    ...
    procedure PaintCellBackground(TargetCanvas: TCanvas; CellRect: TRect;
      Color: TColor); // added by adenry
    ...
    procedure AutoRecheckNodeErrors(Node: PVirtualNode;
      ErrorTypesToCheck: TErrorTypeSet = [];
      RecheckType: TRecheckTypes = rtNormal; FormOnly: Boolean = False);
    // added by adenry
    procedure AutoRecheckNodeErrorsAndRepaint(Node: PVirtualNode;
      ErrorTypesToCheck: TErrorTypeSet = [];
      RecheckType: TRecheckTypes = rtNormal; FormOnly: Boolean = False);
    // added by adenry later
    procedure AutoRecheckAllErrors(ErrorTypesToCheck: TErrorTypeSet = []);
    // added by adenry
    ...
    procedure RefreshTagButtonState(openTag, closeTag: String; tb: TToolButton;
      mmo: TSWTextEdit); // added by adenry //TMemo changed to TSWTextEdit
    procedure SetNodeTag(Node: PVirtualNode; tag, closeTag: String;
      Translation: Boolean; UndoName: TUndoActionName = uanTextEdit;
      BindUndoToNext: Boolean = False; ForceSet: Boolean = False);
    // added by adenry
    procedure SetBoldItalicUnderline(Sender: TObject; mmo: TSWTextEdit);
    // added by adenry //TMemo changed to TSWTextEdit
    procedure SetNodeColor(Node: PVirtualNode; Color: TColor;
      Translation: Boolean = False; RemoveOtherColors: Boolean = False;
      ToggleSetRemove: Boolean = False); // added by adenry
    procedure SetColor(TextColor: Boolean = False; TransColor: Boolean = False;
      NoDialog: Boolean = False; RemoveOtherColors: Boolean = False;
      ToggleSetRemove: Boolean = False); // added by adenry
    procedure RemoveNodeTags(Node: PVirtualNode; Translation: Boolean;
      BindUndoToNext: Boolean = False); // added by adenry
    procedure RemoveAllTags(mmo: TSWTextEdit);
    // added by adenry //TMemo changed to TSWTextEdit
    ...
    procedure SetColumnVisibility(Col: Byte; Visible: Boolean);
    // added by adenry
    procedure UpdateCaretPosition; // added by adenry
    function SetHideSelection(Control: TWinControl; HideSelection: Boolean)
      : Boolean; // added by adenry
    ...
    // by BDZL
    property TransModified: Boolean read GetTransModified
      write SetTransModified; // by BDZL
  end;
...
uses
  General, Functions, TreeViewHandle, USubtitlesFunctions, USubtitleAPI,
  VideoPreview, FileTypes, InfoErrorsFunctions, Undo, ShortCuts,
  VTInPlaceEdition, PascalScriptsFunctions, OCRScripts,
  formSettings, formSaveAs, formDurationLimits, formAdjustSubtitles,
  formSetDelay, formSearchAndReplace, formBatchConvert, formSplit,
  formConvertCase,
  formJoin, formTimeExpanderReducer, formAbout, formOutputSettings,
  formInfoErrors, formInfoErrorsSettings, formDivideLines,
  formAutomaticDurations,
  formSAMILangExtractor, formVariousInfo, formSetPauses, formUnbreakSubtitles,
  formSmartLineAdjust, formCombineSubtitles, formRoundTimeValues,
  System.IOUtils;
...
begin
  ...
  begin
    SetString(Arg, PChar(Msg.CopyDataStruct.lpData),
      (Msg.CopyDataStruct.cbData div SizeOf(Char)) - 1);
...
procedure TfrmMain.lstSubtitlesInitNode(Sender: TBaseVirtualTree;
  ...
  begin
    case Column of
      ...
      0:
        begin
          ...
          if ColNumDisplayStyle = 0 then
            while Length(CellText) <
              Length(IntToStr(lstSubtitles.RootNodeCount)) do
              CellText := '0' + CellText;
        end;
      // added by adenry: end
      // SHOW column
      ...
      3: // CellText := StringToWideStringEx(ReplaceEnters(Data.Text, '|'), CharsetToCodePage(OrgCharset)); //removed by adenry
        begin
          CellText := StringToWideStringEx(ReplaceEnters(Data.Text, '|'),
            CharsetToCodePage(OrgCharset)); // added by adenry
        end;
      // TRANSLATION column
      4: // CellText := StringToWideStringEx(ReplaceEnters(Data.Translation, '|'), CharsetToCodePage(TransCharset)); //removed by adenry
        CellText := StringToWideStringEx(ReplaceEnters(Data.Translation, '|'),
          CharsetToCodePage(TransCharset)); // added by adenry: begin
      ...
      5:
        begin
          ...
          begin
            ...
            CellText := IntToStr(TimeToFrames(Data.FinalTime - Data.InitialTime,
              GetInputFPS));
        end;
        ...
        begin
          ...
          if Assigned(PrevData) then
            ...
            begin
              ...
              CellText :=
                IntToStr(TimeToFrames(Data.InitialTime - PrevData.FinalTime,
                GetInputFPS));
        end;
      // added by adenry: end
    end;
...
begin
  if MsgBox(Format(ErrorMsg[02], [E.ClassName, Sender.ClassName,
    (Sender as TComponent).Name, E.Message, ID_EMAIL]), BTN_OK, BTN_CANCEL, '',
    MB_ICONERROR, frmMain) = 2 then
...
begin
  ...
  if (frmMain.Font.Charset = TransCharset) or
    ((frmMain.Font.Charset = ActualDefaultCharset) and
    (TransCharset = DEFAULT_CHARSET)) then
    UntranslatedSub := CurrLangUntranslatedSub
  else
...
begin
  ...
  ActualLangFile := ExtractFilePath(Application.ExeName) + 'Langs\' +
    ActualLangFile;
  ...
  try
    ...
    begin
      ...
      ID_PLAINTEXT := ReadString('General', 'PlainText', 'Plain Text') +
        ' (*.txt)|*.txt';
      ID_STPROJECT := ReadString('General', 'STP',
        'Subtitle Translation Project');
      ID_SRF := ReadString('General', 'SRF', 'Subtitle Report File') +
        ' (*.srf)|*.srf';
      ...
      rdoFinalTime.Caption := Copy(tmp, Pos('|', tmp) + 1,
        SmartPos('|', tmp, True, Pos('|', tmp) + 1) - (Pos('|', tmp) + 1));
      rdoBoth.Caption := Copy(tmp, SmartPos('|', tmp, True, Pos('|', tmp) + 1) +
        1, Length(tmp));

      lstSubtitles.Header.Columns[0].Text :=
        StringToWideStringEx(ReadString('Main Form', '08', 'Num'),
        CharsetToCodePage(frmMain.Font.Charset));
      lstSubtitles.Header.Columns[1].Text :=
        StringToWideStringEx(ReadString('Main Form', '09', 'Show'),
        CharsetToCodePage(frmMain.Font.Charset));
      lblShow.Caption := ReadString('Main Form', '09', 'Show') + ':';
      lstSubtitles.Header.Columns[2].Text :=
        StringToWideStringEx(ReadString('Main Form', '10', 'Hide'),
        CharsetToCodePage(frmMain.Font.Charset));
      ...
      lblDuration.Caption := ReplaceString(ReadString('Main Form', '14',
        'Duration'), ':', '') + ':';
      lstSubtitles.Header.Columns[5].Text :=
        StringToWideStringEx(ReadString('Main Form', '14', 'Duration'),
        CharsetToCodePage(frmMain.Font.Charset));
      // added by adenry: begin
      lblPause.Caption := ReadString('Main Form', '19', 'Pause') + ':';
      lstSubtitles.Header.Columns[6].Text :=
        StringToWideStringEx(ReadString('Main Form', '19', 'Pause'),
        CharsetToCodePage(frmMain.Font.Charset));
      // added by adenry: end
      ...
      CurrLangUntranslatedSub := ReadString('Main Form', '16',
        UNTRANSLATED_SUBTITLE); // added by adenry
      // added by adenry: begin
      ...
      NSelectedSubtitles := ReadString('Main Form', '20',
        '%d subtitles selected');
      NSelectedSubtitle := ReadString('Main Form', '21', '%d / %d');
      // added by adenry: end
      lstSubtitles.Header.Columns[3].Text := StringToWideStringEx(LabelText,
        CharsetToCodePage(frmMain.Font.Charset));
      lstSubtitles.Header.Columns[4].Text :=
        StringToWideStringEx(LabelTranslation,
        CharsetToCodePage(frmMain.Font.Charset));
      // added by adenry: begin
      ...
      mnuColDuration.Caption :=
        ReplaceString(ReadString('Main Form', '14', 'Duration'), ':', '');
      ...
      if Assigned(lstSubtitles.FocusedNode) then
        mmoSubTextChange(frmMain as TObject)
      else
      begin
      ...
      if (mnuTranslatorMode.Checked) then
        ...
        if (mnuTranslatorMode.Checked) then
          if Assigned(lstSubtitles.FocusedNode) then
            mmoTransChange(frmMain as TObject)
          else
          begin
      ...
      SetUntranslatedSubString;
      // added by adenry - change the UNTRANSLATED SUBTITLE string
      ...
      // Header
      captions[0] := ReadString('Main menu header', '01', 'File');
      // mnuFile.Caption replaced with captions[0] by adenry
      captions[1] := ReadString('Main menu header', '02', 'Edit');
      // mnuEdit.Caption replaced with captions[1] by adenry
      captions[2] := ReadString('Main menu header', '03', 'Search');
      // mnuSearch.Caption replaced with captions[2] by adenry
      captions[3] := ReadString('Main menu header', '04', 'Tools');
      // mnuTools.Caption replaced with captions[3] by adenry
      captions[4] := ReadString('Main menu header', '05', 'Movie');
      // mnuMovie.Caption replaced with captions[4] by adenry
      captions[5] := ReadString('Main menu header', '06', 'Settings');
      // mnuSettings.Caption replaced with captions[5] by adenry
      captions[6] := ReadString('Main menu header', '07', 'Help');
      // mnuHelp.Caption replaced with captions[6] by adenry
      captions[7] := ReadString('Main menu header', '08', 'View');
      // added by adenry

      // --------- //
      // File menu //
      // --------- //
      mnuNewSubtitle.Caption := ReadString('Main menu/File', '01',
        'New subtitle...');
      mnuLoadSubtitle.Caption := ReadString('Main menu/File', '02',
        'Load subtitle...');
      ...
      if Assigned(FindComponent('mnuClearRecemtListDropdown')) then
        TMenuItem(FindComponent('mnuClearRecemtListDropdown')).Caption :=
          mnuClearList.Caption;
      // added by adenry: end
      // Translator mode
      mnuLoad.Caption := ReadString('Main menu/File', '05', 'Load');
      mnuLoadProject.Caption := ReadString('Main menu/File', '06',
        'Project...');
      mnuLoadOriginal.Caption := ReadString('Main menu/File', '07', 'Original');
      mnuLoadTranslated.Caption := ReadString('Main menu/File', '08',
        'Translated');
      ...
      mnuSaveOriginalAs.Caption := ReadString('Main menu/File', '10',
        'Original as...');
      mnuSaveTranslatedAs.Caption := ReadString('Main menu/File', '11',
        'Translated as...');
      // ------
      ...
      mnuSaveMarking.Caption := ReadString('Main menu/File', '15',
        'Save marking'); // added by adenry
      ...
      mnuInsertSubtitle.Caption := ReadString('Main menu/Edit', '03',
        'Insert subtitle');
      mnuInsertBefore.Caption := ReadString('Main menu/Edit', '04',
        'Insert before');
      mnuRemoveSelected.Caption := ReadString('Main menu/Edit', '05',
        'Remove selected');
      ...
      mnuInvertSelection.Caption := ReadString('Main menu/Edit', '10',
        'Invert selection');
      mnuInsertSymbol.Caption := ReadString('Main menu/Edit', '11',
        'Insert symbol');
      ...
      mnuRemoveColorTags.Caption := ReadString('Popup Menu', '05',
        'Remove color tags');
      mnuRemoveAllTags.Caption := ReadString('Popup Menu', '06',
        'Remove all tags');
      // added by adenry: end
      ...
      // Edit/Timings //
      // ------------ //
      mnuTimings.Caption := ReadString('Main menu/Edit/Timings', '01',
        'Timings');
      mnuSetDurationLimits.Caption := ReadString('Main menu/Edit/Timings', '02',
        'Set duration limits...');
      mnuSetDelay.Caption := ReadString('Main menu/Edit/Timings', '03',
        'Set delay...');
      mnuAdjust.Caption := ReadString('Main menu/Edit/Timings', '04', 'Adjust');
      mnuAdjustSubtitles.Caption := ReadString('Main menu/Edit/Timings', '05',
        'Adjust subtitles...');
      mnuAdjustToSyncSubs.Caption := ReadString('Main menu/Edit/Timings', '06',
        'Adjust to synchronized subtitles');
      mnuTimeExpanderReducer.Caption := ReadString('Main menu/Edit/Timings',
        '07', 'Duration expander/reducer...');
      mnuExtendLength.Caption := ReadString('Main menu/Edit/Timings', '08',
        'Extend length to next');
      mnuAutomaticDurations.Caption := ReadString('Main menu/Edit/Timings',
        '09', 'Automatic durations...');
      mnuReadTimesFromFile.Caption := ReadString('Main menu/Edit/Timings', '10',
        'Read timings from file');
      ShiftMS := ReadString('Main menu/Edit/Timings', '11',
        'Shift %s milliseconds');
      // added by adenry: begin
      mnuExtendLength2.Caption := ReadString('Main menu/Edit/Timings', '12',
        'Extend length to previous');
      mnuSetPauses.Caption := ReadString('Main menu/Edit/Timings', '13',
        'Set Pauses...');
      mnuShiftToNext.Caption := ReadString('Main menu/Edit/Timings', '14',
        'Shift to next');
      mnuShiftToPrevious.Caption := ReadString('Main menu/Edit/Timings', '15',
        'Shift to previous');
      mnuRoundTimeValues.Caption := ReadString('Main menu/Edit/Timings', '16',
        'Round time values...');
      // added by adenry: end
      ...
      mnuSmartLineAdjust.Caption := ReadString('Main menu/Edit/Texts', '02',
        'Smart line adjust...');
      mnuConvertCase.Caption := ReadString('Main menu/Edit/Texts', '03',
        'Convert case...');
      mnuUnbreakSubtitles.Caption := ReadString('Main menu/Edit/Texts', '04',
        'Unbreak subtitles...');
      mnuDivideLines.Caption := ReadString('Main menu/Edit/Texts', '05',
        'Divide lines...');
      mnuFastDivideLines.Caption := ReadString('Main menu/Edit/Texts', '06',
        'Fast divide lines');
      mnuSetMaxLineLength.Caption := ReadString('Main menu/Edit/Texts', '07',
        'Set maximum line length');
      mnuReadTextsFromFile.Caption := ReadString('Main menu/Edit/Texts', '08',
        'Read texts from file');
      // added by adenry: begin
      mnuFastSmartLineAdjust.Caption := ReadString('Main menu/Edit/Texts', '09',
        'Fast smart line adjust');
      mnuFastUnbreakSubtitles.Caption := ReadString('Main menu/Edit/Texts',
        '10', 'Fast unbreak subtitles');
      // added by adenry: end

      // -------------- //
      // Edit/Subtitles //
      // -------------- //
      mnuSubtitles.Caption := ReadString('Main menu/Edit/Subtitles', '01',
        'Subtitles');
      mnuCombineSubtitles.Caption := ReadString('Main menu/Edit/Subtitles',
        '02', 'Combine subtitles');
      // ---
      mnuEffects.Caption := ReadString('Main menu/Edit/Subtitles', '03',
        'Effects');
      mnuTypeEffect.Caption := ReadString('Main menu/Edit/Subtitles', '04',
        'Type effect');
      mnuFlash.Caption := ReadString('Main menu/Edit/Subtitles', '05', 'Flash');
      mnuFastFlash.Caption := ReadString('Main menu/Edit/Subtitles', '06',
        'Fast flash');
      mnuMediumFlash.Caption := ReadString('Main menu/Edit/Subtitles', '07',
        'Medium flash');
      mnuSlowFlash.Caption := ReadString('Main menu/Edit/Subtitles', '08',
        'Slow flash');
      // ---
      mnuRightToLeft.Caption := ReadString('Main menu/Edit/Subtitles', '09',
        'Right-to-Left');
      mnuReverseText.Caption := ReadString('Main menu/Edit/Subtitles', '10',
        'Reverse text');
      mnuFixPunctuation.Caption := ReadString('Main menu/Edit/Subtitles', '11',
        'Fix punctuation');
      mnuSort.Caption := ReadString('Main menu/Edit/Subtitles', '12', 'Sort');
      mnuDeleteUnnecessaryLinks.Caption :=
        ReadString('Main menu/Edit/Subtitles', '13',
        'Delete unnecessary links');
      mnuMarkSelectedSubs.Caption := ReadString('Main menu/Edit/Subtitles',
        '14', 'Mark selected subtitles');
      mnuUnMarkSelectedSubs.Caption := ReadString('Main menu/Edit/Subtitles',
        '15', 'Unmark selected subtitles');
      // added by adenry: begin
      mnuAutoCombineSubtitles.Caption := ReadString('Main menu/Edit/Subtitles',
        '16', 'Auto combine subtitles...');
      mnuDeleteDotsAtBeginning.Caption := ReadString('Main menu/Edit/Subtitles',
        '17', 'Delete dots at the beginning');
      mnuJumpToNextMarked.Caption := ReadString('Main menu/Edit/Subtitles',
        '18', 'Jump to next marked');
      mnuJumpToPrevMarked.Caption := ReadString('Main menu/Edit/Subtitles',
        '19', 'Jump to previous marked');
      // added by adenry: end

      // ---------------- //
      // Edit/Translation //
      // ---------------- //
      mnuTranslation.Caption := ReadString('Main menu/Edit/Translation', '01',
        'Translation');
      mnuTranslatorMode.Caption := ReadString('Main menu/Edit/Translation',
        '02', 'Translator mode');
      ...
      // View menu //
      // --------- //
      mnuShowLeftPanel.Caption := ReadString('Main menu/Settings', '03',
        'Show left panel');
      mnuShowTimeControls.Caption := ReadString('Main menu/Settings', '04',
        'Show time controls');
      // added by adenry: begin
      mnuToolbar.Caption := ReadString('Main menu/View', '01', 'Toolbar');
      mnuTextStyleBars.Caption := ReadString('Main menu/View', '02',
        'Text style bars');
      mnuStatusbar.Caption := ReadString('Main menu/View', '03', 'Statusbar');
      mnuColumns.Caption := ReadString('Main menu/View', '04', 'Columns');
      mnuShowNotesInLeftPanel.Caption := ReadString('Main menu/View', '05',
        'Show notes in left panel');
      mnuShowTimeControlButtons.Caption := ReadString('Main menu/View', '06',
        'Show time control buttons');
      mnuShowLinesCount.Caption := ReadString('Main menu/View', '07',
        'Show lines count');
      mnuShowLinesCount.Caption := ReadString('Main menu/View', '07',
        'Show lines count');
      mnuVisualSubRepr.Caption := ReadString('Main menu/View', '08',
        'Visual subtitle representation');
      // added by adenry: end
      ...
      mnuSearchAndReplace.Caption := ReadString('Main menu/Search', '03',
        'Search && Replace...');
      mnuGoToLineNumber.Caption := ReadString('Main menu/Search', '04',
        'Go to line number...');

      // ---------- //
      // Tools menu //
      // ---------- //
      mnuSpellCheck.Caption := ReadString('Main menu/Tools', '01',
        'Spell check');
      mnuBatchConvert.Caption := ReadString('Main menu/Tools', '02',
        'Batch convert...');
      mnuSplitSubtitle.Caption := ReadString('Main menu/Tools', '03',
        'Split subtitle...');
      mnuJoinSubtitle.Caption := ReadString('Main menu/Tools', '04',
        'Join subtitles...');
      mnuInfoErrors.Caption := ReadString('Main menu/Tools', '05',
        'Information and errors');
      mnuInformationAndErrors.Caption := ReadString('Main menu/Tools', '06',
        'Information and errors...');
      mnuVariousInformation.Caption := ReadString('Main menu/Tools', '07',
        'Various information...');
      mnuOCRScripts.Caption := ReadString('Main menu/Tools', '08',
        'OCR Scripts');
      chkOCRScripts.Caption := mnuOCRScripts.Caption; // added by adenry
      mnuShowInMainForm.Caption := ReadString('Main menu/Tools', '09',
        'Show in Main Window');
      mnuInfoErrorsSettings.Caption := ReadString('Main menu/Tools', '10',
        'Settings...');
      mnuRecheckErrors.Caption := ReadString('Main menu/Tools', '11',
        'Recheck errors');
      mnuFixAllErrors.Caption := ReadString('Main menu/Tools', '12',
        'Fix all errors');
      mnuFixErrorsInSelSubs.Caption := ReadString('Main menu/Tools', '13',
        'Fix errors (selected subtitles)');
      mnuJumpToNextError.Caption := ReadString('Main menu/Tools', '14',
        'Jump to next error');
      mnuAddFPSfromAVI.Caption := ReadString('Main menu/Tools', '15',
        'Add FPS from AVI');
      mnuExternalPreview.Caption := ReadString('Main menu/Tools', '16',
        'External preview');
      mnuSAMILangExtractor.Caption := ReadString('Main menu/Tools', '17',
        'SAMI language extractor');
      mnuPascalScripts.Caption := ReadString('Main menu/Tools', '18',
        'Pascal scripts');
      // added by adenry: begin
      mnuJumpToNextError.Caption := ReadString('Main menu/Tools', '14',
        'Jump to next error');
      mnuJumpToPrevError.Caption := ReadString('Main menu/Tools', '19',
        'Jump to previous error');
      mnuShowCPSHintBoxes.Caption := ReadString('Main menu/Tools', '20',
        'Show CPS boxes');
      // added by adenry: end
      ...
      mnuMovieInfo.Caption := ReadString('Main menu/Movie', '03',
        'Information...');
      mnuVideoPreviewMode.Caption := ReadString('Main menu/Movie', '04',
        'Video preview mode');
      mnuShowSubtitles.Caption := ReadString('Main menu/Movie', '05',
        'Show subtitles');
      mnuSaveMediaStartupFile.Caption := ReadString('Main menu/Movie', '06',
        'Save media startup file');
      mnuAudioStream.Caption := ReadString('Main menu/Movie', '07',
        'Audio stream'); // added by adenry
      NoneAudioStream := ReadString('Main menu/Movie', '08', 'None');
      // added by adenry
      mnuVideoRenderer.Caption := ReadString('Main menu/Movie', '09',
        'Video renderer'); // added by adenry
      mnuDefaultVidRend.Caption := ReadString('Main menu/Movie', '10',
        'Default'); // added by adenry
      mnuVMR9VidRend.Caption := ReadString('Main menu/Movie', '11', 'VMR9');
      // added by adenry

      // -------------- //
      // Movie/Playback //
      // -------------- //
      mnuPlayback.Caption := ReadString('Main menu/Movie/Playback', '01',
        'Playback');
      mnuPlayPause.Caption := ReadString('Main menu/Movie/Playback', '02',
        'Play/Pause');
      mnuStop.Caption := ReadString('Main menu/Movie/Playback', '03', 'Stop');
      mnuRewind.Caption := ReadString('Main menu/Movie/Playback', '04',
        'Backward');
      mnuForward.Caption := ReadString('Main menu/Movie/Playback', '05',
        'Forward');
      mnuBack5Sec.Caption := ReadString('Main menu/Movie/Playback', '06',
        'Backward 5 seconds');
      mnuFwd5Sec.Caption := ReadString('Main menu/Movie/Playback', '07',
        'Forward 5 seconds');
      mnuPlaybackRate.Caption := ReadString('Main menu/Movie/Playback', '08',
        'Playback rate');

      // --------------- //
      // Movie/Subtitles //
      // --------------- //
      mnuVidSubtitles.Caption := ReadString('Main menu/Movie/Subtitles', '01',
        'Subtitles');
      mnuMoveSubtitle.Caption := ReadString('Main menu/Movie/Subtitles', '02',
        'Move subtitle');
      mnuSetShowTime.Caption := ReadString('Main menu/Movie/Subtitles', '03',
        'Set show time');
      mnuSetHideTime.Caption := ReadString('Main menu/Movie/Subtitles', '04',
        'Set hide time');
      mnuStartSubtitle.Caption := ReadString('Main menu/Movie/Subtitles', '05',
        'Start subtitle');
      mnuEndSubtitle.Caption := ReadString('Main menu/Movie/Subtitles', '06',
        'End subtitle');

      // --------------------- //
      // Movie/Synchronization //
      // --------------------- //
      mnuSynchronization.Caption :=
        ReadString('Main menu/Movie/Synchronization', '01', 'Synchronization');
      mnuFirstSyncPoint.Caption := ReadString('Main menu/Movie/Synchronization',
        '02', 'First sync point');
      mnuLastSyncPoint.Caption := ReadString('Main menu/Movie/Synchronization',
        '03', 'Last sync point');
      mnuAddSyncPoint.Caption := ReadString('Main menu/Movie/Synchronization',
        '04', 'Add synchronization point');

      // ------------- //
      // Movie/Display //
      // ------------- //
      mnuSubtitleToDisplay.Caption := ReadString('Main menu/Movie/Display',
        '01', 'Display');
      mnuDisplayOriginal.Caption := ReadString('Main menu/Movie/Display', '02',
        'Original');
      mnuDisplayTranslation.Caption := ReadString('Main menu/Movie/Display',
        '03', 'Translation');
      ...
      mnuOutputSettings.Caption := ReadString('Main menu/Settings', '01',
        'Output settings...');
      mnuLanguage.Caption := ReadString('Main menu/Settings', '02', 'Language');
      mnuUseInPlaceEdition.Caption := ReadString('Main menu/Settings', '05',
        'Use in-place edition');
      mnuRefreshLanguageFiles.Caption := ReadString('Main menu/Settings', '06',
        'Refresh language files'); // added by adenry
      FilesRefreshedStr := ReadString('Main menu/Settings', '07',
        'Files refreshed'); // added by adenry
      KeysStr := ReadString('Main menu/Settings', '08', 'keys');
      // added by adenry
      // --------- //
      // Help menu //
      // --------- //
      mnuHelpOfProgram.Caption := captions[6];
      mnuAboutSubtitleWorkshop.Caption :=
        Format(ReadString('Main menu/Help', '01', 'About %s...'), [ID_PROGRAM]);
      mnuCheckForNewVersion.Caption := ReadString('Main menu/Help', '02',
      ...
      tbInfoErrorsSettings.Hint := mnuInfoErrorsSettings.Caption + ' (' +
        mnuInfoErrors.Caption + ')';
      ...
      mnuRemoveColorTagsPopup.Caption := mnuRemoveColorTags.Caption;
      // modified by adenry
      // added by adenry: begin
      ...
      // -------------------------- //
      tbtnShowSnap.Hint := ReadString('Time control buttons hints', '01',
        'Move to previous/Extend to previous');
      tbtnHideSnap.Hint := ReadString('Time control buttons hints', '02',
        'Move to next/Extend to next');
      tbtnPauseSet.Hint := ReadString('Time control buttons hints', '03',
        'Set default pause/Set default pause, if pause is too short');
      tbtnDurationSet.Hint := ReadString('Time control buttons hints', '04',
      ...
      // Video Preview Hints   //
      // ------------------------ //
      btnPlay.Hint := Format(ReadString('Video preview hints', '01',
        'Play/Pause (%s)'), [ShortCutToText(mnuPlayPause.ShortCut)]);
      btnPause.Hint := btnPlay.Hint;
      btnStop.Hint := Format(ReadString('Video preview hints', '02',
        'Stop (%s)'), [ShortCutToText(mnuStop.ShortCut)]);
      btnScrollList.Hint := ReadString('Video preview hints', '03',
        'Toggle scroll list');
      btnPrevSub.Hint := ReadString('Video preview hints', '04',
        'Jump to previous subtitle');
      btnNextSub.Hint := ReadString('Video preview hints', '05',
        'Jump to next subtitle');
      btnRew.Hint := Format(ReadString('Video preview hints', '06',
        'Backward (%s)'), [ShortCutToText(mnuRewind.ShortCut)]);
      btnForward.Hint := Format(ReadString('Video preview hints', '07',
        'Forward (%s)'), [ShortCutToText(mnuForward.ShortCut)]);
      btnAlterPlaybackRate.Hint := ReadString('Video preview hints', '08',
        'Alter playback rate');
      btnMoveSubtitle.Hint := Format(ReadString('Video preview hints', '09',
        'Move subtitle (%s)'), [ShortCutToText(mnuMoveSubtitle.ShortCut)]);
      btnSetShowTime.Hint := Format(ReadString('Video preview hints', '10',
        'Set show time (%s)'), [ShortCutToText(mnuSetShowTime.ShortCut)]);
      btnSetHideTime.Hint := Format(ReadString('Video preview hints', '11',
        'Set hide time (%s)'), [ShortCutToText(mnuSetHideTime.ShortCut)]);
      btnStartSubtitle.Hint := Format(ReadString('Video preview hints', '12',
        'Start subtitle (%s)'), [ShortCutToText(mnuStartSubtitle.ShortCut)]);
      btnEndSubtitle.Hint := Format(ReadString('Video preview hints', '13',
        'End subtitle (%s)'), [ShortCutToText(mnuEndSubtitle.ShortCut)]);
      btnSyncPoint1.Hint := Format(ReadString('Video preview hints', '14',
        'Mark as first sync point (%s)'),
        [ShortCutToText(mnuFirstSyncPoint.ShortCut)]);
      btnSyncPoint2.Hint := Format(ReadString('Video preview hints', '15',
        'Mark as last sync point (%s)'),
        [ShortCutToText(mnuLastSyncPoint.ShortCut)]);
      btnAddSyncPoint.Hint := Format(ReadString('Video preview hints', '16',
        'Add subtitle/video synchronization point (%s)'),
        [ShortCutToText(mnuAddSyncPoint.ShortCut)]);
      // added by adenry:begin
      ...
      // Video popup menu //
      // ---------------- //
      mnuZoomTo25percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [25]);
      mnuZoomTo50percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [50]);
      mnuZoomTo75percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [75]);
      mnuZoomTo100percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [100]);
      mnuZoomTo150percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [150]);
      mnuZoomTo200percent.Caption :=
        Format(ReadString('Video Popup Menu', '03', 'Zoom to %d%%'), [200]);
      EnterFullScreen := ReadString('Video Popup Menu', '01',
        'Enter full screen');
      mnuToggleFullscreen.Caption := EnterFullScreen;
      ExitFullScreen := ReadString('Video Popup Menu', '02',
        'Exit full screen');
      mnuAspectRatio.Caption := ReadString('Video Popup Menu', '05',
        'Aspect ratio');
      ...
      mnuResetSubVerticalPos.Caption := ReadString('Video Popup Menu', '04',
        'Reset subtitle vertical position');
      ...
      // Error messages      //
      // ------------------------ //
      ErrorMsg[01] := ReadString('Error messages', '01',
        'Could not initialize Subtitle API. Please download %s from %s and copy it to the "%s" directory.');
      ErrorMsg[02] := ReadString('Error messages', '02',
        'The following error (%s) was derived from object %s (%s):||"%s"||Please write to %s informing what you were doing when this error occurred.||Press Ok to continue, and Cancel to exit the program.');
      ErrorMsg[03] := ReadString('Error messages', '03',
        'The file "%s" is a bad subtitle or an unsupported format.');
      ErrorMsg[04] := ReadString('Error messages', '04',
        'The file "%s" is not in %s format or is not a valid subtitle file.');
      ErrorMsg[05] := ReadString('Error messages', '05',
        '"%s" is not a valid video file.');
      ErrorMsg[06] := ReadString('Error messages', '06',
        'Not a valid line number.');
      ErrorMsg[07] := ReadString('Error messages', '07',
        'Select at least one extension to search for.');
      ErrorMsg[08] := ReadString('Error messages', '08',
        'The search path does not exist.');
      ErrorMsg[09] := ReadString('Error messages', '09',
        'Item number is not a valid item.');
      ErrorMsg[10] := ReadString('Error messages', '10',
        'Please add two subtitles or more.');
      ErrorMsg[11] := ReadString('Error messages', '11',
        'Failed to connect to server!');
      ErrorMsg[12] := ReadString('Error messages', '12',
        'Couldn''t remove the read-only attribute from "%s".||Possibly on a write protected drive.');
      ErrorMsg[13] := ReadString('Error messages', '13',
        'Error trying to connect to Microsoft Word!');
      ErrorMsg[14] := ReadString('Error messages', '14',
        'Select the video(s) for previous part(s) first!');
      ErrorMsg[15] := ReadString('Error messages', '15',
        'Select a valid output directory.');
      ErrorMsg[16] := ReadString('Error messages', '16',
        'Select at least one language to extract.');
      ErrorMsg[17] := ReadString('Error messages', '17',
        '"%s" is not a valid SAMI file.');
      ErrorMsg[18] := ReadString('Error messages', '18',
        'Select a subtitle part in the list.');
      ErrorMsg[19] := ReadString('Error messages', '19',
        'If you select a movie fragment for one part of the subtitle you need to select a movie fragment for all parts of the subtitle (except the last part of course).');
      ErrorMsg[20] := ReadString('Error messages', '20',
        'You need two or more points.');
      // added by adenry: begin
      ErrorMsg[21] := ReadString('Error messages', '21',
        'Incorrect regular expression in Custom info %s.');
      ErrorMsg[22] := ReadString('Error messages', '22',
        'Custom Info %d was disabled.|Fix the regular expression to enable it.');
      ErrorMsg[23] := ReadString('Error messages', '23',
        'Subtitle duration too short!');
      ErrorMsg[24] := ReadString('Error messages', '24',
        'Error: can not execute script.');
      ErrorMsg[25] := ReadString('Error messages', '25',
        'Error: can not compile script.');
      ErrorMsg[26] := ReadString('Error messages', '26',
        'Save the subtitles first.');
      ErrorMsg[27] := ReadString('Error messages', '27',
        'The file:|%s|does not exist.');
      ErrorMsg[28] := ReadString('Error messages', '28',
        'The file:|%s|is a bad srf file.');
      // added by adenry: end

      // ------------------------ //
      // Question messages     //
      // ------------------------ //
      QuestionMsg[01] := ReadString('Question messages', '01',
        'File "%s" has changed.||Do you want to save the changes?');
      QuestionMsg[02] := ReadString('Question messages', '02',
        'File "%s" already exists.||Do you want to replace it?');
      QuestionMsg[03] := ReadString('Question messages', '03',
        'Original file ("%s") has changed.||Do you want to save the changes?');
      QuestionMsg[04] := ReadString('Question messages', '04',
        'Translated file ("%s") has changed.||Do you want to save the changes?');
      QuestionMsg[05] := ReadString('Question messages', '05',
        'The selected subtitles are about to be deleted.||Do you want to proceed?');
      QuestionMsg[06] := ReadString('Question messages', '06',
        'Point 1 Subtitle: %s|Point 1 Movie: %s||Point 2 Subtitle: %s|Point 2 Movie: %s||Synchronize subtitle?');
      QuestionMsg[07] := ReadString('Question messages', '07',
        'The video player''s exe hasn''t been specified or doesn''t exist.||Do you want to configure external player?');
      QuestionMsg[08] := ReadString('Question messages', '08',
        'A new version was found!||Do you want to see the list of changes and download it?');
      QuestionMsg[09] := ReadString('Question messages', '09',
        'The file you are trying to save is read-only.||Try to save anyway?');
      QuestionMsg[10] := ReadString('Question messages', '10',
        'Do you want to save the marking in a file?');

      // ------------------------ //
      // Information messages   //
      // ------------------------ //
      InfoMsg[01] := ReadString('Information messages', '01',
        'Could''t find more instances of "%s".');
      InfoMsg[02] := ReadString('Information messages', '02',
        'No new version available.');
      InfoMsg[03] := ReadString('Information messages', '03',
        'Spell check finished.||%d change(s) made.');
      InfoMsg[04] := ReadString('Information messages', '04',
        'Too many parts. Resetting...');
      InfoMsg[05] := ReadString('Information messages', '05',
        'Only one language was found.||If the file contains more languages, add the ones you wish manually opening the file in a text editor and searching for the correct class name.');
      InfoMsg[06] := ReadString('Information messages', '06',
        'Couldn''t locate "STYLE" start and/or close tag!||Probably because of that reason, this file doesn''t contain more than one language.|If it does, try to add manually searching for the class name of the desired language(s).');
      InfoMsg[07] := ReadString('Information messages', '07',
        'Fittest font size for playback is: %d');
      InfoMsg[08] := ReadString('Information messages', '08',
        'Total replacements: %d');
      InfoMsg[09] := ReadString('Information messages', '09',
        'File name: %s|Size: %s|FPS: %s|Duration: %s|Total frames: %d|Resolution: %dx%d|FourCC: %s');
      InfoMsg[10] := ReadString('Information messages', '10',
        'Some of the streams of the movie could not be rendered correctly. Probably the audio or video codec is missing.');
      InfoMsg[11] := ReadString('Information messages', '11',
        'It is not necessary to set the movie fragment corresponding to the last subtitle.');
      InfoMsg[12] := ReadString('Information messages', '12',
        'A sudden error occured:|"%s"');

      // --------------------------------- //
      // Information and errors messages  //
      // --------------------------------- //
      IEMsgBoxes[01] := ReadString('Information and errors messages', '01',
        'Subtitle number %d is an empty subtitle and is about to be deleted.||Do you want to proceed?');
      IEMsgBoxes[02] := ReadString('Information and errors messages', '02',
        'Subtitle number %d:||"%s" has a prohibited character and is about to be deleted.||Do you want to proceed?');
      IEMsgBoxes[03] := ReadString('Information and errors messages', '03',
        'Subtitle number %d:||"%s" is about to be deleted.||Do you want to proceed?');
      IEMsgBoxes[04] := ReadString('Information and errors messages', '04',
        'Part of subtitle number %d:||"%s" is about to be deleted, so the subtitle will be:||"%s"||Do you want to proceed?');
      IEMsgBoxes[05] := ReadString('Information and errors messages', '05',
        'Subtitle number %d:||"%s" is a one-line subtitle and starts with "-".||Do you want to delete the "-" at the beginning?');

      // --------------- //
      // Error reports  //
      // --------------- //
      ErrorReports[01] := ReadString('Information and errors', '16',
        'Contains lines without letters');
      ErrorReports[02] := ReadString('Information and errors', '17',
        'Empty subtitle');
      // ---
      ErrorReports[03] := ReadString('Information and errors', '18',
        'Overlapping with previous subtitle');
      ErrorReports[04] := ReadString('Information and errors', '19',
        'Bad values');
      ErrorReports[05] := ReadString('Information and errors', '20',
        'Too long duration');
      ErrorReports[06] := ReadString('Information and errors', '21',
        'Too short duration');
      ErrorReports[07] := ReadString('Information and errors', '22',
        'Too long line(s)');
      ErrorReports[08] := ReadString('Information and errors', '23',
        'Over two lines');
      // ---
      ErrorReports[09] := ReadString('Information and errors', '24',
        'Hearing impaired');
      ErrorReports[10] := ReadString('Information and errors', '25',
        'Has text before colon (":")');
      ErrorReports[11] := ReadString('Information and errors', '26',
        'Unnecessary dots');
      ErrorReports[12] := ReadString('Information and errors', '27',
        'Contains a prohibited character');
      ErrorReports[13] := ReadString('Information and errors', '28',
        'Repeated character');
      ErrorReports[14] := ReadString('Information and errors', '29',
        'Repeated subtitle');
      ErrorReports[15] := ReadString('Information and errors', '30',
        'OCR error');
      // ---
      ErrorReports[16] := ReadString('Information and errors', '31',
        'One line subtitle starts with "-"');
      ErrorReports[17] := ReadString('Information and errors', '32',
        'No space after custom character');
      ErrorReports[18] := ReadString('Information and errors', '33',
        'No space before custom character');
      ErrorReports[19] := ReadString('Information and errors', '34',
        'Unnecessary spaces');
      // ---
      ErrorReports[20] := ReadString('Information and errors', '35',
        'Marked subtitle');
      // ---
      ErrorReports[21] := ReadString('Information and errors', '54',
        'Unnecessary tags'); // added by adenry
      ErrorReports[22] := ReadString('Information and errors', '56',
        'Dash on first line'); // added by adenry
      ErrorReports[23] := ReadString('Information and errors', '58',
        'Too many CPS'); // added by adenry
      ErrorReports[24] := ReadString('Information and errors', '60',
        'Too short pause'); // added by adenry
      ErrorReports[25] := ReadString('Information and errors', '62',
        'Dialogue on one line'); // added by adenry

      // Custom info
      CustomInfoDelete := ReadString('Custom info', '33', 'Delete');
      // added by adenry
      CustomInfoEdit := ReadString('Custom info', '34', 'Edit');
      // added by adenry
  ...
  try
    ...
    lstSubtitles.Header.Height := lblMode.Height + (frmMain.lblMode.Left div 2);
    // added by adenry
    // added by adenry: begin
    ...
    // added by adenry: end
    lstSubtitles.ParentFont := False; // <----------- Kameleon
    ...
    if tmeDuration.Enabled then // added later
      tmeDurationTimeChange(tmeDuration, tmeDuration.Time);
    // added by adenry later - refresh tmeDuration, tmeShow, and tmeHide text color
    tmePause.ParentFont := True; // added by adenry
    if tmePause.Enabled then // added later
      tmePauseTimeChange(tmePause, tmePause.Time);
    // added by adenry later - refresh tmePause text color
...
begin
  OCRDefFile := ExtractFilePath(Application.ExeName) + 'OCRScripts\' +
    (Sender as TMenuItem).Caption + ID_OCREXT;
  cmbOCRScripts.ItemIndex := cmbOCRScripts.Items.IndexOf
    (ExtractFileName(Copy(OCRDefFile, 1, Length(OCRDefFile) - 4)));
end;
...
begin
  ...
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + 'Langs\*.lng',
      faAnyFile, Busca);
    while i = 0 do
    begin
    ...
    begin
      ...
      if Langs[i] = 'English' then
        A.Default := True; // added by adenry
...
begin
  ...
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + 'OCRScripts\*' +
      ID_OCREXT, faAnyFile, Busca);
    while i = 0 do
    begin
...
begin
  Scripts := TStringList.Create;
  try
    i := FindFirst(ExtractFilePath(Application.ExeName) + 'PascalScripts\*.pas',
      faAnyFile, Busca);
    while i = 0 do
    begin
      Scripts.Add(Copy(Busca.Name, 1, Length(Busca.Name) - 4));
  ...
  if mnuPascalScripts.Count = 0 then
    mnuPascalScripts.Visible := False
  else
...
begin
  ...
  lblInputFPS.Enabled := Flag; // Kameleon
  cmbInputFPS.Enabled := Flag; // Kameleon
  lblFPS.Enabled := Flag;
  cmbFPS.Enabled := Flag;
  ...
  begin
    ...
    else if rdoFinalTime.Checked then
    begin
...
begin
  try
    ...
    begin
      ...
      if (Ext = ID_STPEXT) then // Subtitle translation project
      ...
      else if (Ext = ID_SRFEXT) then // URUSoft Subtitle Report file
        LoadSRF(FileName)
      else
        // Video/audio file
        ...
        if VideoAudioExts.IndexOf(Copy(Ext, 2, MaxInt)) >= 0 then
        // added by adenry
        begin
          // AudioStreamNum := 0; //added by adenry
          if LoadMovie(FileName) = False then
            MsgBox(Format(ErrorMsg[05], [FileName]), BTN_OK, '', '',
              MB_ICONERROR, frmMain)
          else
          begin
        ...
        begin // Subtitle file
          ...
          begin
            LoadSubtitle(FileName, GetInputFPS);
            UTF8_Orig := SubtitleAPI.Get_UTF8_File;
          end
          else
          begin
            LoadSubtitle(FileName, GetFPS, 0, True);
            UTF8_Trans := SubtitleAPI.Get_UTF8_File;
          end;
          RefreshStatusbar;
        end;
      Msg.Result := 0;
...
begin
  ...
  // added by adenry //this also fixes a fullscreen problem
  begin
    ...
    begin
      ...
      // 1 SUBTITLE SELECTED
      begin
        if lstSubtitles.FocusedNode <> lstSubtitles.GetFirstSelected then
        // added by adenry
          lstSubtitles.FocusedNode := lstSubtitles.GetFirstSelected;
        // added by adenry
        ...
        begin
          ...
          SetSWTextEditText(mmoSubtitleText, Data.Text);
          // added by adenry to properly handle RichEdit Text
          mmoSubtitleText.SelStart := s;
          if mnuTranslatorMode.Checked then
          begin
            s := mmoTranslation.SelStart;
            // mmoTranslation.Text := Data.Translation; //removed by adenry
            SetSWTextEditText(mmoTranslation, Data.Translation);
            // added by adenry to properly handle RichEdit Text
            mmoTranslation.SelStart := s;
          end;
          tmeShow.Time := Data.InitialTime;
          tmeHide.Time := Data.FinalTime;
          if Data.FinalTime >= Data.InitialTime // Kameleon
          then tmeDuration.Time := Data.FinalTime - Data.InitialTime
          else tmeDuration.Time := 0 // Kameleon
        end;
        ...
        begin
          ...
          if Data.InitialTime >= Data2.FinalTime  // Kameleon
          then tmePause.Time := Data.InitialTime - Data2.FinalTime
          else tmePause.Time := 0;  // Kameleon
    ...
    if mnuVisualSubRepr.Checked then
      sbSeekBar.Repaint;
    // added by adenry to refresh subtitles graphical representation
  end;
end;
...
begin
  for i := 0 to High(RFMenuItems) do
    if (RFMenuItems[i] = Sender) or (RFMenuItemsDropdown[i] = Sender) then
    // (RFMenuItemsDropdown[i] = Sender) added by adenry
    begin
      Ext := LowerCase(ExtractFileExt(RecentFiles[i]));
      if (Ext = ID_STPEXT) then // Subtitle Translation Project
        LoadProject(RecentFiles[i])
      else
      begin
        if (mnuTranslatorMode.Checked) and (OrgFile <> '') then
        begin
          LoadSubtitle(RecentFiles[i], GetInputFPS, 0, True);
          UTF8_Trans := SubtitleAPI.Get_UTF8_File;
        end
        else
        begin
...
begin
  ...
  if mmoSubtitleText.ClassName = TRichEdit.ClassName then
  // added by adenry //if TSWTextEdit.InheritsFrom(TRichEdit) then
  begin
  ...
  try
    ...
    for i := 0 to mnuLanguage.Count - 1 do
      if mnuLanguage.Items[i].Caption = Copy(ExtractFileName(ActualLangFile), 0,
        Length(ExtractFileName(ActualLangFile)) - 4) then
      begin
    ...
    // ---------------------------------//
    SubtitleAPI := TSubtitleAPI.Create(ExtractFilePath(Application.ExeName) +
      ID_DLLDIR + '\' + ID_DLLNAME);

    if SubtitleAPI.Initialized = False then
    begin
      MsgBox(Format(ErrorMsg[01], [ID_DLLNAME, ID_WEBPAGE,
        ExtractFilePath(Application.ExeName) + ID_DLLDIR]), BTN_EXIT, '', '',
        MB_ICONERROR, Self);
      SubtitleAPI.Free;
      Ini.Free;
      ExitProcess(0);
    ...
    mnuShowInMainForm.Checked := Ini.ReadBool('General',
      'Show OCR Scripts in main form', True);
    mnuShowInMainFormClick(Sender); // added by adenry
    chkOCRScripts.Checked := Ini.ReadBool('General', 'Enable OCR Scripts',
      True); // added by adenry
    ...
    // added by adenry - use Windows' hand cursor instead of Delphi's
    ...
    Width := Ini.ReadInteger('Main Window', 'Width', 800);  // Kameleon
    Height := Ini.ReadInteger('Main Window', 'Height', 665); // Kameleon

    Left := Ini.ReadInteger('Main Window', 'Left', (Screen.Width - Width) div 2);
    Top  := Ini.ReadInteger('Main Window', 'Top', (Screen.Height - Height) div 2);

    // 405 changed to 605 by adenry
    if Ini.ReadBool('Main Window', 'Maximized', False) then
    // True changed to False by adenry
    ...
    begin
      SetWindowPos(frmMain.Handle, HWND_TOPMOST, frmMain.Left, frmMain.Top,
        frmMain.Width, frmMain.Height, SWP_NOMOVE + SWP_NOSIZE);
      FormMainAlwaysOnTop := True; // added by adenry
    end
    else // ; replaced with else by adenry - bug fix
    begin
    ...
    // Charsets             //
    // --------------------------------- //
    cmbOrgCharset.ItemIndex := Ini.ReadInteger('General', 'Original charset',
      1); // 0 changed to 1 by adenry - DEFAULT instead of ANSI
    cmbTransCharset.ItemIndex := Ini.ReadInteger('General',
      'Translated charset', 1);
    // 0 changed to 1 by adenry - DEFAULT instead of ANSI
    OrgCharset := StrCharsetToInt(cmbOrgCharset.Items[cmbOrgCharset.ItemIndex]);
    TransCharset := StrCharsetToInt
      (cmbTransCharset.Items[cmbTransCharset.ItemIndex]);
    ...
    begin
      ...
      //chkOCRScripts.Top := cmbTransCharset.Top + cmbTransCharset.Height + 3; // Kameleon: removed
      // added by adenry
      cmbOrgCharset.Show;
    ...
    begin
      ...
      //chkOCRScripts.Top := cmbOrgCharset.Top; // added by adenry  Kameleon: removed
      cmbOrgCharset.Hide;
      cmbTransCharset.Hide;
    end;
    //cmbOCRScripts.Top := chkOCRScripts.Top + chkOCRScripts.Height; // Kameleon: removed
    // added by adenry
    SendMessage(cmbOCRScripts.Handle, CB_SETDROPPEDWIDTH, pnlParent2.Left, 0);
    // widen the dropdown size a bit //added by adenry

    // added by adenry: begin
    ActualDefaultCharset := GetActualDefaultCharset;
    DashCharsets1 := [ANSI_CHARSET, GREEK_CHARSET, TURKISH_CHARSET,
      VIETNAMESE_CHARSET, HEBREW_CHARSET, ARABIC_CHARSET, BALTIC_CHARSET,
      RUSSIAN_CHARSET, EASTEUROPE_CHARSET, THAI_CHARSET];
    // contain a dash in #150, and #151
    if ActualDefaultCharset in DashCharsets1 then
      DashCharsets1 := DashCharsets1 + [DEFAULT_CHARSET];
    DashCharsets2 := [ANSI_CHARSET, GREEK_CHARSET, TURKISH_CHARSET,
      VIETNAMESE_CHARSET, HEBREW_CHARSET, ARABIC_CHARSET, BALTIC_CHARSET,
      RUSSIAN_CHARSET, EASTEUROPE_CHARSET];
    // contain a dash in #150, #151, and #173
    if ActualDefaultCharset in DashCharsets2 then
      DashCharsets2 := DashCharsets2 + [DEFAULT_CHARSET];
    Dashes := SetDashes(OrgCharset);

    NotesCharset := StrCharsetToInt
      (cmbOrgCharset.Items[Ini.ReadInteger('General', 'Notes charset', 1)]);
    // 0 changed to 1 later
    mmoNotes.Font.Charset := NotesCharset;
    mmoNotes.Text := ReplaceString(Ini.ReadString('General', 'Notes', ''),
      '|', #13#10);
    dlgColor.Color := Ini.ReadInteger('General', 'Tag Color', $0000DFDF);
    // default is yellow
    // added by adenry: end
    ...
    // Particular settings        //
    // --------------------------------- //
    ConfirmDelete := Ini.ReadBool('Settings',
      'Confirm when deleting a subtitle', False);
    InvalidFPlainText := Ini.ReadBool('Settings',
      'Interpret invalid files as plain text', False);
    AutoSearchMovie := Ini.ReadBool('Settings', 'Autosearch for movie', True);
    ForceWorkWithTime := Ini.ReadBool('Settings',
      'Force working in time mode', False);
    KeepOrderOfLines := Ini.ReadBool('Settings',
      'Keep order of lines when reverse text', True);
    SelTextNL := Ini.ReadBool('Settings',
      'Select text on jump to next line', True);
    SelTextPL := Ini.ReadBool('Settings',
      'Select text on jump to previous line', True);
    RFMaxCount := Ini.ReadInteger('Settings', 'MaxRF', 10);

    // Tags settings
    // SubtitleAPI.WorkWithTags          := Ini.ReadBool('Settings', 'Work with style tags', True); //removed by adenry
    // added by adenry: begin
    SubtitleAPI.NoInteractionWithTags := Ini.ReadInteger('Settings',
      'Tags mode', 2) = 1;
    SubtitleAPI.MultiTagsMode := Ini.ReadInteger('Settings',
      'Tags mode', 2) = 2;
    SubtitleAPI.SingleTagsMode := Ini.ReadInteger('Settings',
      'Tags mode', 2) = 3;
    SubtitleAPI.WorkWithTags := SubtitleAPI.MultiTagsMode or
      SubtitleAPI.SingleTagsMode; // added by adenry
    // added by adenry: end

    // ------------ //
    // Advanced   //
    // ------------ //
    TwoLinesIfLongerThan := Ini.ReadInteger('Advanced',
      'Two lines if longer than', 40);
    ...
    AdjustAutomatically := Ini.ReadBool('Advanced',
      'Smart line adjust automatically', True);
    MaxLineLength := Ini.ReadInteger('Advanced', 'Maximum line length', 45);
    ShiftTime := Ini.ReadInteger('Advanced', 'Shift time', 100);
    // added by adenry: begin
    DefaultSubDuration := Ini.ReadInteger('Advanced',
      'Default subtitle duration', 1000);
    DefaultSubPause := Ini.ReadInteger('Advanced',
      'Default subtitle pause', 200);
    // added by adenry: end
    // added by adenry: begin
    ...
    tmeShow.FramesStep := Ini.ReadInteger('Advanced',
      'Increase step frames', 1);
    ...
    // added by adenry: end
    KeepMinDur := Ini.ReadBool('Advanced', 'Keep min duration', False);
    // added by adenry
    KeepMinDurValue := Ini.ReadInteger('Advanced', 'Keep min duration value',
      1000); // added by adenry
    KeepMinDurDontOverlap := Ini.ReadBool('Advanced',
      'Keep min duration but do not overlap', True); // added by adenry
    // added by adenry: begin
    AutoRoundTimeVals := Ini.ReadBool('Advanced',
      'Auto round time values', False);
    RoundingFactor := Ini.ReadInteger('Advanced', 'Auto rounding factor', 100);
    RoundOnLoad := Ini.ReadBool('Advanced',
      'Round time values on subtitle load', False);
    // added by adenry: end
    ...
    tmrSaveWork.Enabled := Ini.ReadBool('Save',
      'Save work automatically', False);
    tmrSaveWork.Interval := Ini.ReadInteger('Save', 'Time interval', 60000);
    SaveAsBackup := Ini.ReadBool('Save', 'Save as backup', True);
    AskToSaveNewSubs := Ini.ReadBool('Save', 'Ask to save new subtitles', True);
    // added by adenry
    SaveEmptyLines := Ini.ReadBool('Save', 'Save empty lines', False);
    // added by adenry
    ...
    OnDoubleClick := Ini.ReadInteger('Video preview',
      'Double click in a subtitle', 1);
    OnShiftDblClick := Ini.ReadInteger('Video preview',
      'Shift-double click in a subtitle', 2);
    ...
    case Ini.ReadInteger('Video preview', 'Playback rate', 0) of
      ...
      9:
        mnu90P.Checked := True;
      10:
        mnu200P.Checked := True;
      11:
        mnu300P.Checked := True;
      12:
        mnu400P.Checked := True;
    end;
    RewFFTime := StrSecToMS(Ini.ReadString('Video preview',
      'Rewind and forward', '0,500'));
    DefAltPlayRate := Ini.ReadInteger('Video preview',
      'Default altered playback rate', 0) + 1;
    SetDefaultShortCut;
    ScrollList := Ini.ReadBool('Video preview', 'Scroll list', False);
    mnuDisplayOriginal.Checked := Ini.ReadBool('Video preview',
      'Displaying original', True);
    mnuDisplayTranslation.Checked := not mnuDisplayOriginal.Checked;
    mnuShowSubtitles.Checked := Ini.ReadBool('Video preview',
      'Show subtitles', True);
    VertVideoAlign := Ini.ReadBool('Video preview', 'Vertical video align',
      True); // added by adenry

    mnuVMR9VidRend.Checked := Ini.ReadBool('Video preview', 'Use VMR9', False);
    // added by adenry
    mnuDefaultVidRend.Checked := not mnuVMR9VidRend.Checked; // added by adenry

    VolumeSet(Ini.ReadInteger('Video preview', 'Volume', 100));
    // added by adenry
    ...
    FullScreenOnDblClick := Ini.ReadBool('Video preview',
      'Full screen on double click', True); // added by adenry
    PlayVideoOnLoad := Ini.ReadBool('Video preview', 'Play video on load',
      True); // added by adenry
    // added by adenry: begin
    VideoAspectRatio := Ini.ReadFloat('Video preview',
      'Video Aspect Ratio', -1.0);
    if SameValue(VideoAspectRatio, 4 / 3, EPSILON) then
      mnuAR4_3.Checked := True
    else if SameValue(VideoAspectRatio, 16 / 9, EPSILON) then
      mnuAR16_9.Checked := True
    else if SameValue(VideoAspectRatio, 1.85) then
      mnuAR185_1.Checked := True
    else if SameValue(VideoAspectRatio, 2.35) then
      mnuAR235_1.Checked := True;
    // added by adenry: end
    // added by adenry: begin
    ...
    // --------------------------------- //
    subSubtitle.Border := Ini.ReadBool('Video preview subtitles',
      'Draw border', True);
    subSubtitle.Shadow := Ini.ReadBool('Video preview subtitles',
      'Draw shadow', True);
    TransparentSubs := Ini.ReadBool('Video preview subtitles',
      'Transparent', True);
    ForceUsingReg := Ini.ReadBool('Video preview subtitles',
      'Force using regions', False) AND TransparentSubs;
    // AND TransparentSubs added by adenry
    subSubtitle.Font.Name := Ini.ReadString('Video preview subtitles',
      'Font name', 'Tahoma');
    subSubtitle.Font.Size := Ini.ReadInteger('Video preview subtitles',
      'Font size', 14);
    SubFontSize := subSubtitle.Font.Size; // added by adenry
    subSubtitle.TextColor := Ini.ReadInteger('Video preview subtitles',
      'Font color', clWhite);
    subSubtitle.Font.Color := subSubtitle.TextColor;
    // added by adenry //subSubtitle.Font.Color stores the color for marking
    BackgroundColor := Ini.ReadInteger('Video preview subtitles',
      'Background color', clBtnFace);
    subSubtitle.BorderWidth := Ini.ReadInteger('Video preview subtitles',
      'Border width', 1);
    subSubtitle.ShadowWidth := Ini.ReadInteger('Video preview subtitles',
      'Shadow width', 1);
    SubBorderWidth := subSubtitle.BorderWidth; // added by adenry
    SubShadowWidth := subSubtitle.ShadowWidth; // added by adenry
    subSubtitle.BorderColor := Ini.ReadInteger('Video preview subtitles',
      'Border color', clBlack); // added by adenry
    subSubtitle.ShadowColor := Ini.ReadInteger('Video preview subtitles',
      'Shadow color', clBlack); // added by adenry
    AlignLeftLineWithDash := Ini.ReadBool('Video preview subtitles',
      'Align left new line with dash', True); // added by adenry
    RelativeFontSize := Ini.ReadBool('Video preview subtitles',
      'Relative font size', True); // added by adenry
    KeepSubOnVideo := Ini.ReadBool('Video preview subtitles',
      'Keep subtitle on video', True); // added by adenry
    subSubtitleTopShift := Ini.ReadFloat('Video preview subtitles',
      'Subtitle top shift', 0.0); // added by adenry
    subSubtitle.Antialiased := Ini.ReadBool('Video preview subtitles',
      'Try antialiasing', not ForceUsingReg or not TransparentSubs);
    // added by adenry
    ...
    begin
      ...
      ShowConfMainForm := Ini.ReadBool('Information and Errors',
        'Show confirmations in main form on fix', False);
      AdvancedErrorMarking := Ini.ReadBool('Information and Errors',
        'Mark errors in main form''s list', True); // added by adenry
      ...
      MarkOnLoad := Ini.ReadBool('Information and Errors',
        'Mark errors on load subtitle', False);
      AutoRecheckErrors := Ini.ReadBool('Information and Errors',
        'Automatically recheck errors', True); // added by adenry
      AutoRecheckOnScripts := Ini.ReadInteger('Information and Errors',
        'Recheck on scripts', 1); // added by adenry
      FixOneUnitOverlap := Ini.ReadBool('Information and Errors',
        'Fix one unit overlap at load', False);
      FixOnLoad := Ini.ReadBool('Information and Errors',
        'Fix errors on load subtitle', False);
      OCRDefFile := Ini.ReadString('Information and Errors',
        'OCR Definitions file', ExtractFilePath(Application.ExeName) +
        'OCRScripts\Default' + ID_OCREXT);
      IndicTooLongLines := Ini.ReadBool('Information and Errors',
        'Indicate too long lines', True); // added by adenry
      TooLongLinesColor := Ini.ReadInteger('Information and Errors',
        'Indicate too long lines color', pnlIndicTooLongLinesColor.Font.Color);
      // added by adenry
      TooLongLinesBackground := Ini.ReadInteger('Information and Errors',
        'Indicate too long lines background',
        pnlIndicTooLongLinesBackground.Font.Color); // added by adenry
      IndicTooShortDuration := Ini.ReadBool('Information and Errors',
        'Indicate too short duration', True); // added by adenry
      TooShortDurationColor := Ini.ReadInteger('Information and Errors',
        'Indicate too short duration color',
        pnlIndicTooShortDurationColor.Font.Color); // added by adenry
      TooShortDurationBackground := Ini.ReadInteger('Information and Errors',
        'Indicate too short duration background',
        pnlIndicTooShortDurationBackground.Font.Color); // added by adenry
      IndicTooLongDuration := Ini.ReadBool('Information and Errors',
        'Indicate too long duration', True); // added by adenry
      TooLongDurationColor := Ini.ReadInteger('Information and Errors',
        'Indicate too long duration color',
        pnlIndicTooLongDurationColor.Font.Color); // added by adenry
      TooLongDurationBackground := Ini.ReadInteger('Information and Errors',
        'Indicate too long duration background',
        pnlIndicTooLongDurationBackground.Font.Color); // added by adenry
      IndicTooShortPause := Ini.ReadBool('Information and Errors',
        'Indicate too short pause', True); // added by adenry
      TooShortPauseColor := Ini.ReadInteger('Information and Errors',
        'Indicate too short pause color',
        pnlIndicTooShortPauseColor.Font.Color); // added by adenry
      TooShortPauseBackground := Ini.ReadInteger('Information and Errors',
        'Indicate too short pause background',
        pnlIndicTooShortPauseBackground.Font.Color); // added by adenry

      IndicOverlapping := Ini.ReadBool('Information and Errors',
        'Indicate overlapping', True); // added by adenry
      OverlappingColor := Ini.ReadInteger('Information and Errors',
        'Indicate overlapping color', pnlIndicOverlappingColor.Font.Color);
      // added by adenry
      OverlappingBackground := Ini.ReadInteger('Information and Errors',
        'Indicate overlapping background',
        pnlIndicOverlappingBackground.Font.Color); // added by adenry

      IndicBadValues := Ini.ReadBool('Information and Errors',
        'Indicate bad values', True); // added by adenry
      BadValuesColor := Ini.ReadInteger('Information and Errors',
        'Indicate bad values color', pnlIndicBadValuesColor.Font.Color);
      // added by adenry
      BadValuesBackground := Ini.ReadInteger('Information and Errors',
        'Indicate bad values background',
        pnlIndicBadValuesBackground.Font.Color); // added by adenry
      IndicOverTwoLines := Ini.ReadBool('Information and Errors',
        'Indicate over two lines', True); // added by adenry
      OverTwoLinesColor := Ini.ReadInteger('Information and Errors',
        'Indicate over two lines color', pnlIndicOverTwoLinesColor.Font.Color);
      // added by adenry
      OverTwoLinesBackground := Ini.ReadInteger('Information and Errors',
        'Indicate over two lines background',
        pnlIndicOverTwoLinesBackground.Font.Color); // added by adenry
      ...
      begin
        ...
        begin
          MarkFlags[i] :=
            TMarkFlags(Word(Ini.ReadInteger('Errors to mark',
            'Mark flags ' + IntToStr(i), Word(GetDefaultMarkFlags(i)))));
          MarkColors[i] := Ini.ReadInteger('Errors to mark',
            'Mark color ' + IntToStr(i),
            TPanel(FindComponent('pnlMarkColor_' + IntToStr(i))).Font.Color);
          MarkBackgrounds[i] := Ini.ReadInteger('Errors to mark',
            'Mark background ' + IntToStr(i),
            TPanel(FindComponent('pnlMarkBckgr_' + IntToStr(i))).Font.Color);
        end;
      end
      else
        SetDefaultStyles(True);
      MarkCPSHintBox := Ini.ReadBool('Information and Errors',
        'Display CPS hint box', True); // added by adenry
      // added by adenry: end

      for i := 0 to mnuOCRScripts.Count - 1 do
        if mnuOCRScripts.Items[i].Caption = ExtractFileName
          (Copy(OCRDefFile, 1, Length(OCRDefFile) - 4)) then
      ...
      // Advanced           //
      // ----------------------------- //
      RepeatableChars := Ini.ReadString('Information and Errors',
        'Repeatable chars', '-¡!¿?";\/_[]=');
      ProhibitedChars := Ini.ReadString('Information and Errors',
        'Prohibited chars', '@#*');
      ToleranceForRepeated := Ini.ReadInteger('Information and Errors',
        'Tolerance for repeated subtitles', 100);
      SpaceAfterChars := Ini.ReadString('Information and Errors',
        'Space after characters', '-');
      SpaceBeforeChars := Ini.ReadString('Information and Errors',
        'Space before characters', '');
      TooLongDuration := Ini.ReadInteger('Information and Errors',
        'Too long duration', 7000);
      TooShortDuration := Ini.ReadInteger('Information and Errors',
        'Too short duration', 1000);
      TooLongLine := Ini.ReadInteger('Information and Errors',
        'Too long line', 45);
      TooShortPause := Ini.ReadInteger('Information and Errors',
        'Too short pause', 200); // added by adenry
      TooMuchCPS := Ini.ReadInteger('Information and Errors', 'Too much CPS',
        20); // added by adenry
      NormalCPSColor := Ini.ReadInteger('Information and Errors',
        'Normal CPS Color', pnlNormalCPSColor.Font.Color); // added by adenry
      NormalCPSBckgr := Ini.ReadInteger('Information and Errors',
        'Normal CPS Background', pnlNormalCPSBckgr.Font.Color);
      // added by adenry
      TooMuchCPSColor := Ini.ReadInteger('Information and Errors',
        'Too much CPS Color', pnlTooMuchCPSColor.Font.Color); // added by adenry
      TooMuchCPSBckgr := Ini.ReadInteger('Information and Errors',
        'Too much CPS Background', pnlTooMuchCPSBckgr.Font.Color);
      // added by adenry
      PercentageInCPSHintBox := Ini.ReadBool('Information and Errors',
        'Percantage in CPS hint box', True); // added by adenry
      CurCPSInCPSHintBox := Ini.ReadBool('Information and Errors',
        'Current CPS in CPS hint box', False); // added by adenry
      MinTimeInCPSHintBox := Ini.ReadBool('Information and Errors',
        'Minimum time in CPS hint box', True); // added by adenry
      MaxCharInCPSHintBox := Ini.ReadBool('Information and Errors',
        'Maximum characters in CPS hint box', True); // added by adenry
    ...
    // Errors to check for      //
    // ----------------------------- //
    ErrorsToCheck.eLinesWithoutLetters := Ini.ReadBool('Errors to check for',
      'Lines without letters', True);
    ErrorsToCheck.eEmptySubtitle := Ini.ReadBool('Errors to check for',
      'Empty subtitles', True);
    ErrorsToCheck.eOverlapping := Ini.ReadBool('Errors to check for',
      'Overlapping subtitles', True);
    ErrorsToCheck.eBadValues := Ini.ReadBool('Errors to check for',
      'Bad values', True);
    ErrorsToCheck.eTooShortPause := Ini.ReadBool('Errors to check for',
      'Too short pause', True); // added by adenry
    ErrorsToCheck.eTooLongDurations := Ini.ReadBool('Errors to check for',
      'Too long durations', True);
    ErrorsToCheck.eTooShortDurations := Ini.ReadBool('Errors to check for',
      'Too short durations', True);
    ErrorsToCheck.eTooLongLines := Ini.ReadBool('Errors to check for',
      'Too long lines', True);
    ErrorsToCheck.eOverTwoLines := Ini.ReadBool('Errors to check for',
      'Subtitles over two lines', True);
    ErrorsToCheck.eHearingImpaired := Ini.ReadBool('Errors to check for',
      'Hearing impaired subtitles', True);
    ErrorsToCheck.eTextBeforeColon := Ini.ReadBool('Errors to check for',
      'Text before colon (":")', True);
    ErrorsToCheck.eOnlyIfCapitalLetters := Ini.ReadBool('Errors to check for',
      'Only if text is in capital letters', True);
    ErrorsToCheck.eUnnecessaryDots := Ini.ReadBool('Errors to check for',
      'Unnecessary dots', True);
    ErrorsToCheck.eProhibitedCharacter := Ini.ReadBool('Errors to check for',
      'Prohibited characters', False);
    ErrorsToCheck.eRepeatedCharacter := Ini.ReadBool('Errors to check for',
      'Repeated characters', True);
    ErrorsToCheck.eRepeatedSubtitle := Ini.ReadBool('Errors to check for',
      'Repeated subtitles', True);
    ErrorsToCheck.eOCRErrors := Ini.ReadBool('Errors to check for',
      'OCR Errors', True);
    ErrorsToCheck.eOpnDlgSubsOneLine := Ini.ReadBool('Errors to check for',
      '"- " in subtitles with one line', True);
    ErrorsToCheck.eDashOnFirstLine := Ini.ReadBool('Errors to check for',
      '"- " on first line', True); // added by adenry
    ErrorsToCheck.eOnlyIfNoDashOnSecondLine :=
      Ini.ReadBool('Errors to check for',
      'Only if there is no "- " on second line', True); // added by adenry
    ErrorsToCheck.eDialogueOnOneLine := Ini.ReadBool('Errors to check for',
      'Dialogue on one line', True); // added by adenry
    ErrorsToCheck.eSpaceAfterCustChars := Ini.ReadBool('Errors to check for',
      'Space after custom characters', True);
    ErrorsToCheck.eSpaceBeforeCustChars := Ini.ReadBool('Errors to check for',
      'Space before custom characters', False);
    ErrorsToCheck.eUnnecessarySpaces := Ini.ReadBool('Errors to check for',
      'Unnecessary spaces', True);
    ErrorsToCheck.eUnnecessaryTags := Ini.ReadBool('Errors to check for',
      'Unnecessary tags', True); // added by adenry
    ErrorsToCheck.eTooMuchCPS := Ini.ReadBool('Errors to check for',
      'Too much CPS', True); // added by adenry
    ErrorsToCheck.eWhatUnnecessarySpaces := [];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Enters and spaces at the beginning and end', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces + [EntersAndSpacesBeginningEnd];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Spaces between enters (left and right)', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces + [SpacesBetweenEnters];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Double spaces and enters', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces + [DoubleSpacesAndEnters];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Spaces in front of punctuation marks', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces + [SpacesFrontPunctuation];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Spaces after "¿" and "¡"', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces +
        [SpacesAfterQuestionAndExclamation];
    if Ini.ReadBool('Unnecessary spaces to check for',
      'Spaces before "?" and  "!"', True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces +
        [SpacesBeforeQuestionAndExclamation];
    if Ini.ReadBool('Unnecessary spaces to check for', 'Spaces between numbers',
      True) then
      ErrorsToCheck.eWhatUnnecessarySpaces :=
        ErrorsToCheck.eWhatUnnecessarySpaces + [SpacesBetweenNumbers];

    // ----------------------- //
    // Errors to fix      //
    // ----------------------- //
    ErrorsToFix.eLinesWithoutLetters := Ini.ReadBool('Errors to fix',
      'Lines without letters', True);
    ErrorsToFix.eEmptySubtitle := Ini.ReadBool('Errors to fix',
      'Empty subtitles', True);
    ErrorsToFix.eOverlapping := Ini.ReadBool('Errors to fix',
      'Overlapping subtitles', True);
    ErrorsToFix.eBadValues := Ini.ReadBool('Errors to fix', 'Bad values', True);
    ErrorsToFix.eTooShortPause := Ini.ReadBool('Errors to fix',
      'Too short pause', True); // added by adenry
    ErrorsToFix.eOverTwoLines := Ini.ReadBool('Errors to fix',
      'Subtitles over two lines', True);
    ErrorsToFix.eHearingImpaired := Ini.ReadBool('Errors to fix',
      'Hearing impaired subtitles', True);
    ErrorsToFix.eTextBeforeColon := Ini.ReadBool('Errors to fix',
      'Text before colon (":")', True);
    ErrorsToFix.eOnlyIfCapitalLetters := Ini.ReadBool('Errors to fix',
      'Only if text is in capital letters', True);
    ErrorsToFix.eUnnecessaryDots := Ini.ReadBool('Errors to fix',
      'Unnecessary dots', True);
    ErrorsToFix.eProhibitedCharacter := Ini.ReadBool('Errors to fix',
      'Prohibited characters', False);
    ErrorsToFix.eRepeatedCharacter := Ini.ReadBool('Errors to fix',
      'Repeated characters', True);
    ErrorsToFix.eRepeatedSubtitle := Ini.ReadBool('Errors to fix',
      'Repeated subtitles', True);
    ErrorsToFix.eOCRErrors := Ini.ReadBool('Errors to fix', 'OCR Errors', True);
    ErrorsToFix.eOpnDlgSubsOneLine := Ini.ReadBool('Errors to fix',
      '"-" in subtitles with one line', False);
    ErrorsToFix.eDashOnFirstLine := Ini.ReadBool('Errors to fix',
      '"- " on first line', True); // added by adenry
    ErrorsToFix.eOnlyIfNoDashOnSecondLine := Ini.ReadBool('Errors to fix',
      'Only if there is no "- " on second line', True); // added by adenry
    ErrorsToFix.eDialogueOnOneLine := Ini.ReadBool('Errors to fix',
      'Dialogue on one line', True); // added by adenry
    ErrorsToFix.eSpaceAfterCustChars := Ini.ReadBool('Errors to fix',
      'Space after custom characters', True);
    ErrorsToFix.eSpaceBeforeCustChars := Ini.ReadBool('Errors to fix',
      'Space before custom characters', False);
    ErrorsToFix.eUnnecessarySpaces := Ini.ReadBool('Errors to fix',
      'Unnecessary spaces', True);
    ErrorsToFix.eUnnecessaryTags := Ini.ReadBool('Errors to fix',
      'Unnecessary tags', True); // added by adenry
    ErrorsToFix.eTooMuchCPS := Ini.ReadBool('Errors to fix', 'Too much CPS',
      True); // added by adenry
    ErrorsToFix.eWhatUnnecessarySpaces := [];
    if Ini.ReadBool('Unnecessary spaces to fix',
      'Enters and spaces at the beginning and end', True) then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
        [EntersAndSpacesBeginningEnd];
    if Ini.ReadBool('Unnecessary spaces to fix',
      'Spaces between enters (left and right)', True) then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
        [SpacesBetweenEnters];
    if Ini.ReadBool('Unnecessary spaces to fix', 'Double spaces and enters',
      True) then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
    ...
    if Ini.ReadBool('Unnecessary spaces to fix', 'Spaces after "¿" and "¡"',
      True) then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
        [SpacesAfterQuestionAndExclamation];
    if Ini.ReadBool('Unnecessary spaces to fix', 'Spaces before "?" and  "!"',
      True) then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
        [SpacesBeforeQuestionAndExclamation];
    if Ini.ReadBool('Unnecessary spaces to fix', 'Spaces between numbers', True)
    then
      ErrorsToFix.eWhatUnnecessarySpaces := ErrorsToFix.eWhatUnnecessarySpaces +
        [SpacesBetweenNumbers];

    // added by adenry later:
    if ((ErrorsToCheck.eOCRErrors) or (ErrorsToFix.eOCRErrors)) and
      (FileExists(OCRDefFile)) then
    ...
    if Ini.ReadBool('File types', 'Register extensions on start', False) then
      AssociateExtensions(Ini.ReadString('File types', 'Associated extensions',
        ''), True);
    // added by adenry: begin
    ...
    begin
      ...
      // removed by adenry: end
      mmoSubtitleText.Alignment :=
        SetObjectAlignment(Ini.ReadInteger('Program look',
        '"Text" and "Translation" fields align', 2)); // added by adenry
      mmoTranslation.Alignment :=
        SetObjectAlignment(Ini.ReadInteger('Program look',
        '"Text" and "Translation" fields align', 2)); // added by adenry
      if TSWTextEdit.ClassName = TRichEdit.ClassName then
      // if TSWTextEdit.InheritsFrom(TRichEdit) then
      begin
        mmoSubtitleText.Paragraph.Alignment := mmoSubtitleText.Alignment;
        mmoTranslation.Paragraph.Alignment := mmoTranslation.Alignment;
      end;
      TagsHighlight := Ini.ReadBool('Program look', 'Highlight tags', True);
      // added by adenry
      TagsHighlightColor := Ini.ReadInteger('Program look',
        'Tags highlight color', pnlTagsHighlightColor.Font.Color);
      // added by adenry
      SmartResize := Ini.ReadBool('Program look', 'Smart resize', True);
      // added by adenry
      SmartResizeColumns := Ini.ReadBool('Program look', 'Smart resize columns',
        True); // added by adenry
      MiMenu.Activo := Ini.ReadBool('Menu look',
        'Use Office XP Style menu', True);
      ...
      VisualSubReprColor := Ini.ReadInteger('Video preview',
        'Visual sub repres color', pnlVisSubReprColor.Font.Color);
      // added by adenry

      if Ini.ReadBool('List look', 'Show grid lines', True) then
        lstSubtitles.TreeOptions.PaintOptions :=
          [toShowHorzGridLines, toShowVertGridLines, toShowButtons,
          toShowDropmark, toShowTreeLines, toThemeAware, toUseBlendedImages,
          toAlwaysHideSelection]
      else // ,toAlwaysHideSelection added by adenry
        lstSubtitles.TreeOptions.PaintOptions := [toShowButtons, toShowDropmark,
          toShowTreeLines, toThemeAware, toUseBlendedImages,
          toAlwaysHideSelection]; // ,toAlwaysHideSelection added by adenry
      ApplyStyleInList := Ini.ReadBool('List look',
        'Apply style to subtitles', True);
      MarkUntransSubs := Ini.ReadBool('List look',
        'Mark untranslated subtitles', True);
      UnTransSubsColor := Ini.ReadInteger('List look',
        'Untranslated subtitles color', pnlUnTransColor.Font.Color);
      // clRed changed to pnlUnTransColor.Font.Color by adenry
      MarkUntransSubsBckgr := Ini.ReadBool('List look',
        'Mark untranslated subtitles with background', True); // added by adenry
      UnTransSubsBckgr := Ini.ReadInteger('List look',
        'Untranslated subtitles background', pnlUnTransBckgr.Font.Color);
      // added by adenry
      if Ini.ReadBool('List look', 'Show horizontal scrollbar', True) = False
      then
      ...
      if Ini.ReadBool('List look', 'Right click selection', True) then
        lstSubtitles.TreeOptions.SelectionOptions :=
          lstSubtitles.TreeOptions.SelectionOptions + [toRightClickSelect]
      else
        lstSubtitles.TreeOptions.SelectionOptions :=
          lstSubtitles.TreeOptions.SelectionOptions - [toRightClickSelect];
      ...
      if Ini.ReadBool('List look', 'Disable rectangle selection', False) then
        lstSubtitles.TreeOptions.SelectionOptions :=
          lstSubtitles.TreeOptions.SelectionOptions + [toDisableDrawSelection];
      // added by adenry: end
      lstSubtitles.DefaultNodeHeight := Ini.ReadInteger('List look',
        'Row height', 18); // added by adenry
      lstSubtitles.TextMargin := Ini.ReadInteger('List look', 'Text margin', 6);
      // added by adenry

      lstSubtitles.Colors.FocusedSelectionColor :=
        Ini.ReadInteger('List look', 'Selection color', pnlSelColor.Font.Color);
      // added by adenry
      lstSubtitles.Colors.FocusedSelectionBorderColor :=
        Ini.ReadInteger('List look', 'Selection color', pnlSelColor.Font.Color);
      // added by adenry
      lstSubtitles.Colors.UnfocusedSelectionColor :=
        MixColors(lstSubtitles.Colors.FocusedSelectionColor, clWhite, 45);
      // added by adenry
      lstSubtitles.Colors.UnfocusedSelectionBorderColor :=
        MixColors(lstSubtitles.Colors.FocusedSelectionBorderColor, clWhite, 45);
      // added by adenry
      SelTransp := Ini.ReadInteger('List look', 'Selection transparency', 50);
      // added by adenry
      SelTextColor := Ini.ReadInteger('List look', 'Selection text color',
        pnlSelTextColor.Font.Color); // added by adenry

      // added by adenry: begin
      lstSubtitles.Header.Columns[0].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look', 'Num column align', 2));
      lstSubtitles.Header.Columns[1].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Show column align', 2));
      lstSubtitles.Header.Columns[2].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Hide column align', 2));
      lstSubtitles.Header.Columns[3].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Text column align', 0));
      lstSubtitles.Header.Columns[4].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Translation column align', 0));
      // added by adenry: end
      // added by adenry: begin
      lstSubtitles.Header.Columns[5].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Duration column align', 1));
      lstSubtitles.Header.Columns[6].Alignment :=
        SetObjectAlignment(Ini.ReadInteger('List look',
        'Pause column align', 1));
      ColNumDisplayStyle := Ini.ReadInteger('List look',
        'Num column display style', 1);
      ColShowDisplayStyle := Ini.ReadInteger('List look',
        'Show column display style', 0);
      ColHideDisplayStyle := Ini.ReadInteger('List look',
        'Hide column display style', 0);
      ColDurDisplayStyle := Ini.ReadInteger('List look',
        'Duration column display style', 1);
      ColPauseDisplayStyle := Ini.ReadInteger('List look',
        'Pause column display style', 1);
      // added by adenry: end
      ...
      // Marking Look          //
      // --------------------------------- //
      MarkingWithColor := Ini.ReadBool('Marking look',
        'Marking with color', True);
      MarkingColor := Ini.ReadInteger('Marking look', 'Marking color',
        pnlMarkingColor.Font.Color);
      MarkingWithBckgr := Ini.ReadBool('Marking look',
        'Marking with background', True);
      MarkingBckgr := Ini.ReadInteger('Marking look', 'Marking background',
        pnlMarkingBckgr.Font.Color);
      ...
      MarkingHighPriority := Ini.ReadBool('Marking look',
        'Marking high priority', True);
      MarkingInVideoPreview := Ini.ReadBool('Marking look',
        'Marking in video preview', True);
      MarkingInVideoPreviewColor := Ini.ReadInteger('Marking look',
        'Marking color in video preview',
        pnlMarkingColorVideoPreview.Font.Color);
      MarkingSave := Ini.ReadInteger('Marking look', 'Marking save', 0);
      MarkingAutoLoad := Ini.ReadBool('Marking look',
        'Marking auto load', True);
      // added by adenry: end
    ...
    mnuShowLeftPanel.Checked := Ini.ReadBool('Interface',
      'Show left panel', True);
    if Ini.ReadBool('Interface', 'Show time controls', True) then
      mnuShowTimeControls.Checked := True
    else
    begin
    ...
    begin
      ...
      lstSubtitles.TreeOptions.MiscOptions :=
        lstSubtitles.TreeOptions.MiscOptions + [toEditable];
    end
    else
    begin
    ...
    SetVideoPreviewMode(Ini.ReadBool('Interface', 'Video Preview Mode', True));
    // False changed to True by adenry
    // added by adenry: begin
    ...
    if mnuColNum.Checked = False then
      lstSubtitles.Header.Columns[0].Options := lstSubtitles.Header.Columns[0]
        .Options - [coVisible];
    ...
    if mnuColShow.Checked = False then
      lstSubtitles.Header.Columns[1].Options := lstSubtitles.Header.Columns[1]
        .Options - [coVisible];
    ...
    if mnuColHide.Checked = False then
      lstSubtitles.Header.Columns[2].Options := lstSubtitles.Header.Columns[2]
        .Options - [coVisible];
    ...
    if mnuColText.Checked = False then
      lstSubtitles.Header.Columns[3].Options := lstSubtitles.Header.Columns[3]
        .Options - [coVisible];
    mnuColDuration.Checked := Ini.ReadBool('Interface',
      'Col Duration visible', True);
    mnuColDurationPopup.Checked := mnuColDuration.Checked;
    if mnuColDuration.Checked = False then
      lstSubtitles.Header.Columns[5].Options := lstSubtitles.Header.Columns[5]
        .Options - [coVisible];
    ...
    if mnuColPause.Checked = False then
      lstSubtitles.Header.Columns[6].Options := lstSubtitles.Header.Columns[6]
        .Options - [coVisible];
    // added by adenry: end

    SetTranslatorMode(Ini.ReadBool('Interface', 'Translator mode',
      False), False);
    EnableVPCtrls(False);

    // added by adenry: begin
    lstSubtitles.Header.Columns[0].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 1, 1));
    lstSubtitles.Header.Columns[1].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 3, 1));
    lstSubtitles.Header.Columns[2].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 5, 1));
    lstSubtitles.Header.Columns[3].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 7, 1));
    lstSubtitles.Header.Columns[4].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 9, 1));
    lstSubtitles.Header.Columns[5].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 11, 1));
    lstSubtitles.Header.Columns[6].Position :=
      StrToInt(Copy(Ini.ReadString('Interface', 'Cols positions',
      '0,2,3,5,6,4,1'), 13, 1));
    ...
    if Ini.ReadBool('Interface', 'Show lines count', True) then
      // mnuShowLinesCount.Checked := True // Kameleon
      mnuShowLinesCount.Checked := false // Kameleon always false
    else
    begin
    ...
    // added by adenry: begin
    mnuShowNotesInLeftPanel.Checked := Ini.ReadBool('Interface',
      'Show notes in left panel', True);
    mnuShowNotesInLeftPanelClick(Sender);
    // added by adenry: end

    // added by adenry: begin
    mnuShowCPSHintBoxes.Checked := Ini.ReadBool('Interface',
      'Show CPS hint boxes', False);
    ShowCPSHintBoxes := mnuShowCPSHintBoxes.Checked;
    // added by adenry: end

    // added by adenry: begin
    mnuShowTimeControlButtons.Checked := Ini.ReadBool('Interface',
      'Show time control buttons', True);
    if mnuShowTimeControlButtons.Checked = False then
      mnuShowTimeControlButtonsClick(Sender);
    pnlTimingControls.Width := Ini.ReadInteger('Main Window',
      'Timing Controls Width', 259);
    pnlTextControls.Width := pnlEditingControls.Width - pnlTimingControls.Width
      - spSplitterEditingControls.Width;
    // we set it by hand here, as it's not automatically set when SW is maximized
    ...
    mnuVisualSubRepr.Checked := Ini.ReadBool('View',
      'Visual sub representation', True); // added later
    // added by adenry: end

    // Opened forms //
    if Ini.ReadBool('General', 'Info and errors form opened', False) then
      mnuInformationAndErrorsClick(frmMain); // added by adenry
    if Ini.ReadBool('Various info', 'Various info form opened', False) then
      mnuVariousInformationClick(frmMain); // added by adenry
    ...
    OutputSettingsFormats[0].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[0].FormatName, True);
    OutputSettingsFormats[1].FormatName := 'Advanced Subtitles';
    OutputSettingsFormats[1].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[1].FormatName, True);
    OutputSettingsFormats[2].FormatName := 'DVDSubtitle';
    OutputSettingsFormats[2].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[2].FormatName, True);
    OutputSettingsFormats[3].FormatName := 'SAMI Captioning';
    OutputSettingsFormats[3].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[3].FormatName, True);
    OutputSettingsFormats[4].FormatName := 'Sonic Scenarist';
    OutputSettingsFormats[4].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[4].FormatName, True);
    OutputSettingsFormats[5].FormatName := 'SubStation Alpha';
    OutputSettingsFormats[5].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[5].FormatName, True);
    OutputSettingsFormats[6].FormatName := 'SubViewer 1.0';
    OutputSettingsFormats[6].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[6].FormatName, True);
    OutputSettingsFormats[7].FormatName := 'SubViewer 2.0';
    OutputSettingsFormats[7].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[7].FormatName, True);
    OutputSettingsFormats[8].FormatName := 'TMPlayer';
    OutputSettingsFormats[8].AlwaysShow := Ini.ReadBool('Output Settings',
      'Always show ' + OutputSettingsFormats[8].FormatName, True);
    // added by adenry: end
  ...
  try
    ...
    // --------------------------- //
    SubtitleAPI.SetOutputSettingsAdvancedSubStationAlpha(True,
      Ini.ReadString('ASS', 'Title', '<untitled>'),
      Ini.ReadString('ASS', 'Script', '<unknown>'),
      Ini.ReadString('ASS', 'Collisions', 'Normal'),
      Ini.ReadInteger('ASS', 'PlayResX', 0), Ini.ReadInteger('ASS', 'PlayResY',
      0), Ini.ReadString('ASS', 'Timer', '100.0000'),
      Ini.ReadString('ASS', 'Font name', 'Tahoma'),
      Ini.ReadInteger('ASS', 'Font size', 24), Ini.ReadBool('ASS', 'Bold',
      True), Ini.ReadBool('ASS', 'Italic', False),
      ...
      Ini.ReadInteger('ASS', 'Outline', 1), Ini.ReadInteger('ASS', 'Shadow', 1),
      (Ini.ReadInteger('ASS', 'Alignment', 2) or (Ini.ReadInteger('ASS',
      'Type of subtitles', 1) shl 2)), Ini.ReadInteger('ASS', 'Left margin',
      30), Ini.ReadInteger('ASS', 'Right margin', 30),
      Ini.ReadInteger('ASS', 'Vertical margin', 415),
      Ini.ReadInteger('ASS', 'Encoding', 1), Ini.ReadBool('ASS', 'Underline',
      False), Ini.ReadBool('ASS', 'StrikeOut', False),
      Ini.ReadInteger('ASS', 'ScaleX', 100), Ini.ReadInteger('ASS', 'ScaleY',
      100), Ini.ReadInteger('ASS', 'Spacing', 0), Ini.ReadString('ASS', 'Angle',
      '0.00'));
    // added by adenry: end
    ...
    SubtitleAPI.SetOutputSettingsXAS(True, Ini.ReadString('XAS', 'Font Name',
      'arialbd.ttf'), Ini.ReadInteger('XAS', 'Font Size', 60),
      Ini.ReadBool('XAS', 'Font Size In Percent', False),
      Ini.ReadInteger('XAS', 'Text Color', clWhite),
      Ini.ReadBool('XAS', 'Shadow', True), Ini.ReadInteger('XAS',
      'Shadow Color', clBlack), Ini.ReadInteger('XAS', 'Transparency', 255),
      // Position
      Ini.ReadInteger('XAS', 'X', 10), Ini.ReadInteger('XAS', 'Y', 89),
      Ini.ReadInteger('XAS', 'Width', 80), Ini.ReadInteger('XAS', 'Height', 11),
    ...
    SubtitleAPI.SetOutputSettingsDVDSubtitle(True,
      ...
      Ini.ReadString('DVD Subtitle', 'Web', ''), Ini.ReadString('DVD Subtitle',
      'Info', ''), Ini.ReadString('DVD Subtitle', 'License', ''));
    ...
    if Ini.ReadString('SAMI', 'Align', 'Center') = 'Left' then
      TextAlign := taLeft
    else if Ini.ReadString('SAMI', 'Align', 'Center') = 'Right' then
      TextAlign := taRight
    else
      TextAlign := taCenter;
    SubtitleAPI.SetOutputSettingsSAMI(True, Ini.ReadString('SAMI', 'Font',
      'Tahoma'), Ini.ReadInteger('SAMI', 'Size', 24),
      Ini.ReadBool('SAMI', 'Bold', True), Ini.ReadBool('SAMI', 'Italic', False),
      Ini.ReadBool('SAMI', 'Underline', False),
      Ini.ReadInteger('SAMI', 'Subtitle color', clWhite),
      Ini.ReadInteger('SAMI', 'Background color', clBlack), TextAlign);

    // ------------------------ //
    // Sonic Scenarist     //
    // ------------------------ //
    SubtitleAPI.SetOutputSettingsSonicScenarist(True,
      Ini.ReadBool('Scenarist', 'PAL', True), Ini.ReadBool('Scenarist',
      'Drop frame', False), Ini.ReadInteger('Scenarist', 'Color0', 3),
      Ini.ReadInteger('Scenarist', 'Color1', 4), Ini.ReadInteger('Scenarist',
      'Color2', 3), Ini.ReadInteger('Scenarist', 'Color3', 9),
      Ini.ReadInteger('Scenarist', 'Contrast0', 0), Ini.ReadInteger('Scenarist',
      'Contrast1', 15), Ini.ReadInteger('Scenarist', 'Contrast2', 15),
      Ini.ReadInteger('Scenarist', 'Contrast3', 15));

    // ------------------------ //
    // SubViewer        //
    // ------------------------ //
    SubtitleAPI.SetOutputSettingsSubViewer1(True, Ini.ReadString('SubViewer',
      'Title', ''), Ini.ReadString('SubViewer', 'Author', ''),
      Ini.ReadString('SubViewer', 'Source', ''), Ini.ReadString('SubViewer',
      'Program', ''), Ini.ReadString('SubViewer', 'Path', ''),
      Ini.ReadInteger('SubViewer', 'Delay', 0));

    // ------------------------ //
    // SubViewer 2       //
    // ------------------------ //
    SubtitleAPI.SetOutputSettingsSubViewer2(True, Ini.ReadString('SubViewer 2',
      'Title', ''), Ini.ReadString('SubViewer 2', 'Author', ''),
      Ini.ReadString('SubViewer 2', 'Source', ''), Ini.ReadString('SubViewer 2',
      'Program', ''), Ini.ReadString('SubViewer 2', 'Path', ''),
      Ini.ReadInteger('SubViewer 2', 'Delay', 0), Ini.ReadInteger('SubViewer 2',
      'CD-Track', 0), Ini.ReadString('SubViewer 2', 'Comment', ''),
      ...
      Ini.ReadBool('SubViewer 2', 'Bold', True), Ini.ReadBool('SubViewer 2',
      'Italic', False), Ini.ReadBool('SubViewer 2', 'Underline', False),
    ...
    SubtitleAPI.SetOutputSettingsSubStationAlpha(True,
      ...
      Ini.ReadInteger('SSA', 'Font size', 24), Ini.ReadBool('SSA', 'Bold',
      True), Ini.ReadBool('SSA', 'Italic', False),
      ...
      Ini.ReadInteger('SSA', 'Outline', 1), Ini.ReadInteger('SSA', 'Shadow', 1),
      (Ini.ReadInteger('SSA', 'Alignment', 2) or (Ini.ReadInteger('SSA',
      'Type of subtitles', 1) shl 2)), Ini.ReadInteger('SSA', 'Left margin',
      30), Ini.ReadInteger('SSA', 'Right margin', 30),
...
const GlobalPercentage = 70; // percentage the control panel has in height

// resize main form (and smart resize of subtitle list columns)
procedure TfrmMain.FormResize(Sender: TObject);
var
  scrollWidth: Integer; // added by adenry
  T, H: integer;// Kameleon
begin

  // added by Kameleon
  // begin
  H := Self.Height * 15 div 100; // 15%
  if H < 86 then H := 86;        // with a minimum of 86
  PnlEditingControls.Height := H;

  if mnuStatusBar.Checked
  then T := sbStatusBar.top - H - 5
  else T := Self.ClientHeight - H - 5;
  PnlEditingControls.Top := T;

  tmeShow.Top := H * 22 div 100;   // 22%
  tmeHide.Top := H * 22 div 100;   // 22%
  tmePause.Top :=  H * 74 div 100;    // 74%
  tmeDuration.Top := H * 74 div 100;  // 74%

  tbShowSnap.Top := (H * 22 div 100) + 5;   // 22% + 5
  tbHideSnap.Top := (H * 22 div 100) + 5;   // 22 % + 5
  tbPauseSet.Top :=  (H * 74 div 100) + 5;  // 74% + 5
  tbDurationSet.Top := (H * 74 div 100) + 5; // 74% + 5

  lblShow.Top :=  H * 5 div 100;    // 5%
  lblHide.Top :=  H * 5 div 100;    // 5%
  lblPause.Top :=  H * 56 div 100;  // 56%
  lblDuration.Top := H * 56 div 100;  // 56%

  PnlControl.Width := Self.ClientWidth * 12 div 100; // 12%  // Kameleon

  lblMode.Top          := pnlControl.Height * 174 * GlobalPercentage div 1000000;// 1.74%

  pnlMode.Top          := pnlControl.Height * 575 * GlobalPercentage div 1000000;// 5.75%
  pnlMode.height 			 := pnlControl.Height * 893 * GlobalPercentage div 1000000;// 8.93%
  pnlMode.Width        := pnlControl.Width;

  rdoModeTime.Top      := 5;
  rdoModeTime.Height   := pnlMode.Height div 3;
  rdoModeTime.width    := pnlMode.Width;

  rdoModeFrames.Top    := (pnlMode.Height div 2) + 5;
  rdoModeFrames.Height := pnlMode.Height div 3;
  rdoModeFrames.width  := pnlMode.Width;

  lblInputFPS.Top 		 := pnlControl.Height * 165 * GlobalPercentage div 100000; // 16,8%
  cmbInputFPS.Top 		 := pnlControl.Height * 205 * GlobalPercentage div 100000; // 20.2%
  cmbInputFPS.Width    := pnlControl.Width - (2 * cmbInputFPS.Left);

  lblFPS.Top 				   := pnlControl.Height * 260 * GlobalPercentage div 100000; // 26.0%
  cmbFPS.Top 				   := pnlControl.Height * 300 * GlobalPercentage div 100000; // 29,6%
  cmbFPS.Width 			   := pnlControl.Width - (2 * cmbFPS.Left);

  lblWorkWith.Top 	   := pnlControl.Height * 38 * GlobalPercentage div 10000; // 38%
  rdoDuration.Top      := pnlControl.Height * 423 * GlobalPercentage div 100000; // 42,3%
  rdoDuration.Width    := pnlControl.Width - (2 * rdoDuration.Left);
  rdoFinalTime.Top 	   := pnlControl.Height * 473 * GlobalPercentage div 100000; // 47.3%
  rdoFinalTime.Width   := pnlControl.Width - (2 * rdoFinalTime.Left);
  rdoBoth.Top 			   := pnlControl.Height * 523 * GlobalPercentage div 100000; // 52.3%
  rdoBoth.Width 		   := pnlControl.Width - (2 * rdoBoth.Left);

  cmbOrgCharset.Top    := pnlControl.Height * 607 * GlobalPercentage div 100000; // GlobalPercentage.7%
  cmbOrgCharset.Width  := cmbFPS.Width;

  cmbTransCharSet.Top  := pnlControl.Height * 667 * GlobalPercentage div 100000; // 66.7%
  cmbTransCharset.Width  := cmbOrgCharset.Width;

  chkOCRScripts.Top    := pnlControl.Height * 773 * GlobalPercentage div 100000; // 77.3%
  chkOCRScripts.Width  := pnlControl.Width - (2 * chkOCRScripts.Left);
  chkOCRScripts.Height := pnlControl.Height * 460 * GlobalPercentage div 1000000; // 4,6%

  cmbOCRScripts.Top    := pnlControl.Height * 823 * GlobalPercentage div 100000; // 82.3%
  cmbOCRScripts.Width  := pnlControl.Width - (2 * cmbOCRScripts.Left);

  //lblNotes.Top :=  see routine
  //mmoNotes.Top :=  see routine
  mmoNotes.Width       := pnlControl.Width - 6;
  mmoNotes.Left := 3;

  mnuShowInMainFormClick(Sender);  // adapt the place of "notes"

  // added by Kameleon end

  pnlParent2.Height := pnlEditingControls.Top - pnlParent2.Top - 1;
  // pnlParent2.Height := lblText.Top - pnlParent2.Top - 7; //added by adenry: fixes a bug appearing in win7 for unknown reasons (the subtitiles list expands all the way to the bottom and hides the text and time controls)
  ...
  begin
    scrollWidth := GetSystemMetrics(SM_CXVSCROLL) + 2 *
      ((2 * Integer(lstSubtitles.BorderStyle)) + lstSubtitles.BorderWidth);
    if mnuTranslatorMode.Checked then
    begin
      lstSubtitles.Header.Columns[3].Width :=
        Rnd((lstSubtitles.Width - scrollWidth - lstSubtitles.Header.Columns[0]
        .Width * Integer(mnuColNum.Checked) - lstSubtitles.Header.Columns[1]
        .Width * Integer(mnuColShow.Checked) - lstSubtitles.Header.Columns[2]
        .Width * Integer(mnuColHide.Checked) - lstSubtitles.Header.Columns[5]
        .Width * Integer(mnuColDuration.Checked) - lstSubtitles.Header.Columns
        [6].Width * Integer(mnuColPause.Checked)) * TextTransColsRatio);
      // was div 2
      lstSubtitles.Header.Columns[4].Width := lstSubtitles.Width - scrollWidth -
    ...
    begin
      lstSubtitles.Header.Columns[3].Width := lstSubtitles.Width - scrollWidth -
        lstSubtitles.Header.Columns[0].Width * Integer(mnuColNum.Checked) -
        lstSubtitles.Header.Columns[1].Width * Integer(mnuColShow.Checked) -
        lstSubtitles.Header.Columns[2].Width * Integer(mnuColHide.Checked) -
        lstSubtitles.Header.Columns[5].Width * Integer(mnuColDuration.Checked) -
  ...
  mnuShowLinesCount.Visible := false; // Kameleon
end;
...
begin
  ...
  Ini := TMemIniFile.Create(IniRoot);
  // TIniFile replaced with TMemIniFile by adenry
  try
    // Save current language...
    Ini.WriteString('Language', 'Language',
      Copy(ExtractFileName(ActualLangFile), 0,
      Length(ExtractFileName(ActualLangFile)) - 4));
    // Save recent files...
    SaveRecentFiles(Ini);
    // Free recent files TStringList...
    RecentFiles.Free;
    // Current OCR Script...
    Ini.WriteString('Information and Errors', 'OCR Definitions file',
      OCRDefFile);
    Ini.WriteBool('General', 'Show OCR Scripts in main form',
      mnuShowInMainForm.Checked);
    Ini.WriteBool('General', 'Enable OCR Scripts', chkOCRScripts.Checked);
    // added by adenry

    // Work with...
    if rdoDuration.Checked then
      Ini.WriteInteger('General', 'Work with', 1)
    else if rdoFinalTime.Checked then
      Ini.WriteInteger('General', 'Work with', 2)
    else if rdoBoth.Checked then
      Ini.WriteInteger('General', 'Work with', 3);

    // Save last folder
    //Ini.WriteString('General', 'Last folder', dlgLoadFile.InitialDir);              // Kameleon
    if dlgLoadFile.FileName > ''  // Kameleon
    then Ini.WriteString('General', 'Last folder', ExtractFilePath(dlgLoadFile.FileName)); // Kameleon

    // Charsets...
    Ini.WriteInteger('General', 'Original charset', cmbOrgCharset.ItemIndex);
    Ini.WriteInteger('General', 'Translated charset',
      cmbTransCharset.ItemIndex);

    // Notes content:
    Ini.WriteString('General', 'Notes', ReplaceString(mmoNotes.Text, #13#10,
      '|')); // added by adenry
    ...
    begin
      ...
      Ini.WriteInteger('Columns width (normal mode)', '1',
        lstSubtitles.Header.Columns[0].Width);
      Ini.WriteInteger('Columns width (normal mode)', '2',
        lstSubtitles.Header.Columns[1].Width);
      Ini.WriteInteger('Columns width (normal mode)', '3',
        lstSubtitles.Header.Columns[2].Width);
      if not FullScreen then // added by adenry
        Ini.WriteInteger('Columns width (normal mode)', '4',
          lstSubtitles.Header.Columns[3].Width)
      else
        Ini.WriteInteger('Columns width (normal mode)', '4', ColTextWidth);
      // added by adenry
      Ini.WriteInteger('Columns width (normal mode)', '6',
        lstSubtitles.Header.Columns[5].Width); // added by adenry
      Ini.WriteInteger('Columns width (normal mode)', '7',
        lstSubtitles.Header.Columns[6].Width); // added by adenry
    end
    else
    begin
      // Columns width for translator mode
      Ini.WriteInteger('Columns width (translator mode)', '1',
        lstSubtitles.Header.Columns[0].Width);
      Ini.WriteInteger('Columns width (translator mode)', '2',
        lstSubtitles.Header.Columns[1].Width);
      Ini.WriteInteger('Columns width (translator mode)', '3',
        lstSubtitles.Header.Columns[2].Width);
      if not FullScreen then // added by adenry
      begin
        Ini.WriteInteger('Columns width (translator mode)', '4',
          lstSubtitles.Header.Columns[3].Width);
        Ini.WriteInteger('Columns width (translator mode)', '5',
      ...
      Ini.WriteInteger('Columns width (translator mode)', '6',
        lstSubtitles.Header.Columns[5].Width); // added by adenry
      Ini.WriteInteger('Columns width (translator mode)', '7',
        lstSubtitles.Header.Columns[6].Width); // added by adenry
    end;
    ...
    // added by adenry: end
    Ini.WriteFloat('Main Window', 'VPRatio', VideoPanelRatio);
    // added by adenry
    Ini.WriteInteger('Main Window', 'Timing Controls Width',
      pnlTimingControls.Width); // added by adenry

    // ----------------- //
    // Video preview   //
    // ----------------- //
    Ini.WriteInteger('Video preview', 'Video panel height', pnlVideo.Height);
    if mnu10P.Checked then
      A := 1
    else if mnu20P.Checked then
      A := 2
    else if mnu30P.Checked then
      A := 3
    else if mnu40P.Checked then
      A := 4
    else if mnu50P.Checked then
      A := 5
    else if mnu60P.Checked then
      A := 6
    else if mnu70P.Checked then
      A := 7
    else if mnu80P.Checked then
      A := 8
    else if mnu90P.Checked then
      A := 9
    else if mnu200P.Checked then
      A := 10
    else if mnu300P.Checked then
      A := 11
    else if mnu400P.Checked then
      A := 12
    else
      A := 0;
    Ini.WriteInteger('Video preview', 'Playback rate', A);
    if FullScreen and not ScrollListState then
      ScrollList := False; // added by adenry
    Ini.WriteBool('Video preview', 'Scroll list', ScrollList);
    Ini.WriteBool('Video preview', 'Displaying original',
      mnuDisplayOriginal.Checked);
    Ini.WriteBool('Video preview', 'Show subtitles', mnuShowSubtitles.Checked);
    Ini.WriteInteger('Video preview', 'Volume', sbVolume.Position);
    // added by adenry
    Ini.WriteFloat('Video preview subtitles', 'Subtitle top shift',
      subSubtitleTopShift); // added by adenry
    Ini.WriteBool('Video preview', 'Use VMR9', mnuVMR9VidRend.Checked);
    // added by adenry
    Ini.WriteFloat('Video preview', 'Video Aspect Ratio', VideoAspectRatio);
    // added by adenry
    ...
    // added by adenry: end
    Ini.WriteBool('Interface', 'Video Preview Mode',
      mnuVideoPreviewMode.Checked);
    Ini.WriteBool('Interface', 'Show left panel', mnuShowLeftPanel.Checked);
    Ini.WriteBool('Interface', 'Show time controls',
      mnuShowTimeControls.Checked);
    Ini.WriteBool('Interface', 'Use in-place edition',
      mnuUseInPlaceEdition.Checked);
    Ini.WriteBool('Interface', 'Show lines count', mnuShowLinesCount.Checked);
    // added by adenry
    Ini.WriteBool('Interface', 'Show notes in left panel',
      mnuShowNotesInLeftPanel.Checked); // added by adenry
    Ini.WriteBool('Interface', 'Show CPS hint boxes',
      mnuShowCPSHintBoxes.Checked); // added by adenry
    Ini.WriteBool('Interface', 'Show time control buttons',
      mnuShowTimeControlButtons.Checked); // added by adenry
    ...
    Ini.WriteBool('View', 'Visual sub representation',
      mnuVisualSubRepr.Checked);
    // ---------------- //
...
begin
  ...
  mnuSaveOriginalAs.Enabled := (lstSubtitles.RootNodeCount > 0) and
    (mnuTranslatorMode.Checked);
  mnuSaveTranslatedAs.Enabled := (lstSubtitles.RootNodeCount > 0) and
    (mnuTranslatorMode.Checked);
...
begin
  ...
  mnuInsertSymbol.Enabled := (ActiveControl = mmoSubtitleText) or
    (ActiveControl = mmoTranslation) or (ActiveControl = mmoNotes);
  // removed by adenry: begin
  ...
  // and (InterfaceEnabled) and (lstSubtitles.Focused or mmoSubtitleText.Focused or mmoTranslation.Focused or tmeShow.Focused or tmeHide.Focused or tmeDuration.Focused or tmePause.Focused or mmoNotes.Focused); //all Focused checks added by adenry
  ...
  mnuDivideLines.Enabled := (lstSubtitles.RootNodeCount > 0) and
    (lstSubtitles.SelectedCount = 1) and (Assigned(lstSubtitles.FocusedNode))
    and ((Length(RemoveSWTags(GetSubText(lstSubtitles.FocusedNode), True, True,
    True, True)) > BreakLineAfter) or
    (Pos(#13#10, GetSubText(lstSubtitles.FocusedNode)) > 0));
  mnuFastDivideLines.Enabled := mnuDivideLines.Enabled or
    (lstSubtitles.RootNodeCount > 0) and (lstSubtitles.SelectedCount > 1);
  // or.... added by adenry
  ...
  mnuExtendLength.Enabled := (lstSubtitles.SelectedCount > 0) and
    ((lstSubtitles.SelectedCount > 1) or (lstSubtitles.GetFirstSelected <>
    lstSubtitles.GetLast));
  mnuExtendLength2.Enabled := (lstSubtitles.SelectedCount > 0) and
    ((lstSubtitles.SelectedCount > 1) or (lstSubtitles.GetFirstSelected <>
    lstSubtitles.GetFirst));
  mnuShiftToNext.Enabled := (lstSubtitles.SelectedCount > 0) and
    (GetLastSelected <> lstSubtitles.GetLast);
  mnuShiftToPrevious.Enabled := (lstSubtitles.SelectedCount > 0) and
    (lstSubtitles.GetFirstSelected <> lstSubtitles.GetFirst);
...
begin
  ...
  mnuFindNext.Enabled := (lstSubtitles.RootNodeCount > 0) and
    (SearchWord <> '');
...
begin
  ...
  mnuRecheckErrors.Enabled := (lstSubtitles.RootNodeCount > 0) and
    (AdvancedErrorMarking);
  // MarkErrorsInList replaced with AdvancedErrorMarking by adenry
  ...
  mnuJumpToPrevError.Enabled := lstSubtitles.RootNodeCount > 0;
  // added by adenry
...
begin
  ...
  // added by adenry
  if (Column in [1 .. 6]) then
  // 4 changed to 6 so that Duration and Pause could also be edited "in-place"
    lstSubtitles.Header.MainColumn := Column;
  // so that the columns could be edited "in-place" - bug fix!
  ...
  // node changed, don't bind next text change undo
  tmrTimesMouseWheelUndoBind.Enabled := False;
  // node changed, don't bind next time change undo
end;
...
begin
  ...
  // added by adenry to refresh time and text controls after key down
  case Key of
    ...
    VK_NEXT, VK_PRIOR:
      if lstSubtitles.TotalCount > 0 then
        ...
        begin
          ...
          VisibleNodesCount :=
            Ceil(lstSubtitles.ClientHeight / lstSubtitles.DefaultNodeHeight);
    ...
    VK_SPACE: // Mark/Unmark selected subtitles
      begin
        ...
        begin
          ...
          // if even one subtitle is not marked, mark all
          begin
    ...
    Ord('S'):
      begin
        if PressedKeyInLstSubtitles = 0 then
          PressedKeyInLstSubtitles := 1;
    ...
    VK_RETURN:
      begin
        ...
        if (lstSubtitles.SelectedCount = 1) then
          lstSubtitles.EditNode(lstSubtitles.FocusedNode,
            lstSubtitles.FocusedColumn);
      end;
    VK_LEFT:
      lstSubtitles.FocusedColumn := GetTreePrevColumn(lstSubtitles,
        lstSubtitles.FocusedColumn);
    VK_RIGHT:
      lstSubtitles.FocusedColumn := GetTreeNextColumn(lstSubtitles,
        lstSubtitles.FocusedColumn);
...
begin
  ...
  begin
    ...
    if LoadMovie(dlgLoadFile.FileName) = False then
      MsgBox(Format(ErrorMsg[05], [dlgLoadFile.FileName]), BTN_OK, '', '',
        MB_ICONERROR, frmMain)
    else
    begin
      if mnuVideoPreviewMode.Checked = False then
  ...
  dlgLoadFile.Filter := SubtitleAPI.FillDialogFilter(AllSupportedFiles) + ID_SRF
    + '|' + ID_PLAINTEXT;
end;
...
begin
  if FullScreen then
    ToggleFullScreen; // added by adenry
  FreeFile;
end;
...
begin
  ...
  begin
    ...
    if VideoDuration > 0 then
      if Playing then
      // added by adenry - when not playing we need the seekbar and timecounter unchanged because of the video preview when paused. Also, we don't want the video to jump to the beginning when seeked to the end of the video when paused
      begin
...
begin
  ...
  begin
    ...
    else if btnForward.State = sPressed then
      SetVideoPos(GetCurrentPos + RewFFTime);
  end;
end;
...
procedure TfrmMain.btnForwardMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  SetVideoPos(GetCurrentPos + RewFFTime);
  Seeking := True;
...
begin
  ...
  // ------
  mnuSynchronization.Enabled := (Player.Initialized) and
    (lstSubtitles.RootNodeCount > 0);
  mnuFirstSyncPoint.Enabled := (Player.Initialized) and
    (lstSubtitles.RootNodeCount > 0);
  mnuLastSyncPoint.Enabled := (Player.Initialized) and
    (lstSubtitles.RootNodeCount > 0);
  mnuAddSyncPoint.Enabled := (Player.Initialized) and
    (lstSubtitles.RootNodeCount > 0);
  // ------
...
begin
  ...
  *)
  ...
  // Kameleon necessary to resize the mainmenu

  frmMain.mnuFile.Visible := not frmMain.mnuFile.Visible;
  frmMain.mnuFile.Visible := not frmMain.mnuFile.Visible; // Kameleon

  tmp := TMenuItem.Create(mnuMain); // Kameleon
  tmp.Caption := 'Test';

  mnuFile.Insert(0, tmp); // Kameleon  Force all menuitem to be redrawn
  mnuFile.Delete(0);

  mnuEdit.Insert(0, tmp); // Kameleon
  mnuEdit.Delete(0);

  mnuView.Insert(0, tmp); // Kameleon
  mnuView.Delete(0);

  mnuSearch.Insert(0, tmp); // Kameleon
  mnuSearch.Delete(0);

  mnuTools.Insert(0, tmp); // Kameleon
  mnuTools.Delete(0);

  mnuMovie.Insert(0, tmp); // Kameleon
  mnuMovie.Delete(0);

  mnuSettings.Insert(0, tmp); // Kameleon
  mnuSettings.Delete(0);

  mnuHelp.Insert(0, tmp); // Kameleon
  mnuHelp.Delete(0);

  mnuInfoErrors.Insert(0, tmp);  // Kameleon
  mnuInfoErrors.Delete(0);

  mnuSubtitleToDisplay.Insert(0, tmp);  // Kameleon
  mnuSubtitleToDisplay.Delete(0);

  mnuPlayback.Insert(0, tmp); // Kameleon
  mnuPlayback.Delete(0);

  mnuPlaybackRate.Insert(0, tmp); // Kameleon
  mnuPlaybackRate.Delete(0);

  mnuVideoRenderer.Insert(0, tmp); // Kameleon
  mnuVideoRenderer.Delete(0);

  mnuVidSubtitles.Insert(0, tmp); // Kameleon
  mnuVidSubtitles.Delete(0);

  mnuSynchronization.Insert(0, tmp); // Kameleon
  mnuSynchronization.Delete(0);

  mnuSaveMediaStartupFile.Insert(0, tmp); // Kameleon
  mnuSaveMediaStartupFile.Delete(0);

  mnuLanguage.Insert(0, tmp); // Kameleon
  mnuLanguage.Delete(0);
...
procedure TfrmMain.tmeShowTimeChangeFromEditOnly(Sender: TObject;
  NewTime: Cardinal);
var
...
begin
  ...
  begin
    ...
    ChangeShowTime(Node, NewTime, Assigned(NextSelNode));
    // most of tmeShowTimeChangeFromEditOnly went in ChangeShowTime
    Node := NextSelNode;
  end;
  // added by adenry: end
end;
...
// added by adenry: begin
procedure TfrmMain.ChangeShowTime(Node: PVirtualNode; NewShowTime: Cardinal;
  BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
  Shift: Boolean = False; UndoActionName: TUndoActionName = uanShowEdit);
var
...
begin
  ...
  if NewShowTime > MAX_TIME then
    NewShowTime := 0; // added later
  ...
  begin
    ...
    if BindUndoToPrev and (UndoList.Count > 0) then
      if PUndoAction(UndoList.Items[UndoList.Count - 1])^.UndoActionName = uanShowEdit
      then // don't bind to other time modifications
    ...
    // Shift subtitle - change both SHOW and HIDE values with the same amount
    begin
      PTimeChange(UndoAction^.Buffer)^.FinalTime := HideTime;
      SetFinalTime(Node, NewShowTime + (HideTime - ShowTime));
      // lstSubtitles.FocusedNode; //modified by adenry
      ...
      begin
        if ((HideTime - NewShowTime < KeepMinDurValue) OR
          (HideTime < NewShowTime)) then
        begin
    ...
    // added by adenry //lstSubtitles.FocusedNode; //modified by adenry
...
procedure TfrmMain.tmeHideTimeChangeFromEditOnly(Sender: TObject;
  NewTime: Cardinal);
var
  Node, NextSelNode: PVirtualNode; // added by adenry
begin
  ...
  begin
    NextSelNode := lstSubtitles.GetNextSelected(Node);
    ChangeHideTime(Node, NewTime, Assigned(NextSelNode));
    // most of tmeHideTimeChangeFromEditOnly went in this procedure
    Node := NextSelNode;
  end;
  // added by adenry: end
end;
...
// added by adenry: begin
procedure TfrmMain.ChangeHideTime(Node: PVirtualNode; NewHideTime: Cardinal;
  BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
  Shift: Boolean = False; UndoActionName: TUndoActionName = uanHideEdit);
var
  UndoAction: PUndoAction;
...
begin
  ShowTime := GetStartTime(Node);
  HideTime := GetFinalTime(Node);
  if NewHideTime > MAX_TIME then
    NewHideTime := 0; // added later
  if NewHideTime <> HideTime then
  begin
    // added by adenry: bind to previous when fast scrolling
    if BindUndoToPrev and (UndoList.Count > 0) then
      if PUndoAction(UndoList.Items[UndoList.Count - 1])^.UndoActionName = uanHideEdit
      then // don't bind to other time modifications
        PUndoAction(UndoList.Items[UndoList.Count - 1])^.BindToNext := True;
    ...
    begin
      PTimeChange(UndoAction^.Buffer)^.StartTime := ShowTime;
      // lstSubtitles.FocusedNode; //modified by adenry
      SetStartTime(Node, NewHideTime - (HideTime - ShowTime));
      // lstSubtitles.FocusedNode; //modified by adenry
    end
    else
      // keep minimum duration
      if KeepMinDur and (lstSubtitles.SelectedCount = 1) then
      begin
        if ((NewHideTime - ShowTime < KeepMinDurValue) OR
          (NewHideTime < ShowTime)) then
        begin
    ...
    if BindUndoToNext = False then // added by adenry later
      RefreshTimes;
    AutoRecheckNodeErrors(Node, TimeErrors);
    // added by adenry //lstSubtitles.FocusedNode;
...
procedure TfrmMain.tmeDurationTimeChangeFromEditOnly(Sender: TObject;
  NewTime: Cardinal);
var
  Node, NextSelNode: PVirtualNode; // added by adenry
begin
  ...
  begin
    NextSelNode := lstSubtitles.GetNextSelected(Node);
    ChangeDuration(Node, NewTime, Assigned(NextSelNode));
    // most of tmeDurationTimeChangeFromEditOnly went in ChangeDuration
    Node := NextSelNode;
  end;
  // added by adenry: end
end;
...
procedure TfrmMain.ChangeDuration(Node: PVirtualNode; NewDuration: Cardinal;
  BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False;
  Shift: Boolean = False);
var
  UndoAction: PUndoAction;
...
begin
  ...
  // added by adenry //was lstSubtitles.FocusedNode
  durationIncrease := NewDuration - (GetFinalTime(Node) - oldStartTime);
  // added by adenry //was lstSubtitles.FocusedNode
  ...
  begin
    // added by adenry: bind to previous when fast scrolling
    if BindUndoToPrev and (UndoList.Count > 0) then
      if PUndoAction(UndoList.Items[UndoList.Count - 1])^.UndoActionName = uanDurEdit
      then // don't bind to other time modifications
        PUndoAction(UndoList.Items[UndoList.Count - 1])^.BindToNext := True;
    ...
    begin
      ...
      SetStartTime(Node, oldStartTime + durationIncrease);
      // lstSubtitles.FocusedNode //modified by adenry
    end;
    // added by adenry: end
    AddUndo(UndoAction);

    SetFinalTime(Node, Cardinal(oldStartTime) + NewDuration);
    // added by adenry //lstSubtitles.FocusedNode
...
// added by adenry: begin
// Set subtitle pause
procedure TfrmMain.tmePauseTimeChangeFromEditOnly(Sender: TObject;
  NewTime: Cardinal);
var
  Node, NextSelNode: PVirtualNode; // added by adenry
begin
  ...
  begin
    NextSelNode := lstSubtitles.GetNextSelected(Node);
    ChangePause(Node, NewTime, Assigned(NextSelNode));
    // most of tmePauseTimeChangeFromEditOnly went in ChangePause
    Node := NextSelNode;
  end;
  // added by adenry: end
end;
...
// added by adenry: begin
procedure TfrmMain.ChangePause(Node: PVirtualNode; NewPause: Cardinal;
  BindUndoToNext: Boolean = False; BindUndoToPrev: Boolean = False);
var
  UndoAction: PUndoAction;
...
begin
  if Node <> lstSubtitles.GetFirst then
    ...
    begin
      // added by adenry: bind to previous when fast scrolling
      if BindUndoToPrev and (UndoList.Count > 0) then
        if PUndoAction(UndoList.Items[UndoList.Count - 1])^.UndoActionName = uanPauseEdit
        then // don't bind to other time modifications
          PUndoAction(UndoList.Items[UndoList.Count - 1])^.BindToNext := True;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (KeepMinDurDontOverlap) and (Node <> lstSubtitles.GetLast) then
            begin
              if GetStartTime(Node.NextSibling) - NewHideTime < TooShortPause
              then
              begin
                NewHideTime := GetStartTime(Node.NextSibling) - TooShortPause;
                if NewHideTime < HideTime then
...
begin
  ...
  try
    ...
    begin
      ...
      // We save original file in it's original format
      begin
        UTF8_Orig := SubtitleAPI.Get_UTF8_File;
        OrgModified := False;
    ...
    begin
      ...
      if SaveFile(TransFile, TransFormat, GetFPS) then
      // We save translated file in it's original format
      begin
        UTF8_Trans := SubtitleAPI.Get_UTF8_File;
        TransModified := False;
    ...
    else // If file doesn't exist then, save as...
    begin
    ...
    begin
      dlgSaveProject.Filter := ID_STPROJECT + ' (*' + ID_STPEXT + ')|*' +
        ID_STPEXT;
      ...
      begin
        ...
        if (ExtractFilePath(OrgFile) = ExtractFilePath(TransFile)) and
          (ExtractFilePath(OrgFile) = ExtractFilePath(dlgSaveProject.FileName))
        then
        begin
        ...
        if (ExtractFilePath(dlgSaveProject.FileName))
          = (ExtractFilePath(MovieFile)) then
          MMovieFile := ExtractFileName(MovieFile)
        else
        ...
        try
          ...
          Ini.WriteInteger('Other', 'Focused node',
            lstSubtitles.FocusedNode.Index);
...
begin
  ...
  begin
    ...
    pnlParent2.Width := frmMain.ClientWidth -
      (pnlControl.Left + pnlControl.Width) - 15;
  end
  else
  begin
  ...
  lblText.Left := mmoSubtitleText.Left +
    (Integer(tbTextStyle.Visible) * tbTextStyle.Width);
  lblTranslation.Top := lblText.Top;
  lblTranslation.Left := mmoTranslation.Left +
    (Integer(tbTransStyle.Visible) * tbTransStyle.Width);
...
begin
  ...
  begin
    if (dlgLoadFile.FilterIndex) <= (SubtitleAPI.FormatsCount + 1) then
      LoadSubtitle(dlgLoadFile.FileName, GetInputFPS,
        dlgLoadFile.FilterIndex, True)
    else
    begin
      if (dlgLoadFile.FilterIndex) = (SubtitleAPI.FormatsCount + 2) then
        LoadSRF(dlgLoadFile.FileName, True)
      else
  ...
  UTF8_Trans := SubtitleAPI.Get_UTF8_File; // Kameleon
  RefreshStatusbar;
end;
...
begin
  NewString := GetSWTextEditText(mmoSubtitleText);
  // added by adenry - properly convert the Unicode text of the RichEdit to ANSI - WITH THE PROPER ENCODING!
  IF (lstSubtitles.SelectedCount = 1) and
    (NewString <> GetSubText(lstSubtitles.FocusedNode)) and
    (GetFocus <> lstSubtitles.Handle) then
  // ReplaceString(GetSubText(lstSubtitles.FocusedNode), '|', #13#10) modified to GetSubText(lstSubtitles.FocusedNode) - TEXT IS NOT SAVED WITH | INSTEAD OF #13#10! //mmoSubtitleText.Text changed to NewString by adenry
  begin
    OldString := GetSubText(lstSubtitles.FocusedNode);
    SetText(lstSubtitles.FocusedNode, NewString);
    // mmoSubtitleText.Text changed to NewString by adenry
    DetectChangesForUndo(OldString, NewString, True,
      tmrFastTypingUndoBind.Enabled);
    // GetSubText(lstSubtitles.FocusedNode) replaced with NewString by adenry
    AutoRecheckNodeErrors(lstSubtitles.FocusedNode, TextErrors);
    // added by adenry
  ...
  lblText.Caption := Format(TextOrTransLength,
    [LabelText, GetLengthForEachLine(NewString)]);
  // mmoSubtitleText.Text replaced with NewString by adenry
  MarkLongLinesInLabel(lblText); // added by adenry
...
var
  ...
  NewString: String; // added by adenry
begin
  NewString := GetSWTextEditText(mmoTranslation);
  // added by adenry - properly convert the Unicode text of the RichEdit to ANSI - WITH THE PROPER ENCODING!
  IF mnuTranslatorMode.Checked = True then
  begin
    if (lstSubtitles.SelectedCount = 1) and
      (NewString <> GetSubTranslation(lstSubtitles.FocusedNode)) and
      (GetFocus <> lstSubtitles.Handle) then
    // mmoTranslation.Text replaced with NewString by adenry
    begin
      OldString := GetSubTranslation(lstSubtitles.FocusedNode);
      SetTranslation(lstSubtitles.FocusedNode, NewString);
      // mmoTranslation.Text changed to NewString by adenry
      DetectChangesForUndo(OldString, NewString, False,
        tmrFastTypingUndoBind.Enabled);
      // GetSubTranslation(lstSubtitles.FocusedNode) replaced with NewString by adenry
      lstSubtitles.RepaintNode(lstSubtitles.FocusedNode);
      TransModified := True;
      // added by adenry - fast typing undo actions binding:
      tmrFastTypingUndoBind.Enabled := False;
  ...
  lblTranslation.Caption := Format(TextOrTransLength,
    [LabelTranslation, GetLengthForEachLine(NewString)]);
  // mmoTranslation.Text replaced with NewString by adenry
  MarkLongLinesInLabel(lblTranslation); // added by adenry
...
begin
  if (lstSubtitles.Enabled)
  { and (mmoSubtitleText.Enabled) and (mmoTranslation.Enabled) } then
  // and (mmoSubtitleText.Enabled) and (mmoTranslation.Enabled) removed by adenry to fix a bug... they are unnecessary anyway...
  begin
    if (Player.Initialized) and (mnuVideoPreviewMode.Checked = True) then
    begin
...
begin
  ...
  if not MarkingHighPriority then
    if Data.Marked and MarkingWithColor then
      if ((Column = 0) and MarkingColNum) or ((Column = 1) and MarkingColShow)
        or ((Column = 2) and MarkingColHide) or
        ((Column = 3) and MarkingColText) or ((Column = 6) and MarkingColPause)
        or ((Column = 5) and MarkingColDur) then
  ...
  begin
    ...
    begin
      Color := GetSubColor(ItemText);
      if (SmartPos('<i>', ItemText, False) > 0) then
      // Pos changed to SmartPos by adenry
        TargetCanvas.Font.Style := TargetCanvas.Font.Style + [fsItalic];
      if (SmartPos('<b>', ItemText, False) > 0) then
      // Pos changed to SmartPos by adenry
        TargetCanvas.Font.Style := TargetCanvas.Font.Style + [fsBold];
    ...
    begin
      ...
      if (Data.Text = Data.Translation) or (Data.Translation = '') or
        (RemoveSWTags(Data.Translation, True, True, True, True)
        = UntranslatedSub) then // RemoveSWTags by adenry
  ...
  begin
    ...
    if Column = 3 then
    begin
      if not(not MarkingHighPriority and Data.Marked and MarkingWithColor and
        MarkingColText) { and not UseOriginalTextColor } then
      // if Marking is not with high priority or the subtitle is not marked with text color in this column
    ...
    if (Data.ErrorType <> []) and (Column in [0 .. 3]) then
      ...
      if (mfEnabled in MarkFlags[0]) then
        for i := 1 to Integer(High(TErrorType)) + 1 do // for each type of error
          if (TErrorType(i - 1) in Data.ErrorType) and
            (mfEnabled in MarkFlags[i]) then
          begin
            // if no marking column is set for current error type
            if not((mfColNum in MarkFlags[i]) or (mfColShow in MarkFlags[i]) or
              (mfColHide in MarkFlags[i]) or (mfColText in MarkFlags[i]) or
              (mfColPause in MarkFlags[i]) or (mfColDur in MarkFlags[i])) then
              if ((Column = 0) and (mfColNum in MarkFlags[0])) or
                ((Column = 1) and (mfColShow in MarkFlags[0])) or
                ((Column = 2) and (mfColHide in MarkFlags[0])) or
                ((Column = 3) and (mfColText in MarkFlags[0])) or
                ((Column = 6) and (mfColPause in MarkFlags[0])) or
                ((Column = 5) and (mfColDur in MarkFlags[0])) then
              begin
                if (MarkColors[0] <> lstSubtitles.Font.Color)
                { and (not UseOriginalTextColor or (Column<>3) or (UseOriginalTextColor and OnlyIfThereIsAColorTag and (GetSubColor(ItemText) = -1))) }
                then
                  TargetCanvas.Font.Color := MarkColors[0];
                if (Column <> 3) { or not UseOriginalTextStyle } then
                begin
                  if mfBold in MarkFlags[0] then
                    TargetCanvas.Font.Style := TargetCanvas.Font.Style
                      + [fsBold];
    ...
    for i := 1 to Integer(High(TErrorType)) + 1 do // for each type of error
      if (TErrorType(i - 1) in Data.ErrorType) and (mfEnabled in MarkFlags[i])
      then
      begin
        if ((Column = 0) and (mfColNum in MarkFlags[i])) or
          ((Column = 1) and (mfColShow in MarkFlags[i])) or
          ((Column = 2) and (mfColHide in MarkFlags[i])) or
          ((Column = 3) and (mfColText in MarkFlags[i])) or
          ((Column = 6) and (mfColPause in MarkFlags[i])) or
          ((Column = 5) and (mfColDur in MarkFlags[i])) then
        begin
          if (MarkColors[i] <> lstSubtitles.Font.Color)
          { and (not UseOriginalTextColor or (Column<>3) or (UseOriginalTextColor and OnlyIfThereIsAColorTag and (GetSubColor(ItemText) = -1))) }
          then
  ...
  if (TargetCanvas.Font.Color = frmMain.lstSubtitles.Font.Color) and
...
var
  ...
  DiffTime: Integer;
begin
  if (Player.Initialized) and (mnuVideoPreviewMode.Checked) and
    (lstSubtitles.SelectedCount >= 1) then
  begin
...
begin
  if (Player.Initialized) and (mnuVideoPreviewMode.Checked) and
    (Assigned(lstSubtitles.FocusedNode)) and (lstSubtitles.SelectedCount >= 1)
  then
  begin
    Node := lstSubtitles.GetFirstSelected;
    ...
    NewShowTime := GetCurrentPos + ShiftShowVal;
    // + ShiftShowVal added by adenry
    ...
    if lstSubtitles.SelectedCount = 1 then
      ChangeShowTime(Node, NewShowTime, False, False, False,
        uanMovieSetStartTime)
    else
      // MORE THAN ONE subtitles selected - shift all selected subtitles
      if lstSubtitles.SelectedCount > 1 then
      begin
        UndoListCount := UndoList.Count;
        ChangeShowTime(Node, NewShowTime, True, False, True,
          uanMovieSetStartTime);

        Node := lstSubtitles.GetNextSelected(Node);
        while Assigned(Node) do
        begin
          ChangeShowTime(Node, GetStartTime(Node) + NewShowTime - StartTime,
            True, False, True, uanMovieSetStartTime);
          Node := lstSubtitles.GetNextSelected(Node);
        end;
...
begin
  ...
  begin
    ...
    // ONE subtitle selected
    if lstSubtitles.SelectedCount = 1 then
      ChangeHideTime(Node, NewHideTime, False, False, False,
        uanMovieSetFinalTime)
    else
      // MORE THAN ONE subtitles selected - shift all selected subtitles
      if lstSubtitles.SelectedCount > 1 then
      begin
        UndoListCount := UndoList.Count;
        ChangeHideTime(Node, NewHideTime, True, False, True,
          uanMovieSetFinalTime);

        Node := lstSubtitles.GetPreviousSelected(Node);
        while Assigned(Node) do
        begin
          ChangeHideTime(Node, GetFinalTime(Node) + NewHideTime - HideTime,
            True, False, True, uanMovieSetFinalTime);
          Node := lstSubtitles.GetPreviousSelected(Node);
        end;
        if UndoList.Count > UndoListCount then
          PUndoAction(UndoList.Items[UndoList.Count - 1])^.BindToNext := False;
    ...
    begin
      ...
      if GetStartTime(Node.NextSibling) < NewHideTime + DefaultSubPause then
      // added by adenry
      begin
        ...
        PTimeChange(UndoAction2^.Buffer)^.StartTime :=
          GetStartTime(Node.NextSibling);
        ...
        // removed by adenry: end
        SetStartTime(Node.NextSibling, NewHideTime + DefaultSubPause);
        // added by adenry //CurrPos replaced with GetFinalTime(Node) later
        AutoRecheckNodeErrorsAndRepaint(Node.NextSibling, TimeErrors);
        // added by adenry
      end;
...
var
  ...
  UndoAction: PUndoAction;
begin
  if (Player.Initialized) and (StartSubTime <> -1) and (lstSubtitles.Enabled)
  then
  begin
    Node := InsertNode;
    if Assigned(lstSubtitles.FocusedNode) then
    ...
    RefreshTimes;
    mmoSubtitleText.SetFocus;
    // moved after RefreshTimes by adenry - fixes a bug
    AutoRecheckNodeErrorsAndRepaint(lstSubtitles.FocusedNode);
    // added by adenry
    AutoRecheckNodeErrorsAndRepaint(lstSubtitles.FocusedNode.NextSibling,
      PauseErrors); // added by adenry
  end;
end;
...
begin
  ...
  AutoRecheckNodeErrorsAndRepaint(lstSubtitles.FocusedNode, [], rtSubAdded);
  // added by adenry //rtSubAdded added later
  AutoRecheckNodeErrorsAndRepaint(lstSubtitles.FocusedNode.NextSibling,
    PauseErrors); // added by adenry
end;
...
begin
  ...
  case Key of
    Ord('S'):
      begin
        if PressedKeyInLstSubtitles = 1 then
          PressedKeyInLstSubtitles := 0;
        tmeShow.Font.Style := tmeShow.Font.Style - [fsUnderline];
        if TempActiveControl <> nil then
          TTimeMaskEdit(TempActiveControl).Font.Style :=
            TTimeMaskEdit(TempActiveControl).Font.Style + [fsUnderline];
      end;
    Ord('E'):
      begin
        if PressedKeyInLstSubtitles = 2 then
          PressedKeyInLstSubtitles := 0;
        tmeHide.Font.Style := tmeHide.Font.Style - [fsUnderline];
        if TempActiveControl <> nil then
          TTimeMaskEdit(TempActiveControl).Font.Style :=
    ...
    Ord('D'):
      begin
        if PressedKeyInLstSubtitles = 3 then
          PressedKeyInLstSubtitles := 0;
        tmeHide.Font.Style := tmeHide.Font.Style - [fsUnderline];
        if TempActiveControl <> nil then
          TTimeMaskEdit(TempActiveControl).Font.Style :=
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.lstSubtitlesMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  // RefreshTimes; //removed by adenry - OnChange event handler handles it now
  // added by adenry: begin
  ...
  if Assigned(lstSubtitles.GetNodeAt(X, Y)) then
    if (lstSubtitles.GetNodeAt(X, Y) = lstSubtitles.FocusedNode) and
      (ssCtrl in Shift) and
      (not(vsSelected in lstSubtitles.GetNodeAt(X, Y).States)) then
...
begin
  ...
  if Screen.ActiveControl.ClassName = 'TRichEdit' then
    TRichEdit(Screen.ActiveControl).CutToClipboard
  else if Screen.ActiveControl.ClassName = 'TMemo' then
    TMemo(Screen.ActiveControl).CutToClipboard
  else if Screen.ActiveControl.ClassName = 'TEdit' then
    TEdit(Screen.ActiveControl).CutToClipboard
  else if Screen.ActiveControl.ClassName = 'TComboBox' then
  begin
    ClipBoard.AsText := TComboBox(Screen.ActiveControl).Text;
    TComboBox(Screen.ActiveControl).Text := '';
  end
  else if Screen.ActiveControl.ClassName = 'TTimeMaskEdit' then
    TTimeMaskEdit(Screen.ActiveControl).CutToClipboard
  else
  // added by adenry: end
  begin
...
begin
  // added by adenry: begin
  if Screen.ActiveControl.ClassName = 'TRichEdit' then
    TRichEdit(Screen.ActiveControl).CopyToClipboard
  else if Screen.ActiveControl.ClassName = 'TMemo' then
    ...
    TTimeMaskEdit(Screen.ActiveControl).CopyToClipboard
  else
    // added by adenry: end
    CopyNodesToClipBoard;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Text := WideStringToStringEx(WText,
        CharsetToCodePage(TExtWinControl(ActiveControl).Font.Charset));
    end
    else
      Text := ClipBoard.AsText;

    if (ActiveControl = mmoSubtitleText) or (ActiveControl = mmoTranslation) or
      ((Assigned(frmDivideLines)) and ((ActiveControl = frmDivideLines.mmoSub1)
      or (ActiveControl = frmDivideLines.mmoSub2))) then
      SetSWTextEditSelText(TSWTextEdit(ActiveControl), Text)
    else // paste plain text, we don't want any formatting //TMemo changed to TSWTextEdit
      if ActiveControl.ClassName = 'TRichEdit' then
        TRichEdit(ActiveControl).PasteFromClipboard
      else // PasteFromClipboard works okay for TRichEdit, as TRichEdit uses Unicode internally
        if ActiveControl.ClassName = 'TMemo' then
          TMemo(ActiveControl).SelText := Text
        else // PasteFromClipboard doesn't convert to ANSI with proper Charset
          if ActiveControl.ClassName = 'TEdit' then
            TEdit(ActiveControl).SelText := Text
          else // PasteFromClipboard doesn't convert to ANSI with proper Charset
            if ActiveControl.ClassName = 'TLabeledEdit' then
              TLabeledEdit(ActiveControl).SelText := Text
            else // PasteFromClipboard doesn't convert to ANSI with proper Charset
              if ActiveControl.ClassName = 'TComboBox' then
                TComboBox(ActiveControl).SelText := Text
              else
                // added by adenry: end

                if ActiveControl.ClassName = 'TTimeMaskEdit' then
                // modified by adenry
                begin
                  ...
                  begin
                    if IsInteger(Text) then
                      Time := FramesToTime(StrToInt(Text), GetFPS)
                    else
                    ...
                    if ActiveControl.Name = 'tmeShow' then
                      tmeShowTimeChangeFromEditOnly(Sender, Time);
                    // SetStartTime(lstSubtitles.FocusedNode, Time) else
                    if ActiveControl.Name = 'tmeHide' then
                      tmeHideTimeChangeFromEditOnly(Sender, Time);
                    // SetFinalTime(lstSubtitles.FocusedNode, Time) else
                    if ActiveControl.Name = 'tmeDuration' then
                      tmeDurationTimeChangeFromEditOnly(Sender, Time);
                    // SetFinalTime(lstSubtitles.FocusedNode, GetStartTime(lstSubtitles.FocusedNode) + Time) else
                    if ActiveControl.Name = 'tmePause' then
                      tmePauseTimeChangeFromEditOnly(Sender, Time)
                    else
                      TTimeMaskEdit(Screen.ActiveControl).Time := Time;

                    if (ActiveControl.Name = 'tmeShow') or
                      (ActiveControl.Name = 'tmeHide') or
                      (ActiveControl.Name = 'tmeDuration') or
                      (ActiveControl.Name = 'tmePause') then
                    begin
                      lstSubtitles.RepaintNode(lstSubtitles.FocusedNode);
                      RefreshTimes;
                    end;
...
begin
  ...
  if (mnuTranslatorMode.Checked) and (A = mmoTranslation.Handle) then
    MemoKeyPress(mmoTranslation, lstSubtitles, True)
  else if (A = mmoSubtitleText.Handle) or (A = lstSubtitles.Handle) then
    MemoKeyPress(mmoSubtitleText, lstSubtitles, True)
  else if A = tmeShow.Handle then
  begin
    i := tmeShow.SelStart;
  ...
  else if A = tmeDuration.Handle then
  begin
...
begin
  A := GetFocus;
  if (mnuTranslatorMode.Checked) and (A = mmoTranslation.Handle) then
    MemoKeyPress(mmoTranslation, lstSubtitles, False)
  else if (A = mmoSubtitleText.Handle) or (A = lstSubtitles.Handle) then
    MemoKeyPress(mmoSubtitleText, lstSubtitles, False)
  else if A = tmeShow.Handle then
  begin
    i := tmeShow.SelStart;
...
begin
  ...
  begin
    ...
    begin
      ...
      if RemoveSWTags(GetSubTranslation(Node), True, True, True, True) <> UntranslatedSub
      then // RemoveSWTags by adenry
...
begin
  ...
  begin
    ...
    begin
      NewText := SetSingleTagsModeFontStyles(NewText,
        GetSingleTagsModeFontStyles(NewText), True);
      NewTrans := SetSingleTagsModeFontStyles(NewTrans,
        GetSingleTagsModeFontStyles(NewTrans), True);
    end;
    // added by adenry: end
    ...
    PUndoAction(UndoList.Last)^.UndoActionName := uanCombineSubs;
    // added by adenry
    ...
    AutoRecheckNodeErrorsAndRepaint(lstSubtitles.FocusedNode);
    // added by adenry
    UpdateSubSubtitleVisibilityAfterNodeChange(UpdNode, -1, -1,
      GetStartTime(UpdNode), FinalTime); // added by adenry
  end;
end;
...
begin
  ...
  begin
    ...
    // added by adenry: begin
    if (mmoSubtitleText.Focused) or
      ((lstSubtitles.Focused) and (lstSubtitles.FocusedColumn = 3)) then
    begin
    ...
    if (mnuTranslatorMode.Checked) and
      ((mmoTranslation.Focused) or ((lstSubtitles.Focused) and
      (lstSubtitles.FocusedColumn = 4)) or (lstSubtitles.SelectedCount > 1))
    then // added by adenry
    begin
      tmp := GetSubTranslation(Node); // moved to the subprocedure by adenry
      if RemoveSWTags(tmp, True, True, True, True) <> UntranslatedSub then
      // RemoveSWTags by adenry
      begin
        // added by adenry: begin
        tmp2 := tmp;
    ...
    // ((mnuTranslatorMode.Checked) and (GetSubTranslation(Node) <> PFullTextChange(UndoAction^.Buffer)^.OldTrans)) then //removed by adenry
      (PFullTextChange(UndoAction^.Buffer)^.OriginalOnly = False) then
    // added by adenry
      AddUndo(UndoAction);
...
begin
  ...
  begin
    ...
    UndoAction^.LineNumber := Node.Index;
    PFullTextChange(UndoAction^.Buffer)^.OriginalOnly := True;
    // not mnuTranslatorMode.Checked; //the NOT was added by adenry - bug fix //modified by adenry to True
...
begin
  ...
  if GetFinalTime(lstSubtitles.FocusedNode) -
    GetStartTime(lstSubtitles.FocusedNode) < DefaultSubPause + 2 *
    Max(1, RoundingFactor * Integer(AutoRoundTimeVals)) then
...
begin
  ...
  begin
    ...
    if ((Length(RemoveSWTags(GetSubText(Node), True, True, True, True)) >
      BreakLineAfter) or (Pos(#13#10, GetSubText(Node)) > 0)) then
    // added by adenry
    begin
      ...
      DivideSubText(Data1.Text, BreaksArray, AdjustAutomatically, Part1, Part2,
        ExtraVar);
      // ProcessStringToDivide replaced with DivideSubText by adenry
      ...
      begin
        ...
        // added by adenry:
        Duration1 := DivideSubDuration(Node, Part1, 1, 1,
          DefaultSubPause, True);
        ...
        SetStartTime(lstSubtitles.GetNextSibling(Node),
          Data1.FinalTime + DefaultSubPause);
        // added by adenry: end
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PFullTextChange(UndoAction^.Buffer)^.OriginalOnly := False;
        // True fixed to False by adnery - bug fix
        PFullTextChange(UndoAction^.Buffer)^.OldTrans := OldTrans;
        SetTranslation(Node, NewTrans);
      end;
    end;
...
begin
  ...
  begin
    ...
    mmoSubtitleText.Font.Charset := OrgCharset;
    if lstSubtitles.SelectedCount = 1 then
      RefreshTimes; // added by adenry - refresh mmoSubtitleText's text
    lstSubtitles.Refresh; // repaint texts in lstSubtitles
    if (Player.Initialized) and (mnuDisplayOriginal.Checked) then
      subSubtitle.Font.Charset := OrgCharset;
    if Assigned(frmInfoErrors) then
      frmInfoErrors.SetCustomInfosCharsets; // added later
    Dashes := SetDashes(OrgCharset);
    // added by adenry - refresh dashes list (not all charsets support all dashes characters)
    AutoRecheckAllErrors(TextErrors);
    // added by adenry - dashes may have changed, so dash-related errors may have changed
  end;
end;
...
begin
  ...
  begin
    ...
    mmoTranslation.Font.Charset := TransCharset;
    if lstSubtitles.SelectedCount = 1 then
      RefreshTimes; // added by adenry - refresh mmoTranslation's text
    SetUntranslatedSubString;
    // added by adenry - refresh the - Untranslated subtitle - string
    lstSubtitles.Refresh; // repaint texts in lstSubtitles
...
begin
  if (Player.Initialized) and (lstSubtitles.RootNodeCount > 1) and
    (lstSubtitles.Enabled) then
  begin
...
begin
  if (Player.Initialized) and (SyncPoint.Point1Sub <> -1) and
    (SyncPoint.Point1Movie <> -1) and (lstSubtitles.RootNodeCount > 1) and
    (lstSubtitles.Enabled) then
  begin
    ...
    if MsgBox(Format(QuestionMsg[06], [TimeToString(SyncPoint.Point1Sub),
      TimeToString(SyncPoint.Point1Movie), TimeToString(SyncPoint.Point2Sub),
      TimeToString(SyncPoint.Point2Movie)]), BTN_YES, BTN_NO, '',
      MB_ICONQUESTION, frmMain) = mrOk then
    begin
...
begin
  if (lstSubtitles.RootNodeCount > 1) and (Assigned(lstSubtitles.FocusedNode))
    and (lstSubtitles.SelectedCount = 1) then
  begin
    ...
    frmAdjustSubtitles.AddSyncPoint(GetStartTime(lstSubtitles.GetFirstSelected),
      GetCurrentPos, lstSubtitles.GetFirstSelected.Index + 1);
  end;
end;
...
begin
  ...
  if Screen.ActiveControl.ClassName = 'TRichEdit' then
    ...
    TComboBox(Screen.ActiveControl).SelectAll
  else if Screen.ActiveControl.ClassName = 'TTimeMaskEdit' then
    TTimeMaskEdit(Screen.ActiveControl).SelectAll
  else
  // added by adenry: end
  begin
...
begin
  ...
  frmSetDelay.SubtitleTime := GetStartTime(lstSubtitles.FocusedNode);
  frmSetDelay.VideoTime := sbSeekBar.Position;
...
begin
  if FindInNode(SearchWord, CaseSensitive, MatchWholeWords, True) = nil then
    MsgBox(Format(InfoMsg[01], [SearchWord]), BTN_OK, '', '',
      MB_ICONINFORMATION, frmMain)
  else
    RefreshTimes;
end;
...
begin
  ...
  begin
    ...
    PTimeChange(UndoAction^.Buffer)^.StartTime := -1;
    PTimeChange(UndoAction^.Buffer)^.FinalTime := GetFinalTime(Node);
    // OldFinalTime //changed by adenry
    UndoAction^.BindToNext := True; // added by adenry

    if Node <> lstSubtitles.GetLast then
      NewFinalTime := GetStartTime(Node.NextSibling) - DefaultSubPause
    else // added by adenry
    ...
    if NewFinalTime <> GetFinalTime(Node) then
    // if OldFinalTime <> GetFinalTime(Node) then //changed by adenry
    begin
...
begin
  ...
  begin
    ...
    if Node <> lstSubtitles.GetFirst then
      NewStartTime := GetFinalTime(Node.PrevSibling) + DefaultSubPause
    else
...
begin
  ...
  begin
    if GetVideoInfo(dlgLoadFile.FileName, FPS, Duration) = True then
      AddFPSItem(FPS, False, True, False)
    else
      MsgBox(Format(ErrorMsg[05], [dlgLoadFile.FileName]), BTN_OK, '', '',
        MB_ICONERROR, frmMain);
  end;
...
begin
  ...
  begin
    tbItalicTrans.Down := not tbItalicTrans.Down;
...
begin
  if (lstSubtitles.FocusedColumn = 4) or (mmoTranslation.Focused) then
  begin
    tbUnderlineTrans.Down := not tbUnderlineTrans.Down;
    // simulate toolbar button click
...
// added by adenry: begin
procedure TfrmMain.SetNodeColor(Node: PVirtualNode; Color: TColor;
  Translation: Boolean = False; RemoveOtherColors: Boolean = False;
  ToggleSetRemove: Boolean = False);
var
  Text: String;
  UndoAction: PUndoAction;
...
begin
  ...
  if ToggleSetRemove and
    (GetSubColor(RemoveSWTags(Text, True, True, True, False), True) = Color)
  then
  begin
  ...
  begin
    ...
    if SubtitleAPI.MultiTagsMode and (mmo.SelLength > 0) and
      (frmMain.ActiveControl = mmo) then
    begin
...
// added by adenry: begin
procedure TfrmMain.SetColor(TextColor: Boolean = False;
  TransColor: Boolean = False; NoDialog: Boolean = False;
  RemoveOtherColors: Boolean = False; ToggleSetRemove: Boolean = False);
  ...
  begin
    ...
    begin
      ...
      while Assigned(Node) do
      begin
        SetNodeColor(Node, dlgColor.Color, Translation, RemoveOtherColors,
          ToggleSetRemove);
        Node := lstSubtitles.GetNextSelected(Node);
      end;
      lstSubtitles.Refresh;
...
begin
  if TextColor then
    SetTheColor(False)
  else if TransColor then
    SetTheColor(True)
  else if (lstSubtitles.FocusedColumn = 4) or (mmoTranslation.Focused) then
    SetTheColor(True)
  else
    SetTheColor(False);
end;
...
var
  ...
  // True for original, false for both (translation and original)
begin
  Node := lstSubtitles.GetFirstSelected;
  while Assigned(Node) do
  begin
    ...
    begin
      SetText(Node, RemoveSWTags(Text, False, False, False, True, True));
      // moved her by adenry
      OriginalOnly := True;
    end;
...
begin
  ...
  else if lstSubtitles.FocusedColumn = 4 then
  begin
...
begin
  if lstSubtitles.RootNodeCount > 0 then
    if not Assigned(frmSaveAs) then
    // added by adenry - because saving the backup just prior to saving a file results in not saving the file!
    begin
      // ------------------ //
      // Save original   //
      // ------------------ //
      ...
      else if (frmSaveAsExecuting = False) and (AskToSaveNewSubs) then
      // added by adenry
      begin
        mnuSaveFileAsClick(Sender);
        UTF8_Orig := SubtitleAPI.Get_UTF8_File;
      ...
      begin
        if (TransFile <> '') then
        begin
          UpdateArray(TransFormat, True);
          // added by adenry  //format index added by adenry
          if SaveAsBackup = False then
          begin
...
begin
  if (Key = Chr(VK_RETURN)) and (IsFloat(cmbInputFPS.Text)) then
    AddFPSItem(StrToFloat(cmbInputFPS.Text), True, True, True)
  else if (Key in ['0' .. '9', System.SysUtils.FormatSettings.DecimalSeparator,
    Chr(VK_RETURN), Chr(VK_BACK)]) = False then
    Key := #0;
end;
...
begin
  if (Key = Chr(VK_RETURN)) and (IsFloat(cmbFPS.Text)) then
    AddFPSItem(StrToFloat(cmbFPS.Text), False, True, False)
  else if (Key in ['0' .. '9', System.SysUtils.FormatSettings.DecimalSeparator,
    Chr(VK_RETURN), Chr(VK_BACK)]) = False then
    Key := #0;
end;
...
begin
  ...
  if (dlgLoadFile.Execute) and (dlgLoadFile.FileName <> '') then
  begin
    if SubtitleAPI.LoadSubtitle(dlgLoadFile.FileName, GetInputFPS,
      dlgLoadFile.FilterIndex - 1) then
    begin
      ...
      Sync.Point2Movie := SubtitleAPI.GetInitialTime
        (SubtitleAPI.SubtitleCount - 1);
      AdjustSubtitles(Sync, 3);
    end
    else
    begin
      if ((dlgLoadFile.FilterIndex - 1) = 0) then
        MsgBox(Format(ErrorMsg[03], [dlgLoadFile.FileName]), BTN_OK, '', '',
          MB_ICONERROR, frmMain)
      else
        MsgBox(Format(ErrorMsg[04], [dlgLoadFile.FileName,
          SubtitleAPI.GetFormatName(dlgLoadFile.FilterIndex - 1)]), BTN_OK, '',
          '', MB_ICONERROR, frmMain);
    end;
  end;
  dlgLoadFile.Filter := SubtitleAPI.FillDialogFilter(AllSupportedFiles) + ID_SRF
...
begin
  dlgLoadFile.Filter := SubtitleAPI.FillDialogFilter(AllSupportedFiles);
  if (dlgLoadFile.Execute) and (dlgLoadFile.FileName <> '') then
    ReadTextsAndTimesFromFile(dlgLoadFile.FileName, dlgLoadFile.FilterIndex - 1,
      True, False);
  dlgLoadFile.Filter := SubtitleAPI.FillDialogFilter(AllSupportedFiles) + ID_SRF
    + '|' + ID_PLAINTEXT;
...
begin
  ...
  begin
    ...
    begin
      ...
      Node := Node.NextSibling;
      UndoAction^.BindToNext := True;
      // the last one is bound to the uaFPSChange //added by adenry
...
begin
  ...
  begin
    if Data.Marked = True then
      HintText := ErrorReports[20]
    else
      HintText := '';
  end;

  if etLinesWithoutLetters in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[01];
  if etEmptySubtitle in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[02];
  // ---
  if etOverlapping in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[03];
  if etBadValues in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[04];
  if etTooLongDuration in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[05];
  if etTooShortDuration in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[06];
  if etTooLongLine in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[07];
  if etOverTwoLines in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[08];
  // ---
  if etHearingImpaired in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[09];
  if etTextBeforeColon in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[10];
  if etUnnecessaryDots in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[11];
  if etProhibitedCharacter in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[12];
  if etRepeatedCharacter in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[13];
  if etRepeatedSubtitle in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[14];
  if etOCRErrors in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[15];
  // ---
  if etOpnDlgSubsOneLine in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[16];
  if etSpaceAfterCustChars in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[17];
  if etSpaceBeforeCustChars in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[18];
  if etUnnecessarySpaces in Data.ErrorType then
    HintText := HintText + #13#10 + ErrorReports[19];

  if (Data.ErrorType = []) and (Data.Marked = False) then
  begin
    if mnuTranslatorMode.Checked then
      HintText := Format(TransLeftLines, [CountUnTranslated])
    else
      lstSubtitles.Hint := '';
  end;
  HintText := StringToWideStringEx(Trim(HintText),
    CharsetToCodePage(frmMain.Font.Charset));
end;
...
begin
  Ini := TIniFile.Create(IniRoot);
  try
    if (FileExists(Ini.ReadString('External Preview', 'Video player', ''))
      = False) then
    begin
      if MsgBox(QuestionMsg[07], BTN_YES, BTN_NO, '', MB_ICONWARNING, frmMain) = 1
      then
      begin
        frmSettings := TfrmSettings.Create(Application);
    ...
    if Ini.ReadBool('External Preview', 'Save in original format', True) then
      ...
      Format := SubtitleAPI.GetFormatIndex(Ini.ReadString('External Preview',
        'Custom format', 'SubRip'));
    ...
    if mnuTranslatorMode.Checked = False then
      FileName := ExtractFileName(OrgFile)
    else
      FileName := ExtractFileName(TransFile);
    if FileName = '' then
      FileName := 'TmpSubFile' + Ext
    else
      FileName := Copy(FileName, 1, LastDelimiter('.', ExtractFileName(FileName)
        ) - 1) + Ext;

    UpdateArray(Format, mnuTranslatorMode.Checked);
    // format index added by adenry
    ...
    ShellExecute(Handle, 'open', PChar(Ini.ReadString('External Preview',
      'Video player', '')), PChar(Params), nil, SW_SHOW);
...
var
  ...
  Data: PSubtitleItem;
begin
  if (Assigned(lstSubtitles.FocusedNode)) and
    (lstSubtitles.FocusedNode <> lstSubtitles.GetLast) then
...
begin
  if (Assigned(lstSubtitles.FocusedNode)) and
    (lstSubtitles.FocusedNode <> lstSubtitles.GetFirst) then
...
begin
  ...
  begin
    ...
    tmpText1 := ReplaceString(RemoveSWTags(Text1, True, True, True,
      True), ' ', '');
    tmpText2 := ReplaceString(RemoveSWTags(Text2, True, True, True,
      True), ' ', '');
    ...
    begin
      ...
      begin
        ...
        // delete dots at the end of Text1
        DotPos := SmartPos('.', Text1, False, Length(Text1), False);
        while (Copy(Text1, DotPos, 1) = '.') or
          (Copy(Text1, DotPos, 1) = ' ') do
        begin
          Delete(Text1, DotPos, 1);
          Dec(DotPos);
        end;
        ...
        // delete dots at the beginning of Text2
        DotPos := Pos('.', Text2);
        while (Copy(Text2, DotPos, 1) = '.') or
          (Copy(Text2, DotPos, 1) = ' ') do
          Delete(Text2, DotPos, 1);
...
begin
  ...
  Dir := GetTempDir + Copy(ID_UPDATEINI, LastDelimiter('/', ID_UPDATEINI) + 1,
    Length(ID_UPDATEINI));
  ...
  begin
    ...
    try
      ...
      NewBuild := Ini.ReadString('Update', 'CurrBuild', ID_BUILD);
      // added by adenry
      // if NewVer <> ID_FULLVERSION Then               //removed by adenry
      if StrToInt(NewBuild) > StrToInt(ID_BUILD) then // added by adenry
      begin // New version!
        if MsgBox(Format(QuestionMsg[08], [NewVer + ' (build ' + NewBuild + ')']
          ), BTN_YES, BTN_NO, '', MB_ICONQUESTION, frmMain) = 1 Then
        // modified by adenry
        begin
          if Ini.ValueExists('Update', Language) then
            URL := Ini.ReadString('Update', Language, ID_WEBPAGE)
          else
            URL := Ini.ReadString('Update', 'English', ID_WEBPAGE);
          ShellExecute(GetDesktopWindow, 'open', PChar(URL), nil, nil,
            SW_SHOWNORMAL);
        end;
      end
...
procedure TfrmMain.sbSeekBarMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if (Player.Initialized) and (mnuVideoPreviewMode.Checked) and (Button = mbLeft)
  then
    tmrVideo.Enabled := False;
end;
...
begin
  ...
  i := ((X - (sbSeekBar.Normal.Width div 2)) * sbSeekBar.Max)
    div (sbSeekBar.Width - (sbSeekBar.Normal.Width));
  if (i < 0) then
    i := 0;
  if (i > sbSeekBar.Max) then
    i := sbSeekBar.Max;
  if FormatType = ftTime then
    tcTimeCounter.Text2 := TimeToString(i)
  else
...
procedure TfrmMain.SpellCheck(Language: Integer = 0);
// Language added by adenry
var
  Node: PVirtualNode;
...
begin
  ...
  try
    ...
    USSpellCheck.Language := Language;
    // added by adenry //TODO: Instead of this, get a list of all installed spellcheck languages and offer the user to choose from them...
  finally
    Screen.Cursor := crDefault; // added by adenry
  end;
  try
    ...
    begin
      ...
      try
          ...
          try
            ...
            begin
              ...
              begin
                ...
                if (i = 0) or ((i > 0) and (StringToCheck[i - 1] = '')) then
                // Avoid lots of problems with subtitles that are more than one line
                begin
                  c := 1;
                  while (i + c < StringToCheck.Count) and
                    (StringToCheck[i + c] <> '') do
                  begin
                    if (StringToCheck[i + c] <> '') then
                    begin
                      if Text <> '' then
                        Text := Text + #13#10 + StringToCheck[i + c]
                      else
            ...
            begin
              ...
              begin
                ...
                UndoAction^.Node := Node;
                UndoAction^.LineNumber := Node.Index;
                PFullTextChange(UndoAction^.Buffer)^.OldText :=
                  GetSubText(Node);
        ...
        AutoRecheckAllErrors(TextErrors); // added by adenry
        MsgBox(Format(InfoMsg[03], [USSpellCheck.NumChanges]), BTN_OK, '', '',
          MB_ICONINFORMATION, frmMain);
        StringToCheck.Free;
      end;
    end
...
begin
  ...
  if FileExists(ManualFile) then
    ShellExecute(Handle, 'open', PChar(ManualFile), nil, nil, SW_MAXIMIZE)
  else
    ShellExecute(Handle, 'open', PChar(ExtractFilePath(Application.ExeName) +
      'Help\Manual.html'), nil, nil, SW_MAXIMIZE);
end;
...
procedure TfrmMain.MarkNode(Node: PVirtualNode;
  BindUndoToNext: Boolean = False);
var
  Data: PSubtitleItem;
  UndoAction: PUndoAction;
begin
...
begin
  ...
  begin
    ...
    AutoRecheckNodeErrors(Node, [], rtNormal, True);
    // refresh info&errors form only //added by adenry
    Node := lstSubtitles.GetNextSelected(Node);
  end;
...
// added by adenry: begin
procedure TfrmMain.UnmarkNode(Node: PVirtualNode;
  BindUndoToNext: Boolean = False);
var
  Data: PSubtitleItem;
  UndoAction: PUndoAction;
...
begin
  ...
  begin
    ...
    if Node = lstSubtitles.GetLast then
    begin
      if (CurrTime >= GetStartTime(Node)) and (CurrTime <= GetFinalTime(Node))
      then
        SetVideoPos(GetStartTime(Node.PrevSibling))
      else
...
begin
  if mnuUseInPlaceEdition.Checked = False then
    lstSubtitles.TreeOptions.MiscOptions := lstSubtitles.TreeOptions.MiscOptions
      - [toEditable]
  else
    lstSubtitles.TreeOptions.MiscOptions := lstSubtitles.TreeOptions.MiscOptions
      + [toEditable];
...
begin
  ...
  begin
    ...
    Node := lstSubtitles.GetNextSelected(Node);
    UndoAction^.BindToNext := True;
    // Assigned(Node); //modified by adenry later
    AddUndo(UndoAction);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not lstSubtitles.Selected[Node.PrevSibling] then
      // added by adenry later
        AutoRecheckNodeErrors(Node, PauseErrors); // added by adenry
      if not lstSubtitles.Selected[Node.NextSibling] then
      // added by adenry later
        AutoRecheckNodeErrors(Node.NextSibling, PauseErrors); // added by adenry
    end;
...
begin
  ...
  begin
    Size := VideoPreview.VideoFileSize / 1024; // in Mb
...
{$I+}
    ...
    MsgBox(Format(InfoMsg[09], [MovieFile, // File name
      ...
      ]), BTN_OK, '', '', MB_ICONINFORMATION, frmMain);
  end;
end;
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesEdited(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex);
begin
  InterceptTabKey := False;
end;

procedure TfrmMain.lstSubtitlesEditCancelled(Sender: TBaseVirtualTree;
  Column: TColumnIndex);
begin
  InterceptTabKey := False;
end;
...
begin
  Allowed := (Column <> 0) and
    (toEditable in lstSubtitles.TreeOptions.MiscOptions);
end;
...
begin
  ...
  begin
    ...
    begin
      FN := Copy(ExtractFileName(OrgFile), 1,
        LastDelimiter('.', ExtractFileName(OrgFile)) - 1);

      UpdateArray(SubtitleAPI.GetFormatIndex('SAMI Captioning'));
      // format index added by adenry
      SaveFile(Dir + '\' + FN + '.smi',
        SubtitleAPI.GetFormatIndex('SAMI Captioning'), GetFPS);
      SubtitleAPI.ClearSubtitles;
      ...
      try
        ...
        ASX.Add('  <Entry>');
        ASX.Add('    ' + Format('<Ref href="%s?sami=%s"/>',
          [MovieFile, Dir + '\' + FN + '.smi']));
        ASX.Add('  </Entry>');
        ASX.Add('</ASX>');
...
begin
  ...
  begin
    ...
    begin
      ...
      UpdateArray(SubtitleAPI.GetFormatIndex('RealTime'));
      // format index added by adenry
      SaveFile(Dir + '\' + FN + '.rt',
        SubtitleAPI.GetFormatIndex('RealTime'), GetFPS);
      SubtitleAPI.ClearSubtitles;
      ...
      try
        ...
        SMIL.Add('      ' +
          Format('<video src="%s" begin="1s" clip-begin="0s" region="video_center"/>',
          [MovieFile]));
        SMIL.Add('      ' +
          Format('<textstream src="%s" region="text_subtitle"/>',
          [Dir + '\' + FN + '.rt']));
        SMIL.Add('    </par>');
        SMIL.Add('  </body>');
        SMIL.Add('</smil>');
...
begin
  ...
  try
    ...
    PSCompExec.Script.LoadFromFile(ExtractFilePath(Application.ExeName) +
      'PascalScripts\' + (Sender as TMenuItem).Caption + '.pas');
    // StripHotKey added by adenry for toolbar button fix

    if PSCompExec.Compile then
    begin
      if not PSCompExec.Execute then
        OutputMsg(ErrorMsg[24])
      else
      begin
...
procedure TfrmMain.OnScriptExecute(Sender: TPSScript);
begin
...
procedure TfrmMain.PSCompExecExecImport(Sender: TObject; se: TPSExec;
  X: TPSRuntimeClassImporter);
begin
  RIRegister_Std(X);
...
begin
  OCRDefFile := ExtractFilePath(Application.ExeName) + 'OCRScripts\' +
    cmbOCRScripts.Items[cmbOCRScripts.ItemIndex] + ID_OCREXT;
  // added by adenry later:
  if ((ErrorsToCheck.eOCRErrors) or (ErrorsToFix.eOCRErrors)) and
    (FileExists(OCRDefFile)) then
  begin
...
begin
  ...
  begin
    //lblNotes.Top := cmbOCRScripts.Top + cmbOCRScripts.Height + 11   // Kameleon
    lblNotes.Top := pnlControl.Height * 899 * GlobalPercentage div 100000; // 89,9% // Kameleon
    mmoNotes.Top := pnlControl.Height * 940 * GlobalPercentage div 100000; // 94%   // Kameleon
  end
  else
  begin
    //lblNotes.Top := cmbOCRScripts.Top;        // Kameleon
    lblNotes.Top := pnlControl.Height * 773 * GlobalPercentage div 100000; // 77.3%  // Kameleon
    mmoNotes.Top := pnlControl.Height * 823 * GlobalPercentage div 100000; // 82.3%  // Kameleon
  end;
  //mmoNotes.Top := lblNotes.Top + lblNotes.Height + 3;  // Kameleon
...
begin
  ...
  // False replaced with mnuShowSubtitles.Checked by adenry - bug fix
end;
...
begin
  ...
  begin
    ...
    for i := 1 to lbl.tag do
    // the Tag property is used to count the number of red labels
    ...
    LengthOfEachLineText := GetLengthForEachLine(GetSWTextEditText(mmo));
    // added later //mmo.Text

    if (Pos(LengthOfEachLineText, lbl.Caption) > 0) and (lbl.Visible) then
    begin
      LabelOriginalText := lbl.Caption; // to restore lbl.Caption later
      lbl.Caption := Copy(lbl.Caption, 1, Pos(LengthOfEachLineText, lbl.Caption)
        - 1); // this will later be used to define the new red label .Left property
      LengthOfEachLineText := LengthOfEachLineText + '=';
      // to end the while loop when there is only one line
      while (True) do
      begin
        num := '';
        i := 1;
        while ((LengthOfEachLineText[i] <> '/') and
          (LengthOfEachLineText[i] <> '=')) do
        begin
        ...
        Delete(LengthOfEachLineText, 1, i);
        // delete the processed numbers plus the following '/'
      end;
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesColumnResize(Sender: TVTHeader;
  Column: TColumnIndex);
var
  w: Integer;
begin
  ...
  begin
    if WindowState <> wsMaximized then
    begin
      w := (pnlParent2.Left - pnlControl.Left) *
        Integer(mnuShowLeftPanel.Checked) + lstSubtitles.Header.Columns[0].Width
        * Integer(mnuColNum.Checked) + lstSubtitles.Header.Columns[1].Width *
        Integer(mnuColShow.Checked) + lstSubtitles.Header.Columns[2].Width *
        Integer(mnuColHide.Checked) + lstSubtitles.Header.Columns[3].Width *
        Integer(mnuColText.Checked) + lstSubtitles.Header.Columns[4].Width *
        Integer(mnuColTranslation.Checked) + lstSubtitles.Header.Columns[5]
        .Width * Integer(mnuColDuration.Checked) + lstSubtitles.Header.Columns
        [6].Width * Integer(mnuColPause.Checked) +
        GetSystemMetrics(SM_CXVSCROLL) + 2 *
        ((2 * Integer(lstSubtitles.BorderStyle)) + lstSubtitles.BorderWidth) + 2
        * pnlControl.Left;
    ...
    begin
      w := lstSubtitles.Width - lstSubtitles.Header.Columns[0].Width *
        Integer(mnuColNum.Checked) - lstSubtitles.Header.Columns[1].Width *
        Integer(mnuColShow.Checked) - lstSubtitles.Header.Columns[2].Width *
        Integer(mnuColHide.Checked) - lstSubtitles.Header.Columns[5].Width *
        Integer(mnuColDuration.Checked) - lstSubtitles.Header.Columns[6].Width *
        Integer(mnuColPause.Checked) - GetSystemMetrics(SM_CXVSCROLL) - 2 *
        ((2 * Integer(lstSubtitles.BorderStyle)) + lstSubtitles.BorderWidth);
      if mnuTranslatorMode.Checked then
      begin
        if Column = 3 then
          lstSubtitles.Header.Columns[4].Width :=
            w - lstSubtitles.Header.Columns[3].Width
        else if Column = 4 then
          lstSubtitles.Header.Columns[3].Width :=
            w - lstSubtitles.Header.Columns[4].Width
        else
        begin
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesAfterColumnWidthTracking(Sender: TVTHeader;
  Column: TColumnIndex);
var
  Ini: TIniFile;
begin
  ...
  if mnuTranslatorMode.Checked then
    TextTransColsRatio := lstSubtitles.Header.Columns[3].Width /
      (lstSubtitles.Header.Columns[3].Width + lstSubtitles.Header.Columns
      [4].Width);
end;
// added by adenry: end
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesBeforeColumnWidthTracking(Sender: TVTHeader;
  Column: TColumnIndex; Shift: TShiftState);
begin
  if SmartResize then
  begin
    lstSubtitles.ScrollBarOptions.ScrollBars := ssVertical;
    CallLstSubtitlesColumnResize;
    // resize the moment the column separator is clicked
...
begin
  ...
  if lstSubtitles.Focused then
    selColor := lstSubtitles.Colors.FocusedSelectionColor
  else
    selColor := lstSubtitles.Colors.UnfocusedSelectionColor;
  if vsSelected in Node.States then
    PaintCellBackground(TargetCanvas, CellRect,
      MixColors(selColor, lstSubtitles.Color, 130 - SelTransp))
  else
  ...
  if not MarkingHighPriority then
    if Data.Marked and MarkingWithBckgr then
      if ((Column = 0) and MarkingColNum) or ((Column = 1) and MarkingColShow)
        ...
        if vsSelected in Node.States then
          PaintCellBackground(TargetCanvas, CellRect,
            MixColors(MarkingBckgr, lstSubtitles.Color, 100 - SelTransp))
        else
          PaintCellBackground(TargetCanvas, CellRect, MarkingBckgr);

  // ERROR MARKING
  if (Data.ErrorType <> []) and (Column in [0, 1, 2, 3, 5, 6])
  { and (not UseOriginalBackground) } then
  begin
    // OLD error marking
            ...
            if not((mfColNum in MarkFlags[i]) or (mfColShow in MarkFlags[i]) or
              ...
              begin
                if (MarkBackgrounds[0] <> lstSubtitles.Color) then
                  if vsSelected in Node.States then
                    PaintCellBackground(TargetCanvas, CellRect,
                      MixColors(selColor, MarkBackgrounds[0], 100 - SelTransp))
                  else
                    PaintCellBackground(TargetCanvas, CellRect,
                      MarkBackgrounds[0]);
              end;
        ...
        begin
          ...
          begin
            if (MarkBackgrounds[i] <> lstSubtitles.Color) then
              if vsSelected in Node.States then
                PaintCellBackground(TargetCanvas, CellRect,
                  MixColors(selColor, MarkBackgrounds[i], 100 - SelTransp))
              else
  ...
  begin
    if (Data.Text = Data.Translation) or (Data.Translation = '') or
      (RemoveSWTags(Data.Translation, True, True, True, True) = UntranslatedSub)
    then // RemoveSWTags added by adenry
    begin
      PaintCellBackground(TargetCanvas, CellRect, UnTransSubsBckgr);
      if vsSelected in Node.States then
        PaintCellBackground(TargetCanvas, CellRect,
          MixColors(selColor, UnTransSubsBckgr, 100 - SelTransp));
    end;
  end;
...
// added by adenry: begin
procedure TfrmMain.PaintCellBackground(TargetCanvas: TCanvas; CellRect: TRect;
  Color: TColor);
var
  tmpStyle: TPenStyle;
  tmpColor: TColor;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((NewTime < Cardinal(TooShortDuration)) or (NewTime > MAX_TIME)) and
        (IndicTooShortDuration) then // Cardinal cast added by adenry
      begin
        ...
        if (NewTime > Cardinal(TooLongDuration)) and (IndicTooLongDuration) then
        // Cardinal cast added by adenry
        begin
    ...
    if (tmeDuration.Time > MAX_TIME) and (FormatType = ftTime) then
      tmeDuration.Time := 0;
    // the Show or Hide values of the subtitle are not changed, this is just for viewing and editing convenience
  end;
end;
// added by adenry: end
...
begin
  ...
  begin
    ...
    begin
      lines := 1 + Length(lbl.Caption) -
        Length(ReplaceString(lbl.Caption, '/', ''));
    ...
    if (lines > 0) and ((tmpLabel <> lblTranslationLines) or
      (mnuTranslatorMode.Checked)) then // added later
...
begin
  ...
  lblTextLines.Left := mmoSubtitleText.Left + mmoSubtitleText.Width -
    lblTextLines.Width;
  lblTranslationLines.Left := mmoTranslation.Left + mmoTranslation.Width -
    lblTranslationLines.Width;
...
begin
  ...
  begin
    ...
    begin
      ...
      pnlParent2.Top := pnlControl.Left * 2 +
        (pnlControl.Left * 2 * Integer(tbMainToolBar.Visible));
      pnlParent2.Height := pnlEditingControls.Top - pnlParent2.Top - 1;
      // pnlParent2.Height := lblText.Top - pnlParent2.Top - 7;
    ...
    begin
      ...
      SetWindowPos(frmMain.Handle, HWND_TOPMOST, 0, 0, Screen.Width,
        Screen.Height, SWP_FRAMECHANGED);
...
// added by adenry: begin
procedure TfrmMain.PlayerShortcutsKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Data: PSubtitleItem;
  Node: PVirtualNode;
begin
  if ShortCut(Key, Shift) = mnuStop.ShortCut then
    mnuStopClick(Sender)
  else if ShortCut(Key, Shift) = mnuRewind.ShortCut then
    mnuRewindClick(Sender)
  else if ShortCut(Key, Shift) = mnuForward.ShortCut then
    mnuForwardClick(Sender)
  else if ShortCut(Key, Shift) = mnuBack5Sec.ShortCut then
    mnuBack5SecClick(Sender)
  else if ShortCut(Key, Shift) = mnuFwd5Sec.ShortCut then
    mnuFwd5SecClick(Sender)
  else if ShortCut(Key, Shift) = DefAltPlayRateShortcut then
    btnAlterPlaybackRateClick(Sender)
  else
  ...
  begin
    ...
    case Key of
      ...
      VK_RETURN:
        ToggleFullScreen;
      VK_LEFT:
        if not((ssAlt in Shift) or (ssCtrl in Shift) or (ssShift in Shift)) and
          not btnRew.Down then
        begin
          btnRew.Down := True;
          btnRewMouseDown(Sender, mbLeft, Shift, 0, 0);
        end;
      VK_RIGHT:
        if not((ssAlt in Shift) or (ssCtrl in Shift) or (ssShift in Shift)) and
          not btnForward.Down then
        begin
          btnForward.Down := True;
          btnForwardMouseDown(Sender, mbLeft, Shift, 0, 0);
        end;
      VK_UP:
      ...
      VK_SPACE:
        ...
        begin
          ...
          Pause;
      Ord('B'), Ord('O'):
        if Assigned(Node) and subSubtitle.Visible and (subSubtitle.Text <> '')
        then
          SetNodeTag(Node, '<b>', '</b>', mnuTranslatorMode.Checked and
            mnuDisplayTranslation.Checked);
      Ord('I'):
        if Assigned(Node) and subSubtitle.Visible and (subSubtitle.Text <> '')
        then
          SetNodeTag(Node, '<i>', '</i>', mnuTranslatorMode.Checked and
            mnuDisplayTranslation.Checked);
      Ord('U'):
        if Assigned(Node) and subSubtitle.Visible and (subSubtitle.Text <> '')
        then
          SetNodeTag(Node, '<u>', '</u>', mnuTranslatorMode.Checked and
            mnuDisplayTranslation.Checked);
      Ord('C'), Ord('Y'):
        if Assigned(Node) and subSubtitle.Visible and (subSubtitle.Text <> '')
        then
          SetNodeColor(Node, dlgColor.Color, mnuTranslatorMode.Checked and
            mnuDisplayTranslation.Checked, True, True);
      VK_BACK:
        if Assigned(Node) and subSubtitle.Visible and (subSubtitle.Text <> '')
        then
          RemoveNodeTags(Node, mnuTranslatorMode.Checked and
            mnuDisplayTranslation.Checked);
    end;
  end;
end;
...
// added by adenry: begin
procedure TfrmMain.PlayerShortcutsKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_LEFT:
      if btnRew.Down then
      begin
        btnRewMouseUp(Sender, mbLeft, Shift, 0, 0);
        btnRew.Down := False;
      end;
    VK_RIGHT:
      if btnForward.Down then
      begin
        btnForwardMouseUp(Sender, mbLeft, Shift, 0, 0);
        btnForward.Down := False;
      end;
  end;
end;
// added by adenry: end
...
// added by adenry: begin
procedure TfrmMain.pnlVideoDisplayMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if FullScreen and ((Mouse.CursorPos.X <> MouseCoords.X) or
    (Mouse.CursorPos.Y <> MouseCoords.Y)) then
  begin
    pnlVideoDisplay.Cursor := crArrow;
...
begin
  ...
  begin
    ...
    if vol > 0 then
      r := 25 * log10(vol / 100) * 100
    else // 20 or 10 is correct, but with 25 sounds smoother
      r := -10000;
    i := Round(r);
  ...
  if vol > 75 then
    tmpSWButton := btnMute100
  else if vol > 50 then
    tmpSWButton := btnMute075
  else if vol > 25 then
    tmpSWButton := btnMute050
  else if vol > 0 then
    tmpSWButton := btnMute025
  else
    tmpSWButton := btnMute000;
  if btnMute.MouseOver <> tmpSWButton.MouseOver then
    btnMute.MouseOver := tmpSWButton.MouseOver;
  if btnMute.Disabled <> tmpSWButton.Disabled then
    btnMute.Disabled := tmpSWButton.Disabled;
  if btnMute.Normal <> tmpSWButton.Normal then
    btnMute.Normal := tmpSWButton.Normal;
  if btnMute.Pressed <> tmpSWButton.Pressed then
    btnMute.Pressed := tmpSWButton.Pressed;
...
// added by adenry: begin
procedure TfrmMain.subSubtitleMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
// added by adenry: begin
procedure TfrmMain.subSubtitleMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  ...
  begin
    ...
    // added by adenry later:
    if FullScreen and ((Mouse.CursorPos.X <> MouseCoords.X) or
      (Mouse.CursorPos.Y <> MouseCoords.Y)) and not subSubtitleDragging then
    begin
      pnlVideoDisplay.Cursor := crArrow;
      subSubtitle.Cursor := crArrow;
      Player.VideoWindow.HideCursor(False);
...
// added by adenry: begin
procedure TfrmMain.subSubtitleMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  H: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      if KeepSubOnVideo then
        subSubtitleTopShift := (subSubtitle.Top + subSubtitle.Height -
          ((pnlVideoDisplay.Height - H) div 2)) / H
      else
        subSubtitleTopShift := (pnlVideoDisplay.Height - subSubtitle.Top -
          subSubtitle.Height) / H;
...
// added by adenry: begin
procedure TfrmMain.AutoRecheckNodeErrors(Node: PVirtualNode;
  ErrorTypesToCheck: TErrorTypeSet = []; RecheckType: TRecheckTypes = rtNormal;
  FormOnly: Boolean = False);
begin
  if AutoRecheckErrors then
  begin
    if FormOnly = False then
    // we recheck only the Info&Errors form when: 1. The Node's marking was changed; 2. The Node was deleted; 3. The Node's Data.ErrorType was restored by Undo
...
// added later:
procedure TfrmMain.AutoRecheckNodeErrorsAndRepaint(Node: PVirtualNode;
  ErrorTypesToCheck: TErrorTypeSet = []; RecheckType: TRecheckTypes = rtNormal;
  FormOnly: Boolean = False);
begin
...
begin
  ...
  begin
    ...
    lstSubtitles.Refresh; // added by adenry later
end;
// added by adenry: end
...
procedure TfrmMain.mnuAudioStreamPopupClick(Sender: TObject);
begin
...
// added by adenry: begin
procedure TfrmMain.tmeShowMouseWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
...
begin
  if FormatType = ftTime then
    step := TTimeMaskEdit(Sender).TimeStep
  else
  ...
  begin
    NextSelNode := lstSubtitles.GetNextSelected(Node);
    ChangeShowTime(Node, GetStartTime(Node) - step, Assigned(NextSelNode),
      tmrTimesMouseWheelUndoBind.Enabled);
    Node := NextSelNode;
  end;
  tmrTimesMouseWheelUndoBind.Enabled := False;
  ...
  begin
    if Sender <> tmeShow then
      TTimeMaskEdit(Sender).Time := GetStartTime(lstSubtitles.GetFirstSelected);
    // added later
...
// added by adenry: begin
procedure TfrmMain.tmeShowMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
  Node, NextSelNode: PVirtualNode; // added later
...
// added by adenry: begin
procedure TfrmMain.tmeHideMouseWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
  Node, NextSelNode: PVirtualNode; // added later
begin
  ...
  begin
    NextSelNode := lstSubtitles.GetNextSelected(Node);
    ChangeHideTime(Node, GetFinalTime(Node) - step, Assigned(NextSelNode),
      tmrTimesMouseWheelUndoBind.Enabled);
    Node := NextSelNode;
  end;
  tmrTimesMouseWheelUndoBind.Enabled := False;
  ...
  if lstSubtitles.SelectedCount = 1 then // added later
  begin
    if Sender <> tmeHide then
      TTimeMaskEdit(Sender).Time := GetFinalTime(lstSubtitles.GetFirstSelected);
    // added later
    TTimeMaskEdit(Sender).SelStart := 7;
    TTimeMaskEdit(Sender).SelLength := 0;
  end;
...
// added by adenry: begin
procedure TfrmMain.tmeHideMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
  Node, NextSelNode: PVirtualNode; // added later
...
// added by adenry: begin
procedure TfrmMain.tmeDurationMouseWheelDown(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
...
begin
  ...
  begin
    ...
    if Duration >= step then // avoid setting negative duration
      ChangeDuration(Node, Duration - step, Assigned(NextSelNode),
        tmrTimesMouseWheelUndoBind.Enabled)
    else
      ChangeDuration(Node, 0, Assigned(NextSelNode),
        tmrTimesMouseWheelUndoBind.Enabled);
    Node := NextSelNode;
  end;
  tmrTimesMouseWheelUndoBind.Enabled := False;
  ...
  if lstSubtitles.SelectedCount = 1 then // added later
  begin
    if Sender <> tmeDuration then
      TTimeMaskEdit(Sender).Time := GetFinalTime(lstSubtitles.GetFirstSelected)
        - GetStartTime(lstSubtitles.GetFirstSelected); // added later
    TTimeMaskEdit(Sender).SelStart := 7;
    TTimeMaskEdit(Sender).SelLength := 0;
  end;
...
// added by adenry: begin
procedure TfrmMain.tmeDurationMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
  Duration: Cardinal; // added later
...
// added by adenry: begin
procedure TfrmMain.tmePauseMouseWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
...
begin
  ...
  begin
    ...
    if Pause >= step then // avoid setting a negative pause
      ChangePause(Node, Pause - step, Assigned(NextSelNode),
        tmrTimesMouseWheelUndoBind.Enabled)
    else
      ChangePause(Node, 0, Assigned(NextSelNode),
        tmrTimesMouseWheelUndoBind.Enabled);
    Node := NextSelNode;
  end;
  tmrTimesMouseWheelUndoBind.Enabled := False;
  ...
  if lstSubtitles.SelectedCount = 1 then // added later
  begin
    if Sender <> tmePause then
      TTimeMaskEdit(Sender).Time := GetPause(lstSubtitles.GetFirstSelected);
    // added later
    TTimeMaskEdit(Sender).SelStart := 7;
    TTimeMaskEdit(Sender).SelLength := 0;
  end;
...
// added by adenry: begin
procedure TfrmMain.tmePauseMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
var
  step: Cardinal;
...
begin
  ...
  begin
    ...
    if Pause > 0 then
      ...
      ChangePause(Node, step, Assigned(NextSelNode),
        tmrTimesMouseWheelUndoBind.Enabled);
    Node := NextSelNode;
  end;
  tmrTimesMouseWheelUndoBind.Enabled := False;
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesMouseWheel(Sender: TObject; Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  // added later: begin
  ...
  begin
    ...
    begin // WHEEL UP
      ...
      if PressedKeyInLstSubtitles = 3 then
        tmeDurationMouseWheelUp(tmeDuration, Shift, MousePos, Handled);
    end
    else
    begin // WHEEL DOWN
      if PressedKeyInLstSubtitles = 1 then
  ...
  else if (ssLeft in Shift) and Assigned(lstSubtitles.FocusedNode) and
    (lstSubtitles.SelectedCount = 1) and
    (((MousePos.X = ClickPoint.X) and (MousePos.Y = ClickPoint.Y)) or
    (toDisableDrawSelection in lstSubtitles.TreeOptions.SelectionOptions)) then
  begin
    if WheelDelta > 0 then
    begin // WHEEL UP
      if lstSubtitles.FocusedColumn = 1 then
        tmeShowMouseWheelUp(tmeShow, Shift, MousePos, Handled);
      if lstSubtitles.FocusedColumn = 2 then
        tmeHideMouseWheelUp(tmeHide, Shift, MousePos, Handled);
      if lstSubtitles.FocusedColumn = 5 then
        tmeDurationMouseWheelUp(tmeDuration, Shift, MousePos, Handled);
      if lstSubtitles.FocusedColumn = 6 then
        tmePauseMouseWheelUp(tmePause, Shift, MousePos, Handled);
    end
    else
    begin // WHEEL DOWN
      if lstSubtitles.FocusedColumn = 1 then
      ...
      if lstSubtitles.FocusedColumn = 5 then
        tmeDurationMouseWheelDown(tmeDuration, Shift, MousePos, Handled);
      if lstSubtitles.FocusedColumn = 6 then
        tmePauseMouseWheelDown(tmePause, Shift, MousePos, Handled);
    end;
  end
  else
    // added later: end
...
// added by adenry: begin
procedure TfrmMain.RemoveNodeTags(Node: PVirtualNode; Translation: Boolean;
  BindUndoToNext: Boolean = False);
var
  UndoAction: PUndoAction;
  OldText: String;
...
// added by adenry: begin
procedure TfrmMain.RemoveAllTags(mmo: TSWTextEdit);
// TMemo changed to TSWTextEdit
var
  Node: PVirtualNode;
  { OldText    : String;
    NewText    : String;
...
begin
  ...
  while Assigned(Node) do
  begin
    RemoveNodeTags(Node, mmo = mmoTranslation,
      Assigned(lstSubtitles.GetNextSelected(Node)));
    Node := lstSubtitles.GetNextSelected(Node);
    {
      New(UndoAction);
      UndoAction^.UndoActionType := uaFullTextChange;
...
begin
  ...
  else if Sender = tbRemoveAllTagsTrans then
    RemoveAllTags(mmoTranslation)
  else if (lstSubtitles.FocusedColumn = 4) or (mmoTranslation.Focused) then
    RemoveAllTags(mmoTranslation)
  else
    RemoveAllTags(mmoSubtitleText);
...
begin
  ...
  Data := Sender.GetNodeData(Node);
  if (Column = 3) or (Column = 4) then
    if ShowCPSHintBoxes or (AdvancedErrorMarking { and MarkErrorsInList } and
      (etTooMuchCPS in Data.ErrorType) and MarkCPSHintBox and (Column = 3)) then
    begin
      if Column = 3 then
        SubText := Data.Text
      else
      ...
      begin
        CurCPSDouble := GetSubCPS(Node);
        // (Length(ReplaceString(RemoveSWTags(SubText,True,True,True,True),#13#10,'')))/((Data.FinalTime-Data.InitialTime)/1000);
      ...
      begin
        GoodChars :=
          Math.Floor((TooMuchCPS * (Data.FinalTime - Data.InitialTime) / 1000));
        if GoodChars < 0 then
          Text := '     ' + CharString
        else
        begin
          Text := IntToStr(GoodChars) + ' ' + CharString;
          if GoodChars < 10 then
            Text := '  ' + Text;
        end;
      end;
      // time/frames
      ...
      begin
        GoodTime :=
          Trunc(Math.SimpleRoundTo((Length(ReplaceString(RemoveSWTags(SubText,
          True, True, True, True), #13#10, '')) / TooMuchCPS), -3) * 1000);
        if Text <> '' then
          Text := ' / ' + Text;
        if FormatType = ftTime then
        begin
        ...
        begin
          ...
          if GoodFrames < 100 then
            Text := '  ' + Text;
          if GoodFrames < 10 then
            Text := '  ' + Text;
        end;
      end;
      // current CPS
      ...
      begin
        ...
        begin
          ...
          CurPercent := Math.Ceil(100 * (CurCPSDouble / TooMuchCPS));
          // fixed later
          Text := IntToStr(CurPercent) + '%' + Text;
          if CurPercent < 100 then
            Text := '  ' + Text;
          if CurPercent < 10 then
            Text := '  ' + Text;
        end;
      end;
      ...
      r.Bottom := r.Bottom +
        Round((lstSubtitles.DefaultNodeHeight - r.Bottom) / 2);
      r.Left := CellRect.Right - (r.Right - r.Left) - 4;
      r.Right := CellRect.Right - 4;

      // DRAW RECTANGLE AND TEXT
      // set colors
      if ((etTooMuchCPS in Data.ErrorType) and MarkCPSHintBox and (Column = 3))
        or ((Column = 4) and (CurCPS > TooMuchCPS)) then
      begin
        TargetCanvas.Brush.Color := TooMuchCPSBckgr;
        // rectangle background color
        TargetCanvas.Font.Color := TooMuchCPSColor; // text color
      end
      else
      begin
        TargetCanvas.Brush.Color := NormalCPSBckgr;
        // rectangle background color
...
begin
  if Visible then
    lstSubtitles.Header.Columns[Col].Options := lstSubtitles.Header.Columns[Col]
      .Options + [coVisible]
  else
    lstSubtitles.Header.Columns[Col].Options := lstSubtitles.Header.Columns[Col]
      .Options - [coVisible];
  case Col of
    0:
      begin
        mnuColNum.Checked := Visible;
        mnuColNumPopup.Checked := Visible;
      end;
    1:
      begin
        mnuColShow.Checked := Visible;
        mnuColShowPopup.Checked := Visible;
      end;
    2:
      begin
        mnuColHide.Checked := Visible;
        mnuColPausePopup.Checked := Visible;
      end;
    3:
      begin
        mnuColText.Checked := Visible;
        mnuColTextPopup.Checked := Visible;
      end;
    5:
      begin
        mnuColDuration.Checked := Visible;
        mnuColDurationPopup.Checked := Visible;
      end;
    6:
      begin
        mnuColPause.Checked := Visible;
        mnuColPausePopup.Checked := Visible;
      end;
  end;
  FormResize(nil);
...
begin
  ...
  begin
    ...
    tempText := ReplaceString(RemoveSWTags(Text, True, True, True,
      True), ' ', '');
    ...
    begin
      ...
      if (Copy(tempText, 1, 2) = '..') or (Copy(tempText, 1, 3) = '-..') or
        ((OrgCharset in DashCharsets1) and (Copy(tempText, 1, 3) = #150'..')) or
        ((OrgCharset in DashCharsets1) and (Copy(tempText, 1, 3) = #151'..')) or
        ((OrgCharset in DashCharsets2) and (Copy(tempText, 1, 3) = #173'..'))
      then
      begin
        PFullTextChange(UndoAction^.Buffer)^.OriginalOnly := False;
        PFullTextChange(UndoAction^.Buffer)^.OldTrans := Text;
...
begin
  ...
  tbTransStyle.Visible := mnuTextStyleBars.Checked and
    mnuTranslatorMode.Checked; // and mnuTranslatorMode.Checked added later
  lblText.Left := mmoSubtitleText.Left +
    (Integer(tbTextStyle.Visible) * tbTextStyle.Width);
  lblTranslation.Left := mmoTranslation.Left +
    (Integer(tbTransStyle.Visible) * tbTransStyle.Width);
...
procedure TfrmMain.tbNewSubtitleMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuNewSubtitleClick(mnuNewSubtitle);
end;

procedure TfrmMain.tbLoadSubtitleMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if tbLoadSubtitle.tag = 0 then
    mnuLoadSubtitleClick(mnuLoadSubtitle)
  else
    tbLoadSubtitle.tag := 0;
end;

procedure TfrmMain.tbSaveFileMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuSaveFileClick(mnuSaveFile);
end;

procedure TfrmMain.tbUndoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if tbUndo.tag = 0 then
  // for proper button behaviour! Don't perform click if we are just hiding the dropdown!
    mnuUndoClick(mnuUndo)
  else
    tbUndo.tag := 0;
end;

procedure TfrmMain.tbRedoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if tbRedo.tag = 0 then
  // for proper button behaviour! Don't perform click if we are just hiding the dropdown!
...
procedure TfrmMain.tbCutMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.tbCopyMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.tbPasteMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.tbSearchMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuSubSearchClick(mnuSubSearch);
end;

procedure TfrmMain.tbSettingsMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuSubSettingsClick(mnuSubSettings);
end;

procedure TfrmMain.tbPascalScriptsMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuPascalScriptClick(mnuPascalScripts);
end;

procedure TfrmMain.tbTranslatorModeMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  mnuTranslatorModeClick(mnuTranslatorMode);
end;

procedure TfrmMain.tbInfoErrorsMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.tbVariousInfoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if not Assigned(frmVariousInfo) then
    frmVariousInfo := TfrmVariousInfo.Create(Application);
  if frmVariousInfo.Visible then
    frmVariousInfo.Close
  else
    frmVariousInfo.Show;
end;

procedure TfrmMain.tbInfoErrorsSettingsMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  mnuInfoErrorsSettingsClick(mnuInfoErrorsSettings);
end;

procedure TfrmMain.tbVideoPreviewModeMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  mnuVideoPreviewModeClick(mnuVideoPreviewMode);
end;

procedure TfrmMain.tbSpellCheckMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Button = mbRight then
    SpellCheck($00000409)
...
procedure TfrmMain.tbTimingsClick(Sender: TObject);
begin;
end;

procedure TfrmMain.tbTimingsTextsSubtitlesMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  if Enabled = False then
  begin
    if UndoList.Count < UndoNumWhenOrgSaved then
      UndoNumWhenOrgSaved := -1;
    if UndoList.Count < UndoNumWhenTransSaved then
      UndoNumWhenTransSaved := -1;
  end;
end;
// added by adenry: end
...
// added by adenry: begin
function TfrmMain.GetUndoActionNameString(UndoActionName
  : TUndoActionName): String;
begin
  Result := '?';
  case UndoActionName of
    // Base subtitle edit
    uanTextEdit:
      Result := lstSubtitles.Header.Columns[3].CaptionText;
    uanTransEdit:
      Result := lstSubtitles.Header.Columns[4].CaptionText;
    uanShowEdit:
      Result := lstSubtitles.Header.Columns[1].CaptionText;
    uanHideEdit:
      Result := lstSubtitles.Header.Columns[2].CaptionText;
    uanPauseEdit:
      Result := lstSubtitles.Header.Columns[6].CaptionText;
    uanDurEdit:
      Result := lstSubtitles.Header.Columns[5].CaptionText;
    // FPS edit
    uanInputFPSEdit:
      Result := ReplaceString(lblInputFPS.Caption, ':', '');
    uanFPSEdit:
      Result := ReplaceString(lblFPS.Caption, ':', '');
    // Text style edit
    uanItalic:
      Result := mnuItalicPopup.Caption;
    uanBold:
      Result := mnuBoldPopup.Caption;
    uanUnderline:
      Result := mnuUnderlinePopup.Caption;
    uanColor:
      Result := mnuSetColorPopup.Caption;
    uanRemoveColor:
      Result := mnuRemoveColorTagsPopup.Caption;
    uanRemoveAllTags:
      Result := mnuRemoveAllTagsPopup.Caption;
    // Edit menu
    uanInsert:
      Result := mnuInsertSubtitle.Caption;
    uanInsertBefore:
      Result := mnuInsertBefore.Caption;
    uanRemoveSelected:
      Result := mnuRemoveSelected.Caption;
    uanCut:
      Result := mnuCut.Caption;
    uanPaste:
      Result := mnuPaste.Caption;
    uanSwap:
      Result := mnuSwap.Caption;
    // Edit menu -> Timings
    uanSetDurLimits:
      Result := mnuSetDurationLimits.Caption;
    uanSetDelay:
      Result := mnuSetDelay.Caption;
    uanAdjustSubs:
      Result := mnuAdjustSubtitles.Caption;
    uanAdjustToSynchSubs:
      Result := mnuAdjustToSyncSubs.Caption;
    uanTimeExpRed:
      Result := mnuTimeExpanderReducer.Caption;
    uanExtLen:
      Result := mnuExtendLength.Caption;
    uanExtLen2:
      Result := mnuExtendLength2.Caption;
    uanAutoDur:
      Result := mnuAutomaticDurations.Caption;
    uanReadTimeFromFile:
      Result := mnuReadTimesFromFile.Caption;
    uanSetPauses:
      Result := mnuSetPauses.Caption;
    uanRoundTimeValues:
      Result := mnuRoundTimeValues.Caption;
    uanShiftPlus:
      Result := mnuShiftPlusMS.Caption;
    uanShiftMinus:
      Result := mnuShiftLessMS.Caption;
    uanShiftToNext:
      Result := mnuShiftToNext.Caption; // added later
    uanShiftToPrev:
      Result := mnuShiftToPrevious.Caption; // added later
    // Edit menu -> Texts
    uanSmartLineAdjust:
      Result := mnuSmartLineAdjust.Caption;
    uanFastSmartLineAdj:
      Result := mnuFastSmartLineAdjust.Caption;
    uanConvertCase:
      Result := mnuConvertCase.Caption;
    uanUnbreakSubs:
      Result := mnuUnbreakSubtitles.Caption;
    uanFastUnbreakSubs:
      Result := mnuFastUnbreakSubtitles.Caption;
    uanDivideLines:
      Result := mnuDivideLines.Caption;
    uanFastDivideLines:
      Result := mnuFastDivideLines.Caption;
    uanSetMaxLineLength:
      Result := mnuSetMaxLineLength.Caption;
    uanReadTextsFromFile:
      Result := mnuReadTextsFromFile.Caption;
    // Edit menu -> Subtitles
    uanAutoCombineSubs:
      Result := mnuAutoCombineSubtitles.Caption;
    uanCombineSubs:
      Result := mnuCombineSubtitles.Caption;
    uanTypeEffect:
      Result := mnuTypeEffect.Caption;
    uanFlashEffect:
      Result := mnuFlash.Caption;
    uanRTLReverseText:
      Result := mnuReverseText.Caption;
    uanRTLFixPunctuation:
      Result := mnuFixPunctuation.Caption;
    uanSort:
      Result := mnuSort.Caption;
    uanDelUnnecessaryLinks:
      Result := mnuDeleteUnnecessaryLinks.Caption;
    uanDelDotsAtBeginning:
      Result := mnuDeleteDotsAtBeginning.Caption;
    uanMark:
      Result := mnuMarkSelectedSubs.Caption;
    uanUnmark:
      Result := mnuUnMarkSelectedSubs.Caption;
    // Search menu & Tools menu
    uanSearchAndReplace:
      Result := mnuSearchAndReplace.Caption;
    uanSpellCheck:
      Result := mnuSpellCheck.Caption;
    uanInfoErrorsFix:
      Result := mnuFixAllErrors.Caption;
    uanFixAllErrors:
      Result := mnuFixAllErrors.Caption;
    uanFixErrorsSelSubs:
      Result := mnuFixErrorsInSelSubs.Caption;
    uaPascalScript:
      Result := mnuPascalScripts.Caption;
    // Movie menu
    uanMovieMoveSubtitle:
      Result := mnuMoveSubtitle.Caption;
    uanMovieSetStartTime:
      Result := mnuSetShowTime.Caption;
    uanMovieSetFinalTime:
      Result := mnuSetHideTime.Caption;
    uanMovieStartEndSubTime:
      Result := mnuEndSubtitle.Caption;
    uanAdjustSubsSyncPoints:
      Result := mnuLastSyncPoint.Caption;
  end;
...
begin
  tbUndo.tag := 1;
  // for proper button behaviour! Next MouseUp on the button should not click it, just hide the dropdown!
  ...
  begin
    ...
    begin
      ...
      begin
        if MiMenu.Activo then
          MiMenu.Activo := True; // refresh the XP style
        exit;
      end;
      UndoAction := UndoList.Items[UndoList.Count - j];
      j := j + 1;
      if UndoAction^.BindToNext = False then
        break;
    end;
...
procedure TfrmMain.tbUndoMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  tbUndo.tag := 0;
end;

procedure TfrmMain.tbRedoMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  tbRedo.tag := 0;
end;

procedure TfrmMain.tbLoadSubtitleMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  tbLoadSubtitle.tag := 0;
end;
...
// added by adenry: begin
procedure TfrmMain.MiHintShowHint(var HintStr: String; var CanShow: Boolean;
  var HintInfo: THintInfo);
var
  UndoAction: PUndoAction;
begin
  ...
  begin
    ...
    if Assigned(UndoAction) then
      HintStr := HintStr + ' (' + GetUndoActionNameString
...
// added by adenry: begin
procedure TfrmMain.SetNodeTag(Node: PVirtualNode; tag, closeTag: String;
  Translation: Boolean; UndoName: TUndoActionName = uanTextEdit;
  BindUndoToNext: Boolean = False; ForceSet: Boolean = False);
var
  UndoAction: PUndoAction;
...
begin
  ...
  else if Pos('<c:#', tag) > 0 then
    UndoName := uanColor
  else if Translation and (UndoName = uanTextEdit) then
    UndoName := uanTransEdit;
  ...
  OriginalText := Text;
  Text := RemoveSWTags(Text, tag = '<b>', tag = '<i>', tag = '<u>',
    Pos('<c:#', tag) > 0, True);
  if (TextHasFullTag(OriginalText, tag = '<b>', tag = '<i>', tag = '<u>',
    Pos('<c:#', tag) > 0) = False) or ForceSet then
  begin
    Text := tag + Text;
    if SubtitleAPI.MultiTagsMode then
      Text := Text + closeTag;
...
// added by adenry: begin
procedure TfrmMain.SetBoldItalicUnderline(Sender: TObject; mmo: TSWTextEdit);
// TMemo changed to TSWTextEdit
var
  tag, closeTag: String;
...
begin
  ...
  begin
    ...
    else if (tbButton = tbItalic) or (tbButton = tbItalicTrans) then
    begin
      tag := '<i>';
      closeTag := '</i>';
      UndoName := uanItalic;
    end
    else if (tbButton = tbUnderline) or (tbButton = tbUnderlineTrans) then
    begin
      tag := '<u>';
      closeTag := '</u>';
      UndoName := uanUnderline;
    ...
    if SubtitleAPI.MultiTagsMode and (lstSubtitles.SelectedCount = 1) and
      (mmo.SelLength > 0) then
    // multiple tags allowed, only one subtitle selected, and selection length in the memo > 0
    begin
      Node := lstSubtitles.GetFirstSelected;
      Text := GetSWTextEditText(mmo);
      Text := StringReplace(Text, #$0a, '', [rfReplaceAll]);
      if mmo.SelStart + mmo.SelLength > Length(Text) then
        mmo.SelLength := Length(Text) - mmo.SelStart;
      // added later - fix RichEdit selection
      ...
      begin
        ...
        tagPos := SmartPos(tag, Text, False, mmo.SelStart + mmo.SelLength + 2 -
          Length(closeTag));
        closeTagPos := SmartPos(closeTag, Text, False,
          mmo.SelStart + mmo.SelLength + 2 - Length(closeTag));
        ...
        // insert close tag
        if insertCloseTag then
          Insert(closeTag, Text, mmo.SelStart + mmo.SelLength + 1);
        // delete all tags of the same type inside the selection
        Delete(Text, mmo.SelStart + 1, mmo.SelLength);
        Insert(RemoveSWTags(mmo.SelText, tag = '<b>', tag = '<i>', tag = '<u>',
          False, True), Text, mmo.SelStart + 1);
        // insert open tag
        if insertTag then
          Insert(tag, Text, mmo.SelStart + 1);
      end
      else
    ...
    begin
      ...
      while Assigned(Node) do
      begin
        SetNodeTag(Node, tag, closeTag, mmo = mmoTranslation, UndoName,
          Assigned(lstSubtitles.GetNextSelected(Node)), tbButton.Down);
        {
          New(UndoAction);
          UndoAction^.UndoActionType := uaFullTextChange;
          UndoAction^.UndoActionName := UndoName;
...
procedure TfrmMain.tbBoldItalicUnderlineMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  SetBoldItalicUnderline(Sender, mmoSubtitleText);
end;

procedure TfrmMain.tbBoldItalicUnderlineTransMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
// added by adenry: begin
procedure TfrmMain.tbSetColorMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  SetColor(True, False, False, True);
end;

procedure TfrmMain.tbRemoveAllTagsMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  mnuRemoveAllTagsClick(tbRemoveAllTags);
end;

procedure TfrmMain.tbSetColorTransMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  SetColor(False, True, False, True);
end;

procedure TfrmMain.tbRemoveAllTagsTransMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.RefreshTagButtonState(openTag, closeTag: String;
  tb: TToolButton; mmo: TSWTextEdit); // TMemo changed to TSWTextEdit
var
...
begin
  ...
  begin
    ...
    posCloseTag2 := SmartPos(closeTag, mmo.Text, False,
      mmo.SelStart + mmo.SelLength);
    posOpenTag2 := SmartPos(openTag, mmo.Text, False,
      mmo.SelStart + mmo.SelLength);
    tagInSelText := (SmartPos(closeTag, mmo.SelText, False) > 0) or
      (SmartPos(openTag, mmo.SelText, False) > 0);
    if (posOpenTag > posCloseTag) and
      ((posCloseTag2 < posOpenTag2) or (posOpenTag2 = 0)) and not tagInSelText
    then
      tb.Down := True
    else
      tb.Down := False;
...
procedure TfrmMain.mmoSubTextMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.mmoSubTextKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  RefreshTagButtonState('<b>', '</b>', tbBold, mmoSubtitleText);
  RefreshTagButtonState('<i>', '</i>', tbItalic, mmoSubtitleText);
...
procedure TfrmMain.mmoTransMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.mmoTransKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  RefreshTagButtonState('<b>', '</b>', tbBoldTrans, mmoTranslation);
  RefreshTagButtonState('<i>', '</i>', tbItalicTrans, mmoTranslation);
...
procedure TfrmMain.reMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if TSWTextEdit.ClassName = TRichEdit.ClassName then
...
procedure TfrmMain.reMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  p: Integer;
  Text: String;
...
begin
  ...
  begin
    ...
    if mmo.SelLength > 0 then
    begin
      while (mmo.SelText[1] in SpecialChars) or
      ...
      while (mmo.SelText[mmo.SelLength] in SpecialChars) or
...
begin
  ...
  begin
    ...
    begin
      ...
      while Assigned(Node) do
      begin
        if TextHasFullTag(GetSubText(Node), True, False, False, False) = False
        then
          tbBold.Down := False;
        ...
        if TextHasFullTag(GetSubText(Node), False, True, False, False) = False
        then
          tbItalic.Down := False;
        ...
        if TextHasFullTag(GetSubText(Node), False, False, True, False) = False
        then
          tbUnderline.Down := False;
        ...
        if (tbBold.Down = False) and (tbItalic.Down = False) and
          (tbUnderline.Down = False) then
          break;
        Node := lstSubtitles.GetNextSelected(Node);
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        while Assigned(Node) do
        begin
          if TextHasFullTag(GetSubTranslation(Node), True, False, False, False)
            = False then
            tbBoldTrans.Down := False;
          ...
          if TextHasFullTag(GetSubTranslation(Node), False, True, False, False)
            = False then
            tbItalicTrans.Down := False;
          ...
          if TextHasFullTag(GetSubTranslation(Node), False, False, True, False)
            = False then
            tbUnderlineTrans.Down := False;
          ...
          if (tbBoldTrans.Down = False) and (tbItalicTrans.Down = False) and
            (tbUnderlineTrans.Down = False) then
            break;
          Node := lstSubtitles.GetNextSelected(Node);
        end;
      end;
...
var
  Text, Text2, Text3, OrigFormat, TransFormat: String;
begin
  if sbStatusbar.Visible then
  begin
    Text := '';
    ...
    begin
      if lstSubtitles.SelectedCount <> 1 then
        Text := Format(NSelectedSubtitles, [lstSubtitles.SelectedCount,
          lstSubtitles.TotalCount])
      else if lstSubtitles.GetFirstSelected <> nil then
      // added later to fix a bug when deleting a subtitle
        Text := Format(NSelectedSubtitle,
          [lstSubtitles.GetFirstSelected.Index + 1, lstSubtitles.TotalCount])
      else
        Text := IntToStr(lstSubtitles.TotalCount);
    end;
    sbStatusbar.Panels[0].Text := Text;

    if UTF8_Orig // Kameleon
    then
      Text2 := '  UTF-8'
    else
      Text2 := '  ANSI';

    if UTF8_Trans // Kameleon
    then
      Text3 := '  UTF-8'
    else
      Text3 := '  ANSI';

    OrigFormat := SubtitleAPI.GetFormatName(frmMain.OrgFormat);
    if OrigFormat > '' then
      OrigFormat := OrigFormat + Text2;
    TransFormat := SubtitleAPI.GetFormatName(frmMain.TransFormat);
    if TransFormat > '' then
      TransFormat := TransFormat + Text3;

    if TextTransColsSwapped = False then
    begin
      sbStatusbar.Panels[1].Text := OrigFormat;
      sbStatusbar.Panels[2].Text := TransFormat;
    end
    else
    begin
      sbStatusbar.Panels[2].Text := OrigFormat;
      sbStatusbar.Panels[1].Text := TransFormat;
    end;
  end;
end;
// added by adenry: end
...
procedure TfrmMain.sbStatusbarMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if X <= sbStatusbar.Panels[0].Width then
    if Button = mbRight then
      CenterListOnLastSelected
    else
      CenterListOnSelected;
end;
// added by adenry: end
...
procedure TfrmMain.tcTimeCounterMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  TheLastActiveControl := Screen.ActiveControl;
...
procedure TfrmMain.tcTimeCounterMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  TheLastActiveControl.SetFocus;
...
procedure TfrmMain.tcTimeCounterMouseWheelDown(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  if ssLeft in Shift then
  begin
...
procedure TfrmMain.tcTimeCounterMouseWheelUp(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  if ssLeft in Shift then
  begin
...
begin
  if Application.Active then
    if (TempActiveControl <> tcTimeCounter) and
      (Screen.ActiveControl.ClassName <> 'TTimeMaskEdit') and
      (PressedKeyInLstSubtitles = 0) then
    // if we haven't clicked on tcTimeCounter and another TTimeMaskEdit is not currently active
    begin
      TempActiveControl := TWinControl(Sender);
      TTimeMaskEdit(Sender).Font.Style := TTimeMaskEdit(Sender).Font.Style +
        [fsUnderline];
    end;
end;
...
begin
  if Application.Active then
    if (TempActiveControl <> tcTimeCounter) and
      (TempActiveControl = TTimeMaskEdit(Sender)) then
    begin
      TempActiveControl := nil;
      TTimeMaskEdit(Sender).Font.Style := TTimeMaskEdit(Sender).Font.Style -
        [fsUnderline];
...
begin
  if TempActiveControl <> nil then
  begin
    if TempActiveControl = tmeHide then
    begin
      tmeHideMouseWheelUp(tmeHide, Shift, MousePos, Handled);
      Handled := True;
    end
    else if TempActiveControl = tmeShow then
    begin
      tmeShowMouseWheelUp(tmeShow, Shift, MousePos, Handled);
      Handled := True;
    end
    else if TempActiveControl = tmeDuration then
    begin
      tmeDurationMouseWheelUp(tmeDuration, Shift, MousePos, Handled);
      Handled := True;
    end
    else if TempActiveControl = tmePause then
    begin
      tmePauseMouseWheelUp(tmePause, Shift, MousePos, Handled);
      Handled := True;
    end
    else if TempActiveControl = sbVolume then
    begin
      VolumeSet(sbVolume.Position + 5);
      Application.ActivateHint(Mouse.CursorPos);
      Handled := True;
    ...
    if (Mouse.CursorPos.X > pnlVideo.ClientToScreen(Point(0, 0)).X) and
      (Mouse.CursorPos.X < pnlVideo.ClientToScreen(Point(0, 0)).X +
      pnlVideo.Width) and
      (Mouse.CursorPos.Y > pnlVideo.ClientToScreen(Point(0, 0)).Y) and
      (Mouse.CursorPos.Y < pnlVideo.ClientToScreen(Point(0, 0)).Y +
      pnlVideo.Height) then
    begin
      VolumeSet(sbVolume.Position + 5);
      Handled := True;
    end;
...
begin
  ...
  begin
    ...
    p.X := 0;
    p.Y := 0;
    if (Mouse.CursorPos.X < sbSeekBar.ClientToScreen(p).X) or
      (Mouse.CursorPos.X > sbSeekBar.ClientToScreen(p).X + sbSeekBar.Width) or
      (Mouse.CursorPos.Y < sbSeekBar.ClientToScreen(p).Y) or
      (Mouse.CursorPos.Y > sbSeekBar.ClientToScreen(p).Y + sbSeekBar.Height)
    then
      if tcTimeCounter.Text2Color <> clWindowText then
      // check if MouseLeave wasn't already triggered
...
begin
  ...
  tmeShow.Width := (pnlTimingControls.Width - 7 -
    Integer(mnuShowTimeControlButtons.Checked) * (2 * tbHideSnap.Width + 2 *
    6)) div 2;
  tmeShow.Left := (tbtnShowSnap.Width + 6) *
    Integer(frmMain.mnuShowTimeControlButtons.Checked);
...
begin
  ...
  pnlTimingControls.Width := tmeHide.Left + tmeHide.Width +
    Integer(mnuShowTimeControlButtons.Checked) * (tbHideSnap.Width + 6);
...
procedure TfrmMain.tbtnShowSnapMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  TToolButton(Sender).Down := False;
end;

procedure TfrmMain.tbtnShowSnapMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Button = mbLeft then
  begin
...
procedure TfrmMain.tbtnHideSnapMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Button = mbLeft then
  begin
...
procedure TfrmMain.tbtnPauseSetMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TfrmMain.tbtnDurationSetMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  MethodIsCPS: Boolean;
  ...
  begin
    ...
    begin
      ...
      if rdoAllCases.Checked then
        ApplyMethod := 1
      else if rdoNewDurGreaterOrg.Checked then
        ApplyMethod := 2
      else if rdoNewDurSmallerOrg.Checked then
        ApplyMethod := 3;
      CPS := udCPS.Position;
      ...
      if ApplyMethod = 1 then
        rdoAllCases.Checked := True
      else if ApplyMethod = 2 then
        rdoNewDurGreaterOrg.Checked := True
      else if ApplyMethod = 3 then
...
begin
  ...
  if Assigned(Node) and Assigned(Node.NextSibling) then
    if (GetFinalTime(Node) + DefaultSubPause < GetStartTime(Node.NextSibling))
    then
    begin
      ...
      begin
        ...
        if lstSubtitles.Selected[Node.NextSibling] = False then
          ShiftValue := GetStartTime(Node.NextSibling) - GetFinalTime(Node) -
            DefaultSubPause;
...
begin
  ...
  if Assigned(Node) and Assigned(Node.PrevSibling) then
    if (GetFinalTime(Node.PrevSibling) + DefaultSubPause < GetStartTime(Node))
    then
    begin
      ShiftValue := 0; // only to avoid compiler warnings
      ...
      begin
        ...
        if lstSubtitles.Selected[Node.PrevSibling] = False then
          ShiftValue := GetStartTime(Node) - GetFinalTime(Node.PrevSibling) -
            DefaultSubPause;
...
begin
  ...
  begin
    ...
    pnlCaret.Height := Abs(mmoSubtitleText.Font.Height +
      mmoSubtitleText.Font.Height div 4);
  end
  else if frmMain.ActiveControl = mmoTranslation then
  begin
    APoint := mmoTranslation.ClientToParent(APoint, frmMain);
    pnlCaret.Height := Abs(mmoTranslation.Font.Height +
      mmoTranslation.Font.Height div 4);
  end
  else if frmMain.ActiveControl = mmoNotes then
  begin
...
function TfrmMain.SetHideSelection(Control: TWinControl;
  HideSelection: Boolean): Boolean;
...
begin
  ...
  begin
    if (HideSelection = False) and (TRichEdit(Control).SelLength = 0) then
      exit;
...
procedure TfrmMain.sdSymbolDialogueInsertSymbol(Sender: TObject; Font: TFont;
  Chr: Char);
begin
  if frmMain.ActiveControl = mmoSubtitleText then
  begin
...
begin
  if Key in SpecialChars then
  // a special character means a word's end, so we stop binding undo actions
    tmrFastTypingUndoBind.Enabled := False;
  if Key = #13 then // don't bind anything after typing a new line
    tmrFastTypingUndoBind.tag := 1
  else
...
// added by adenry: begin
// Video Popup menu:
procedure TfrmMain.pnlVideoDisplayMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  if WindowState = wsMaximized then
  begin
    if (NewWidth <= pnlVideoDisplay.Width) and
      (NewHeight <= pnlParent2.Height - spSplitter.MinSize -
      pnlVideoControls.Height - spSplitter.Height) then
    begin
      VideoPanelRatio := (NewHeight + pnlVideoControls.Height) /
        pnlParent2.Height;
  ...
  begin
    FormHeightDiff := (NewHeight - pnlVideoDisplay.Height) -
      (lstSubtitles.Height - spSplitter.MinSize);
    FormWidthDiff := (NewWidth - pnlVideo.Width);

    if (Height + FormHeightDiff <= Screen.Height) and
      (Width + FormWidthDiff <= Screen.Width) then
    begin
      VideoPanelRatio := (NewHeight + pnlVideoControls.Height) /
        Max(pnlParent2.Height, pnlParent2.Height + FormHeightDiff);
...
procedure TfrmMain.mnuZoomTo25percentClick(Sender: TObject);
begin
  ZoomVideo(25);
end;

procedure TfrmMain.mnuZoomTo50percentClick(Sender: TObject);
begin
  ZoomVideo(50);
end;

procedure TfrmMain.mnuZoomTo75percentClick(Sender: TObject);
begin
  ZoomVideo(75);
end;

procedure TfrmMain.mnuZoomTo100percentClick(Sender: TObject);
begin
  ZoomVideo(100);
end;

procedure TfrmMain.mnuZoomTo150percentClick(Sender: TObject);
begin
  ZoomVideo(150);
end;

procedure TfrmMain.mnuZoomTo200percentClick(Sender: TObject);
begin
  ZoomVideo(200);
end;
// added by adenry: end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        w := Max(1, Rnd(Duration / mult));
        // Max added by adenry, Round replaced with rnd by adenry
...
begin
  ...
  else if mnuTextTransPopup.PopupComponent = mmoTranslation then
  begin
  ...
  mnuCutPopup.Enabled := TSWTextEdit(mnuTextTransPopup.PopupComponent).SelLength
    > 0; // TMemo changed to TSWTextEdit
  mnuCopyPopup.Enabled := TSWTextEdit(mnuTextTransPopup.PopupComponent)
    .SelLength > 0; // TMemo changed to TSWTextEdit
  mnuPastePopup.Enabled := mnuPaste.Enabled;
  mnuDeletePopup.Enabled := TSWTextEdit(mnuTextTransPopup.PopupComponent)
    .SelLength > 0; // TMemo changed to TSWTextEdit
  mnuSelectAllPopup.Enabled := TSWTextEdit(mnuTextTransPopup.PopupComponent)
    .Text <> ''; // TMemo changed to TSWTextEdit
end;

procedure TfrmMain.mnuDeletePopupClick(Sender: TObject);
begin
  TSWTextEdit(TPopupMenu(TMenuItem(Sender).GetParentMenu).PopupComponent)
    .SelText := ''; // TMemo changed to TSWTextEdit
end;
// added by adenry: end
...
procedure TfrmMain.lstSubtitlesChange(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
begin
  if ShouldRefreshTimes then
    if Assigned(Node) // regular selection
      or (GetKeyState(VK_SHIFT) < 0)
    // Shift key is held - SHIFT+HOME, SHIFT+END, etc.
...
begin
  if mnuShowTimeControls.Checked then
  begin
    sbStatusbar.Panels[0].Width := pnlEditingControls.Left +
      pnlTimingControls.Width + 4;
    sbStatusbar.Panels[1].Width := mmoSubtitleText.Width + 4 + 4 *
      Integer(mnuTranslatorMode.Checked = False);
  end
  else
  begin
    sbStatusbar.Panels[0].Width := 213;
    sbStatusbar.Panels[1].Width := mmoSubtitleText.Width - sbStatusbar.Panels[0]
      .Width + (pnlControl.Width + pnlControl.Left) *
      Integer(mnuShowLeftPanel.Checked) + 8 + 4 *
      Integer(mnuTranslatorMode.Checked = False);
  end;
end;
// added by adenry: end
        ...
        try
          ...
          for Ini := 2 to mnuLanguage.Count - 1 do
            ...
            begin
              ...
              lang := TMemIniFile.Create('Langs\' + mnuLanguage.Items[Ini]
                .Caption + '.lng');
              try
                ...
                for sec := 0 to SectionsEng.Count - 1 do // check all sections
                  ...
                  begin
                    ...
                    // check all keys in the section
                      if lang.ValueExists(SectionsEng[sec], KeysEng[sec][Key]) = False
                      then
                      // Key does not exist, so we add it along with its english value
                      begin
                        lang.WriteString(SectionsEng[sec], KeysEng[sec][Key],
                          Copy(ValuesEng[sec][Key],
                          Pos('=', ValuesEng[sec][Key]) + 1, MaxInt));
                  ...
                  begin
                    ...
                    begin
                      lang.WriteString(SectionsEng[sec], KeysEng[sec][Key],
                        Copy(ValuesEng[sec][Key], Pos('=', ValuesEng[sec][Key])
                        + 1, MaxInt));
                      Inc(ChangesCount[Ini]); // Report info
                    end;
                  end;
              ...
              Changes := Changes + mnuLanguage.Items[Ini].Caption + ':' + ' ' +
                IntToStr(ChangesCount[Ini]) + ' ' + KeysStr + #13#10;
              // Add report info
            end;
...
// added by adenry: begin
procedure TfrmMain.mnuAROriginalClick(Sender: TObject);
begin
  SetAspectRatio(-1.0);
end;

procedure TfrmMain.mnuAR4_3Click(Sender: TObject);
begin
  SetAspectRatio(4 / 3);
end;

procedure TfrmMain.mnuAR16_9Click(Sender: TObject);
begin
  SetAspectRatio(16 / 9);
end;

procedure TfrmMain.mnuAR185_1Click(Sender: TObject);
begin
  SetAspectRatio(1.85);
end;

procedure TfrmMain.mnuAR235_1Click(Sender: TObject);
begin
  SetAspectRatio(2.35);
end;
// added by adenry: end
...
begin
  ...
  for i := 0 to lstSubtitles.Header.Columns.Count - 1 do
    lstSubtitles.Header.Columns[i].Options := lstSubtitles.Header.Columns[i]
      .Options + [coAllowFocus];
end;
// added by adenry: end
...
procedure TfrmMain.lstSubtitlesHeaderDragged(Sender: TVTHeader;
...
// added by adenry: begin
procedure TfrmMain.lstSubtitlesHeaderClick(Sender: TVTHeader;
  HitInfo: TVTHeaderHitInfo);
var
  WidthsSum: Integer;
begin
  case HitInfo.Column of
    ...
    3, 4:
      if mnuTranslatorMode.Checked then
      begin
        WidthsSum := lstSubtitles.Header.Columns[3].Width +
          lstSubtitles.Header.Columns[4].Width;
        lstSubtitles.Header.Columns[3].Width := WidthsSum div 2;
        lstSubtitles.Header.Columns[4].Width := WidthsSum -
          lstSubtitles.Header.Columns[3].Width;
...
procedure TfrmMain.CompExecCompile(Sender: TPSScript);
begin
  PascalScriptsFunctions.CompExecCompile(Sender); // Add functions
  Sender.AddRegisteredVariable('Application', 'TApplication');
  Sender.AddRegisteredVariable('frmMain', 'TForm');
end;
