...
uses
  ...
  _CommonTypes, Mask, System.UITypes, Math;
...
type
  TfrmOutputSettings = class(TForm)
    ...
    edtDVDSubtitleDiskId: TEdit;
    lblDVDSubtitleDiskId: TLabel;
    edtDVDSubtitleDVDTitle: TEdit;
    lblDVDSubtitleDVDTitle: TLabel;
    edtDVDSubtitleLanguage: TEdit;
    lblDVDSubtitleLanguage: TLabel;
    edtDVDSubtitleAuthor: TEdit;
    lblDVDSubtitleAuthor: TLabel;
    edtDVDSubtitleWeb: TEdit;
    lblDVDSubtitleWeb: TLabel;
    edtDVDSubtitleInfo: TEdit;
    lblDVDSubtitleInfo: TLabel;
    edtDVDSubtitleLicense: TEdit;
    lblDVDSubtitleLicense: TLabel;
    edtSubViewer1Title: TEdit;
    lblSubViewer1Title: TLabel;
    edtSubViewer1Author: TEdit;
    lblSubViewer1Author: TLabel;
    edtSubViewer1Source: TEdit;
    lblSubViewer1Source: TLabel;
    edtSubViewer1Program: TEdit;
    lblSubViewer1Program: TLabel;
    edtSubViewer1Path: TEdit;
    lblSubViewer1Path: TLabel;
    lblSubViewer2Title: TLabel;
    edtSubViewer2Title: TEdit;
    edtSubViewer2Author: TEdit;
    lblSubViewer2Author: TLabel;
    edtSubViewer2Source: TEdit;
    lblSubViewer2Source: TLabel;
    edtSubViewer2Program: TEdit;
    lblSubViewer2Program: TLabel;
    edtSubViewer2Path: TEdit;
    lblSubViewer2Path: TLabel;
    edtSubViewer2Comment: TEdit;
    lblSubViewer2Comment: TLabel;
  ...
  private
    var
      OrigDims, OrigDimsPnlHeading, OrigDimsFormats,
      OrigDimsAdvancedSubStationAlpha, OrigDimsXAS, OrigDimsDVDSubtitle,
      OrigDimsSAMI, OrigDimsSonicScenarist, OrigDimsSubStationAlpha, OrigDimsSubViewer,
      OrigDimsSubViewer2, OrigDimsTMPlayer: TOrigDims;

      OrigDimsCtrlAss, OrigDimsAssCosmetics, OrigDimsAssOthers, OrigDimsAssAdvanced: TOrigDims;
      OrigDimsXASFont, OrigDimsFont, OrigDimsXasJoin, OrigDimsXASLanguage,
      OrigDimsXASPosition: TOrigDims;

      OrigDimsCtrlSSA, OrigDimsSSACosmetics, OrigDimsSSAOthers,
      OrigDimsTMPlayerMultilineFormat:TOrigDims;
    // Kameleon
...
procedure TfrmOutputSettings.GetDims(Control: TWinControl; var Dims: TOrigDims);
var
  I, P: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      if (Control.Controls[I] is TSpinEdit) then TSpinEdit(Control.Controls[I]).AutoSize := true;
...
procedure TfrmOutputSettings.GetOrigDims; // Kameleon
var
  I, P: Integer;
begin
  ...
  GetDims(pgeAdvancedSubStationAlpha, OrigDimsAdvancedSubStationAlpha);
  GetDims(pgeXAS, OrigDimsXas);
  GetDims(pgeDVDSubtitle, OrigDimsDVDSubtitle);
  GetDims(pgeSAMI    , OrigDimsSAMI);
  GetDims(pgeSonicScenarist     , OrigDimsSonicScenarist);
  GetDims(pgeSubStationAlpha      , OrigDimsSubStationAlpha);
  GetDims(pgeSubViewer       , OrigDimsSubViewer);
  GetDims(pgeSubViewer2    ,OrigDimsSubViewer2);
  GetDims(pgeTMPlayer     , OrigDimsTMPlayer);

  GetDims(pgeCtrlASS, OrigDimsCtrlAss);
  GetDims(pgeASSCosMetics, OrigDimsAssCosmetics);
  GetDims(pgeASSOthers, OrigDimsAssOthers);
  GetDims(pgeASSAdvanced, OrigDimsAssAdvanced);
  GetDims(gbXASFont, OrigDimsXASFont);
  GetDims(pnFont, OrigDimsFont);
  GetDims(gbXASJoin, OrigDimsXasJoin);
  GetDims(gbXASLanguage, OrigDimsXASLanguage);
  GetDims(gbXASPosition, OrigDimsXASPosition);

  GetDims(pgeCtrlSSA, OrigDimsCtrlSSA);
  GetDims(pgeSSACosmetics, OrigDimsSSACosmetics);
  GetDims(pgeSSAOthers, OrigDimsSSAOthers);
  GetDims(gbTMPlayerMultilineFormat, OrigDimsTMPlayerMultilineFormat);
end;

procedure TfrmOutputSettings.ChangeDims(Control: TWinControl; var Dims: TOrigDims);
var
  I: Integer;
  L, T, W, H: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      W := Floor(HFactor * Dims.OrigDimsArr[I, 0]); // width
      H := Floor(VFactor * Dims.OrigDimsArr[I, 1]); // height
      L := Floor(HFactor * Dims.OrigDimsArr[I, 2]); // left
      T := Floor(VFactor * Dims.OrigDimsArr[I, 3]); // top
      ...
      begin
        ...
        TComboBox(Control.Controls[I]).ItemHeight := H - 6;
               ...
               (Control.Controls[I] is TSpinEdit) or
...
procedure TfrmOutputSettings.DoFormResize(Sender: TObject);
begin
  ...
  ChangeDims(pgeFormats, OrigDimsFormats);
  ChangeDims(pgeAdvancedSubStationAlpha, OrigDimsAdvancedSubStationAlpha);
  ChangeDims(pgeXAS, OrigDimsXas);
  ChangeDims(pgeDVDSubtitle, OrigDimsDVDSubtitle);
  ChangeDims(pgeSAMI    , OrigDimsSAMI);
  ChangeDims(pgeSonicScenarist     , OrigDimsSonicScenarist);
  ChangeDims(pgeSubStationAlpha      , OrigDimsSubStationAlpha);
  ChangeDims(pgeSubViewer       , OrigDimsSubViewer);
  ChangeDims(pgeSubViewer2    ,OrigDimsSubViewer2);
  ChangeDims(pgeTMPlayer     , OrigDimsTMPlayer);

  ChangeDims(pgeCtrlASS, OrigDimsCtrlAss);
  ChangeDims(pgeASSCosMetics, OrigDimsAssCosmetics);
  ChangeDims(pgeASSOthers, OrigDimsAssOthers);
  ChangeDims(pgeASSAdvanced, OrigDimsAssAdvanced);
  ChangeDims(gbXASFont, OrigDimsXASFont);
  ChangeDims(pnFont, OrigDimsFont);
  ChangeDims(gbXASJoin, OrigDimsXasJoin);
  ChangeDims(gbXASLanguage, OrigDimsXASLanguage);
  ChangeDims(gbXASPosition, OrigDimsXASPosition);

  ChangeDims(pgeCtrlSSA, OrigDimsCtrlSSA);
  ChangeDims(pgeSSACosmetics, OrigDimsSSACosmetics);
  ChangeDims(pgeSSAOthers, OrigDimsSSAOthers);
  ChangeDims(gbTMPlayerMultilineFormat, OrigDimsTMPlayerMultilineFormat);
end;

procedure TfrmOutputSettings.FormResize(Sender: TObject);
begin
  DoFormResize(Sender);
  DoFormResize(Sender);
end;

procedure TfrmOutputSettings.FormShow(Sender: TObject);
var Ini: TIniFile;
    L, T, W, H: integer;
begin

  Ini := TIniFile.Create(IniRoot);
  W := abs(Ini.ReadInteger('OutputSettings Window', 'Width', 945));
  H := abs(Ini.ReadInteger('OutputSettings Window', 'Height', 670));
  L := abs(Ini.ReadInteger('OutputSettings Window', 'Left', (Screen.Width - W) div 2));
  T := abs(Ini.ReadInteger('OutputSettings Window', 'Top', (Screen.Height - H) div 2));

  SetBounds(L, T, W, H);

  if Ini.ReadBool('OutputSettings Window', 'Maximized', False) then
...
begin
  ...
  try
    ...
    begin
      ...
      // DVD Subtitle
      lblDVDSubtitleDiskId.Caption             := ReadString('Output settings','02','Disk ID:');
      lblDVDSubtitleDVDTitle.Caption           := ReadString('Output settings','03','DVD Title:');
      lblDVDSubtitleLanguage.Caption           := ReadString('Output settings','04','Language:');
      lblDVDSubtitleAuthor.Caption             := ReadString('Output settings','05','Author:');
      lblDVDSubtitleWeb.Caption                := ReadString('Output settings','06','Web:');
      lblDVDSubtitleInfo.Caption               := ReadString('Output settings','07','Info:');
      lblDVDSubtitleLicense.Caption            := ReadString('Output settings','08','License:');
      ...
      // SubViewer
      lblSubViewer1Title.Caption     := edtSSATitle.EditLabel.Caption;
      lblSubViewer1Author.Caption    := lblDVDSubtitleAuthor.Caption;
      lblSubViewer1Source.Caption    := ReadString('Output settings','43','Source:');
      lblSubViewer1Program.Caption   := ReadString('Output settings','44','Program:');
      lblSubViewer1Path.Caption      := ReadString('Output settings','45','Path:');
      lblSubViewer1Delay.Caption     := ReadString('Output settings','46','Delay:');

      // SubViewer 2
      lblSubViewer2Title.Caption     := edtSSATitle.EditLabel.Caption;
      lblSubViewer2Author.Caption    := lblDVDSubtitleAuthor.Caption;
      lblSubViewer2Source.Caption    := lblSubViewer1Source.Caption;
      lblSubViewer2Program.Caption   := lblSubViewer1Program.Caption;
      lblSubViewer2Path.Caption      := lblSubViewer1Path.Caption;
      lblSubViewer2Delay.Caption     := lblSubViewer1Delay.Caption;
      lblSubViewer2CDTrack.Caption   := ReadString('Output settings','47','CD-Track:');
      lblSubViewer2Comment.Caption   := ReadString('Output settings','48','Comment:');
...
begin
  ...
  if clrDlg.Execute then
    TPanel(Sender).Color := clrDlg.Color;
end;
...
begin
  ...
  try
    ...
    begin
      Ini.WriteBool('OutputSettings Window', 'Maximized', True);
    end
    else
    begin
      Ini.WriteInteger('OutputSettings Window', 'Left', Left);
      Ini.WriteInteger('OutputSettings Window', 'Top', Top);
      Ini.WriteInteger('OutputSettings Window', 'Width', Width);
      Ini.WriteInteger('OutputSettings Window', 'Height', Height);
      Ini.WriteBool('OutputSettings Window', 'Maximized', False);
    end;
