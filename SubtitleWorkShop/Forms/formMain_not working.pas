...
type
  TDims = array [0 .. 3] of Integer;
  TDimsArr = Array of TDims;

  TOrigDims = record
    OrigW, OrigH: Integer; // original width, original Height
    OrigDimsArr: TDimsArr;
  end; // record
...
type
  TfrmMain = class(TForm)
    ...
    procedure GetDims(Control: TWinControl; var Dims: TOrigDims); // Dany
    procedure GetOrigDims; // Dany
    procedure ChangeDims(Control: TWinControl; var Dims: TOrigDims); // Dany
  ...
  public
    ...
    public
      var OrigDims,
      pnlControlOrigDims,
  		pnlVideoDisplayOrigDims,
  		pnlVideoControlsOrigDims,
      pnlEditingControlsOrigDims,
  		pnlTimingControlsOrigDims,
  		pnlTextControlsOrigDims,
      pnlParent2OrigDims,
      pnlParent1OrigDims,
      pnlVideoOrigDims,
      sbStatusBarOrigDims,
      tbMainToolBarOrigDims
      : TOrigDims;
  end;
...
procedure TfrmMain.GetDims(Control: TWinControl; var Dims: TOrigDims);
var
  I, P: Integer;
begin
  Dims.OrigW := abs(Control.Width); // original top sizes
  Dims.OrigH := abs(Control.Height);

  if Control is TWinControl then
  begin
    for I := 0 to TWinControl(Control).ControlCount - 1 do
    // original lower level sizes
    begin
      P := Length(Dims.OrigDimsArr);

      SetLength(Dims.OrigDimsArr, P + 1);

      Dims.OrigDimsArr[P, 0] := abs(TWinControl(Control).Controls[I].Width);
      Dims.OrigDimsArr[P, 1] := abs(TWinControl(Control).Controls[I].Height);
      Dims.OrigDimsArr[P, 2] := abs(TWinControl(Control).Controls[I].Left);
      Dims.OrigDimsArr[P, 3] := abs(TWinControl(Control).Controls[I].Top);

      if TWinControl(Control).Controls[I] is TLabel then
      begin
        TLabel(TWinControl(Control).Controls[I]).AutoSize := false;
        Dims.OrigDimsArr[P, 0] := 2 * Dims.OrigDimsArr[P, 0]; // double the width
        Dims.OrigDimsArr[P, 1] := 25;
      end;
    end;
  end;
end;

procedure TfrmMain.GetOrigDims; // Dany
var
  I, P: Integer;
begin
  GetDims(Self, OrigDims);
  GetDims(pnlControl, pnlControlOrigDims);
  GetDims(pnlVideoDisplay, pnlVideoDisplayOrigDims);
  GetDims(pnlVideoControls, pnlVideoControlsOrigDims);
  GetDims(pnlEditingControls, pnlEditingControlsOrigDims);
  GetDims(pnlTimingControls, pnlTimingControlsOrigDims);
  GetDims(pnlTextControls, pnlTextControlsOrigDims);
  GetDims(pnlParent2, pnlParent2OrigDims);
  GetDims(pnlParent1, pnlParent1OrigDims);
  GetDims(pnlVideo, pnlVideoOrigDims);
  GetDims(sbStatusBar, sbStatusBarOrigDims);
  GetDims(tbMainToolBar, tbMainToolBarOrigDims);
end;

procedure TfrmMain.ChangeDims(Control: TWinControl; var Dims: TOrigDims);
// Dany
var
  I: Integer;
  L, T, W, H: Integer;
  HFactor, VFactor: real;
  NewWidth: Integer;
  NewHeight: Integer;
begin
  NewWidth  := abs(Control.Width);
  NewHeight := abs(Control.Height);
  ...
  begin
    ...
    begin
      if Control.Controls[I] is TWinControl then
        (Control.Controls[I] as TWinControl).DisableAlign;

      HFactor := 1;
      if Dims.OrigW > 0 then
      HFactor := NewWidth  / Dims.OrigW;

      VFactor := 1;
      if Dims.OrigH > 0 then
      VFactor := NewHeight / Dims.OrigH;
      ...
      W := Trunc(HFactor * Dims.OrigDimsArr[I, 0]); // width
      H := Trunc(VFactor * Dims.OrigDimsArr[I, 1]); // height
      L := Trunc(HFactor * Dims.OrigDimsArr[I, 2]); // left
      T := Trunc(VFactor * Dims.OrigDimsArr[I, 3]); // top
      ...
      if (Control.Controls[I] is TComboBox) then // special case
      begin
        Control.Controls[I].Width := W;
        Control.Controls[I].Left  := L;
        Control.Controls[I].Top   := T;
        TComboBox(Control.Controls[I]).ItemHeight := H - 6;
      end
      else
      begin
        Control.Controls[I].SetBounds(L, T, W, H);
      end;
...
procedure TfrmMain.MyFormResize(Sender: TObject); // dany
begin
  ChangeDims(Self, OrigDims); // the form itself
  ChangeDims(pnlControl, pnlControlOrigDims);
  ChangeDims(pnlVideoDisplay, pnlVideoDisplayOrigDims);
  ChangeDims(pnlVideoControls, pnlVideoControlsOrigDims);
  ChangeDims(pnlEditingControls, pnlEditingControlsOrigDims);
  ChangeDims(pnlTimingControls, pnlTimingControlsOrigDims);
  ChangeDims(pnlTextControls, pnlTextControlsOrigDims);
  ChangeDims(pnlParent2, pnlParent2OrigDims);
  ChangeDims(pnlParent1, pnlParent1OrigDims);
  ChangeDims(pnlVideo, pnlVideoOrigDims);
  ChangeDims(sbStatusBar, sbStatusBarOrigDims);
  ChangeDims(tbMainToolBar, tbMainToolBarOrigDims);
end;
...
begin
  ...
  try
    ...
    // added by adenry: end
    lstSubtitles.ParentFont := False; // <----------- dany
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Data.FinalTime >= Data.InitialTime // dany
          then tmeDuration.Time := Data.FinalTime - Data.InitialTime
          else tmeDuration.Time := 0 // dany
        end;
        ...
        begin
          ...
          if Data.InitialTime >= Data2.FinalTime  // dany
          then tmePause.Time := Data.InitialTime - Data2.FinalTime
          else tmePause.Time := 0;  // dany
...
begin
  ...
  try
    ...
    // --------------------------------- //
    Position := poDesigned;
    Left := Ini.ReadInteger('Main Window', 'Left', (Screen.Width div 2) -
      (frmMain.Width div 2));
    Top := Ini.ReadInteger('Main Window', 'Top', (Screen.Height div 2) -
      (frmMain.Height div 2));
    Width := Ini.ReadInteger('Main Window', 'Width', 724);
    Height := Ini.ReadInteger('Main Window', 'Height', 605);
    ...
    begin
      ...
      chkOCRScripts.Top := cmbTransCharset.Top + cmbTransCharset.Height + 3;
      // added by adenry
      cmbOrgCharset.Show;
    ...
    cmbOCRScripts.Top := chkOCRScripts.Top + chkOCRScripts.Height;
    // added by adenry
    ...
    if Ini.ReadBool('Interface', 'Show lines count', True) then
      mnuShowLinesCount.Checked := True
    else
    begin
...
begin
  ...
  *)
  ...
  // Dany necessary to resize the mainmenu
  frmMain.mnuFile.Visible := not frmMain.mnuFile.Visible; // Dany

  tmp := TMenuItem.Create(mnuMain); // Dany
  tmp.Caption := 'Test';

  mnuFile.Insert(0, tmp); // Dany  Force all menuitem to be redrawn
  mnuFile.Delete(0);

  mnuEdit.Insert(0, tmp); // Dany
  mnuEdit.Delete(0);

  mnuView.Insert(0, tmp); // Dany
  mnuView.Delete(0);

  mnuSearch.Insert(0, tmp); // Dany
  mnuSearch.Delete(0);

  mnuTools.Insert(0, tmp); // Dany
  mnuTools.Delete(0);

  mnuMovie.Insert(0, tmp); // Dany
  mnuMovie.Delete(0);

  mnuSettings.Insert(0, tmp); // Dany
  mnuSettings.Delete(0);

  mnuHelp.Insert(0, tmp); // Dany
  mnuHelp.Delete(0);
...
begin
  ...
  begin
    ...
    pnlParent2.Left := pnlControl.Left + pnlControl.Width + 7;  // Dany
    pnlParent2.Width := frmMain.ClientWidth -
      (pnlControl.Left + pnlControl.Width) - 15; // dany
  end
  else
  begin
    pnlControl.Visible := False;
    pnlParent2.Left := pnlControl.Left;              // dany
    pnlParent2.Width := frmMain.ClientWidth - 16;    // dany
  end;
  ...
  // added by adenry:
  pnlEditingControls.Left := pnlParent2.Left;   // dany
  pnlEditingControls.Width := pnlParent2.Width; // dany
...
begin
  ...
  UTF8_Trans := SubtitleAPI.Get_UTF8_File; // Dany
  RefreshStatusbar;
end;
...
begin
  ...
  if cmbOCRScripts.Visible then
    lblNotes.Top := cmbOCRScripts.Top + cmbOCRScripts.Height + 11
  else
