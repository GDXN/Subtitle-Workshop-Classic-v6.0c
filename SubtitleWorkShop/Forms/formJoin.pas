...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ComCtrls, StdCtrls, ExtCtrls, IniFiles, System.UITypes;
...
begin
  ...
  try
    ...
    begin
      Ini.WriteBool('Join', 'Maximized', True);
    end
    else
    begin
    Ini.WriteInteger('Join', 'Left', Left);
    Ini.WriteInteger('Join', 'Top', Top);
    Ini.WriteInteger('Join', 'Height', Height);
    Ini.WriteInteger('Join', 'Width', Width);
    Ini.WriteBool('Join', 'Maximized', False);
    end;
...
begin
  ...
  MyResize_ := TMyResize.Create(Self); // Kameleon

  //Constraints.MaxWidth := Width - ClientWidth + btnCancel.Left + btnCancel.Width + pnlJoin.Left; // Kameleon
  //Constraints.MinWidth := Constraints.MaxWidth;  // Kameleon
...
procedure TfrmJoin.FormResize(Sender: TObject);
begin
...
begin
  Ini := TIniFile.Create(IniRoot); // Kameleon
  try
    L := Ini.ReadInteger('Join', 'Left', 192);
    T := Ini.ReadInteger('Join', 'Top', 103);
    H := Ini.ReadInteger('Join', 'Height', 533);
    W := Ini.ReadInteger('Join', 'Width', 858);

    Font.Size := frmMain.Font.Size;
    SetBounds(L, T, W, H);

  if Ini.ReadBool('Join', 'Maximized', False) then
