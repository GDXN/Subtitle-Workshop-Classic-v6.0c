...
type
  ...
  private
    ...
    procedure UpDownChangingEx(Sender: TObject; var AllowChange: Boolean; NewValue: {SmallInt:} Integer; Direction: TUpDownDirection);
...
begin
  ...
  begin
    ...
    if TimeToChange >= 0 then // Kameleon
    if (Time + {Cardinal}(TimeToChange)) <= MaxTime then
      Time := Time + {Cardinal}(TimeToChange);

    if TimeToChange < 0 then                             // Kameleon
    if (Time + {Cardinal}(TimeToChange)) >= MinTime then  // Kameleon
      Time := Time + {Cardinal}(TimeToChange);           // Kameleon
...
procedure TTimeMaskEdit.UpDownChangingEx(Sender: TObject; var AllowChange: Boolean; NewValue: {SmallInt} Integer; Direction: TUpDownDirection);
begin
  if (ReadOnly = False) then
