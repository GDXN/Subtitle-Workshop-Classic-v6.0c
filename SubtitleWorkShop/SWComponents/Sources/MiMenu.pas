...
uses
  Windows, Classes, Graphics, Menus, Forms, SysUtils, System.UiTypes;
...
begin
  ...
  if MenuPrincipal = True then
    //Inc(Width, 2)              // Kameleon
    Width := ACanvas.TextWidth(Texto) + 0 // Kameleon
    else
    Width := ACanvas.TextWidth(Texto+'WWW') + {15} Acanvas.Font.Size; // Kameleon

  if (Sender As TMenuItem).Caption <> '-' then
    Height := ACanvas.TextHeight((Sender As TMenuItem).Caption) + 8
  else
    //Height:= 3; // Kameleon
    Height := ACanvas.TextHeight((Sender As TMenuItem).Caption) + 8; // Kameleon
end;
