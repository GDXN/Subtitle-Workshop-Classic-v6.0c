...
uses
  ...
  Forms, ComCtrls, Math, RichEdit, FastStrings, System.UITypes, System.Types; //added by adenry
...
begin
  ...
  //Replace BOLD tags
  tPos := SmartPos(b, S, False);
  While(tPos > 0) do
  begin
    t2Pos := SmartPos(b2, S, False);
  ...
  //Replace ITALIC tags
  tPos := SmartPos(i, S{re.Text}, False);
  While(tPos > 0) do
  begin
    t2Pos := SmartPos(i2, S{re.Text}, False);
  ...
  tPos := SmartPos(u, S, False);
  While(tPos > 0) do
  begin
    t2Pos := SmartPos(u2, S, False);
  ...
  //Replace COLOR tags
  tPos := SmartPos(c, S, False);
  SearchStart := 1;
  While(tPos > 0) do
  begin
    if (Copy(S, tPos+10, 1) = '>') and ( (SmartPos('<', S, False, tPos+4) > tPos+10) or (SmartPos('<', S, False, tPos+4)=0) ) then
    begin  //it's a proper <c:#XXXXXX> tag:
      t2Pos := SmartPos(c2, S, False, tPos+4); //nearest </c> tag
      ...
      while (True) do
      begin
        tPos2 := SmartPos(c, S, False, SelStart); //get nearest <c:# tag
        if tPos2 = 0 then break; //there are no more <c:# tags, so break
        if Copy(S, tPos2+10, 1) <> '>' then
      ...
      if (Color[1] in HexChars)and(Color[2] in HexChars)and(Color[3] in HexChars)and(Color[4] in HexChars)and(Color[5] in HexChars)and(Color[6] in HexChars) then
        re.SelAttributes.Color := RGB(StrToInt('$'+Copy(S, tPos + 4, 2)), StrToInt('$'+Copy(S, tPos + 6, 2)),StrToInt('$'+Copy(S, tPos + 8, 2)));
    ...
    begin //it's not a proper <c:#XXXXXX> tag:
      t2Pos := SmartPos('>', S, False, tPos+4);
      if (t2Pos > 0) and ( (t2Pos < SmartPos('<', S, False, tPos+4)) or (SmartPos('<', S, False, tPos+4)=0) ) then
    ...
    tPos := SmartPos(c, S, False, SearchStart);
  end;
...
var
  i, {CharIdx, }NeededWidth: integer; // Kameleon
  {RHSPt, }CharPt : TPoint;           // Kameleon
  al: TAlignment;
const
  MinWidth : integer = 10;
begin
...
*)
  ...
  NeededWidth := MinWidth;
  re.width := 4000;
  for I := 1 to Length(re.text) - 1 do
  begin
    re.Perform(EM_POSFROMCHAR, Integer(@CharPt), I);
    if CharPt.x > NeededWidth then NeededWidth := CharPt.x;
  end;
  re.Width := NeededWidth  * 55 div 40; // + re.Width - re.ClientWidth;
  re.Paragraph.Alignment := al; //restore alignment
