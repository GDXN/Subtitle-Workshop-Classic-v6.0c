...
begin
  ...
  begin
   ...
   {$IFNDEF FPC} //RegisterProperty('OnDrawItem', 'TMenuDrawItemEvent', iptrw);  // Kameleon
    //RegisterProperty('OnAdvancedDrawItem', 'TAdvancedMenuDrawItemEvent', iptrw); // Kameleon
    //RegisterProperty('OnMeasureItem', 'TMenuMeasureItemEvent', iptrw); // Kameleon
   {$ENDIF}
  end;
end;
...
{$IFNDEF FPC}
  //Cl.addTypeS('TMenuDrawItemEvent', 'procedure (Sender: TObject; ACanvas: TCanvas; ARect: TRect; Selected: Boolean)'); // Kameleon

  //Cl.addTypeS('TAdvancedMenuDrawItemEvent', 'procedure (Sender: TObject; ACanvas: TCanvas; ARect: TRect; State: TOwnerDrawState)'); // Kameleon

  //Cl.addTypeS('TMenuMeasureItemEvent', 'procedure (Sender: TObject; ACanvas: TCanvas; var Width, Height: Integer)'); // Kameleon
...
{$ENDIF}
  ...
  // Cl.addTypeS('TMenuChangeEvent', 'procedure (Sender: TObject; Source: TMenuItem; Rebuild: Boolean)'); // Kameleon
...
//TODO: it should work,but somehow TShiftState is not defined
  //Cl.AddDelphiFunction('function ShortCut(Key: Word; Shift: TShiftState): TShortCut'); // Kameleon

  //Cl.AddDelphiFunction('procedure ShortCutToKey(ShortCut: TShortCut; var Key: Word; var Shift: TShiftState)'); // Kameleon
...
{$ENDIF}
  ...
  //Cl.AddDelphiFunction('function NewMenu(Owner: TComponent; const AName: string; Items: array of TMenuItem): TMainMenu'); // Kameleon

  //Cl.AddDelphiFunction('function NewPopupMenu(Owner: TComponent; const AName: string; Alignment: TPopupAlignment; AutoPopup: Boolean; const Items: array of TCMenuItem): TPopupMenu'); // Kameleon


  //Cl.AddDelphiFunction('function NewSubMenu(const ACaption: string; HCTX: Word; const AName: string; Items: array of TMenuItem; AEnabled: Boolean): TMenuItem'); // Kameleon

  //Cl.AddDelphiFunction('function NewItem(const ACaption: string; AShortCut: TShortCut; Achecked, AEnabled: Boolean; AOnClick: TNotifyEvent; HCTX: Word; const AName: string): TMenuItem'); // Kameleon


  Cl.AddDelphiFunction('function NewLine: TMenuItem');
{$IFNDEF FPC}
  //Cl.AddDelphiFunction('procedure DrawMenuItem(MenuItem: TMenuItem; ACanvas: TCanvas; ARect: TRect; State: TOwnerDrawState)'); // Kameleon
